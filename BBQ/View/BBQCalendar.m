  ...
  if (self = [super init]) {
    ...
    leftButton.frame =
        CGRectMake(type == BBQCalendarTypeDailyReport ? 0 : 15,
                   kFSCalendarDefaultHeaderHeight / 2.0 - 12, 24, 24);
    rightButton.frame =
...
- (void)setupAppearance {
  self.appearance.autoAdjustTitleSize = NO;
  ...
  self.appearance.titleTextSize = 16;
  self.appearance.headerTitleTextSize = 17;
...
- (void)scrollCalendar:(UIButton *)button {
  ...
  if (button == self.leftButton) {
    [self.collectionView
        setContentOffset:CGPointMake(currentOffset.x - self.fs_width,
                                     currentOffset.y)
                animated:YES];
  } else {
    [self.collectionView
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView {
  ...
  self.currentPage =
      [self.minimumDate fs_dateByAddingMonths:scrollView.contentOffset.x /
                                              scrollView.fs_width]
