    ...
    if (!_displayLink && self.animatedImage) {
      _displayLink =
          [CADisplayLink displayLinkWithTarget:self
                                      selector:@selector(changeKeyframe:)];
...
- (BOOL)isAnimating {
  return
      [super isAnimating] || (self.displayLink && !self.displayLink.isPaused);
}
...
- (void)changeKeyframe:(CADisplayLink *)displayLink {
  ...
  while (self.accumulator >=
         self.animatedImage.frameDurations[self.currentFrameIndex]) {
    self.accumulator -=
        self.animatedImage.frameDurations[self.currentFrameIndex];
    if (++self.currentFrameIndex >= [self.animatedImage.images count]) {
    ...
    self.currentFrameIndex =
        MIN(self.currentFrameIndex, [self.animatedImage.images count] - 1);
    self.currentFrame =
        [self.animatedImage getFrameWithIndex:self.currentFrameIndex];
    [self.layer setNeedsDisplay];
  }
}
...
- (void)didMoveToSuperview {
  ...
  if (self.superview) {
    ...
    // Doesn't have superview, let's check later if we need to remove the
    // displayLink
    dispatch_async(dispatch_get_main_queue(), ^{
      CADisplayLink *curDisplayLink;
      curDisplayLink = [self displayLink];
