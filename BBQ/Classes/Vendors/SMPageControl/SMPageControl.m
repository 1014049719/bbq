...
// Page Control used for stealing page number localizations for accessibility
// labels
// I'm not sure I love this technique, but it's the best way to get exact
...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect {
  NSMutableArray *pageRects =
      [NSMutableArray arrayWithCapacity:self.numberOfPages];
  ...
  for (NSInteger i = 0; i < _numberOfPages; i++) {
    ...
    if (i == _displayedPage) {
      fillColor = _currentPageIndicatorTintColor
      ...
      fillColor = _pageIndicatorTintColor
                      ? _pageIndicatorTintColor
                      : [[UIColor whiteColor] colorWithAlphaComponent:0.3f];
    ...
    if (image) {
      ...
      CGFloat centeredXOffset =
          xOffset + floorf((_measuredIndicatorWidth - image.size.width) / 2.0f);
      [image drawAtPoint:CGPointMake(centeredXOffset, yOffset)];
      indicatorRect = CGRectMake(centeredXOffset, yOffset, image.size.width,
                                 image.size.height);
    } else if (maskingImage) {
      yOffset = [self _topOffsetForHeight:maskSize.height rect:rect];
      CGFloat centeredXOffset =
          xOffset + floorf((_measuredIndicatorWidth - maskSize.width) / 2.0f);
      indicatorRect =
          CGRectMake(centeredXOffset, yOffset, maskSize.width, maskSize.height);
      CGContextDrawImage(context, indicatorRect, maskingImage);
    } else {
      yOffset = [self _topOffsetForHeight:_indicatorDiameter rect:rect];
      CGFloat centeredXOffset =
          xOffset +
          floorf((_measuredIndicatorWidth - _indicatorDiameter) / 2.0f);
      indicatorRect = CGRectMake(centeredXOffset, yOffset, _indicatorDiameter,
                                 _indicatorDiameter);
...
- (CGSize)sizeForNumberOfPages:(NSInteger)pageCount {
  ...
  CGSize size =
      CGSizeMake(marginSpace + indicatorSpace, _measuredIndicatorHeight);
  return size;
}
...
- (CGRect)rectForPageIndicator:(NSInteger)pageIndex {
  ...
  CGRect rect = CGRectMake(left + size.width - _measuredIndicatorWidth, 0,
...
- (void)_setImage:(UIImage *)image
          forPage:(NSInteger)pageIndex
             type:(SMPageControlImageType)type {
  if (pageIndex < 0 || pageIndex >= _numberOfPages) {
    return;
  }
...
- (CGSize)intrinsicContentSize {
  ...
  CGSize intrinsicContentSize = CGSizeMake(
      UIViewNoIntrinsicMetric, MAX(_measuredIndicatorHeight, _minHeight));
  return intrinsicContentSize;
}

- (void)updatePageNumberForScrollView:(UIScrollView *)scrollView {
  NSInteger page =
      (int)floorf(scrollView.contentOffset.x / scrollView.bounds.size.width);
  self.currentPage = page;
}

- (void)setScrollViewContentOffsetForCurrentPage:(UIScrollView *)scrollView
...
- (CGImageRef)createMaskForImage:(UIImage *)image CF_RETURNS_RETAINED {
  ...
  CGContextRef context = CGBitmapContextCreate(
      NULL, pixelsWide, pixelsHigh, CGImageGetBitsPerComponent(image.CGImage),
      bitmapBytesPerRow, NULL, (CGBitmapInfo)kCGImageAlphaOnly);
  CGContextTranslateCTM(context, 0.f, pixelsHigh);
  CGContextScaleCTM(context, 1.0f, -1.0f);

  CGContextDrawImage(context, CGRectMake(0, 0, pixelsWide, pixelsHigh),
                     image.CGImage);
...
- (void)_updateMeasuredIndicatorSizes {
  ...
  if ((self.pageIndicatorImage || self.pageIndicatorMaskImage) &&
  ...
  if (self.currentPageIndicatorImage) {
    [self _updateMeasuredIndicatorSizeWithSize:self.currentPageIndicatorImage
                                                   .size];
  }

  if (self.pageIndicatorMaskImage) {
    [self
        _updateMeasuredIndicatorSizeWithSize:self.pageIndicatorMaskImage.size];
  }
...
// We're using touchesEnded: because we want to mimick UIPageControl as close as
// possible
// As of iOS 6, UIPageControl still (as far as we know) does not use a tap
// gesture recognizer. This means that actions like
// touching down, sliding around, and releasing, still results in the page
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (SMPageControlTapBehaviorJump == self.tapBehavior) {
    ...
    [self.pageRects enumerateObjectsUsingBlock:^(NSValue *value,
...
- (void)updateAccessibilityValue {
  ...
  NSString *accessibilityValue =
      self.accessibilityPageControl.accessibilityValue;

  if (pageName) {
    self.accessibilityValue =
        [NSString stringWithFormat:@"%@ - %@", pageName, accessibilityValue];
  } else {
