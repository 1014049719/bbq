    ...
    if (_needsAdjustingViewFrame || CGSizeEqualToSize(_titleLabel.frame.size, CGSizeZero)) {
        ...
        if (_subtitle) {
            ...
            _titleLabel.frame = CGRectMake(0,
                                           (self.contentView.height*5.0/6.0-height)*0.5+_appearance.titleVerticalOffset,
                                           self.width,
                                           titleHeight);
            
            _subtitleLabel.frame = CGRectMake(0,
                                              _titleLabel.bottom - (_titleLabel.height-_titleLabel.font.pointSize)+_appearance.subtitleVerticalOffset,
                                              self.width,
                                              subtitleHeight);
        } else {
            _titleLabel.frame = CGRectMake(0, _appearance.titleVerticalOffset, self.width, floor(self.contentView.height*5.0/6.0));
        }
...
{
    if (self.hasEvent) {
        return self.preferedEventColor;
    }
    if (self.dateIsSelected || self.isSelected) {
