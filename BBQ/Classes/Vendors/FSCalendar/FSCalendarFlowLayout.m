...
- (void)prepareLayout
{
    ...
    if (!_calendar.floatingMode) {
        ...
        switch (_calendar.scope) {
            case FSCalendarScopeMonth: {
                self.itemSize = CGSizeMake(
                                           self.collectionView.width/7-(self.scrollDirection == UICollectionViewScrollDirectionVertical)*0.1,
                                           rowHeight
                                          );
                CGFloat padding = (self.collectionView.height-rowHeight*6)/2;
                self.sectionInset = UIEdgeInsetsMake(padding, 0, padding, 0);
                break;
            }
            case FSCalendarScopeWeek: {
                self.itemSize = CGSizeMake(self.collectionView.width/7, rowHeight);
                CGFloat padding = (self.collectionView.height-rowHeight)/2;
                self.sectionInset = UIEdgeInsetsMake(padding, 0, padding, 0);
                break;
            }
            default: {
        ...
        self.headerReferenceSize = CGSizeMake(self.collectionView.width, headerHeight);
        self.itemSize = CGSizeMake(
                                   self.collectionView.width/7-(self.scrollDirection == UICollectionViewScrollDirectionVertical)*0.1,
                                   rowHeight
                                  );
