 ...
 activityIndicatorView:(UIView *)activity {
    self = [super
            initWithFrame:CGRectMake(
                                     0, -(kTotalViewHeight + scrollView.contentInset.top),
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:@"contentInset"]) {
        if (!_ignoreInset) {
            ...
            self.frame =
            CGRectMake(0, -(kTotalViewHeight + self.scrollView.contentInset.top),
                       self.scrollView.frame.size.width, kTotalViewHeight);
        }
        return;
    }
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            CGPointMake(0, kMaxDistance + offset + kOpenedViewHeight);
            [CATransaction commit];
            
            _activity.center = CGPointMake(
            ...
            if (offset < 0) {
                ...
                if (offset >= -kOpenedViewHeight) {
                    if (!self.scrollView.dragging) {
                        ...
                        [self.scrollView
                         setContentInset:UIEdgeInsetsMake(
                                                          -offset + self.originalContentInset.top,
                                                          self.originalContentInset.left,
                                                          self.originalContentInset.bottom,
                                                          self.originalContentInset.right)];
                    }
    ...
    // Top semicircle
    CGPathAddArc(path, NULL, topOrigin.x, topOrigin.y, currentTopRadius, 0, M_PI,
                 YES);
    ...
    CGPathAddCurveToPoint(path, NULL, leftCp1.x, leftCp1.y, leftCp2.x, leftCp2.y,
                          leftDestination.x, leftDestination.y);
    
    // Bottom semicircle
    CGPathAddArc(path, NULL, bottomOrigin.x, bottomOrigin.y, currentBottomRadius,
                 M_PI, 0, YES);
...
- (void)beginRefreshing {
    if (!_refreshing) {
        ...
        _ignoreInset = YES;
        [self.scrollView
         setContentInset:UIEdgeInsetsMake(kOpenedViewHeight +
                                          self.originalContentInset.top,
                                          self.originalContentInset.left,
                                          self.originalContentInset.bottom,
                                          self.originalContentInset.right)];
...
- (void)endRefreshing {
    if (_refreshing) {
        ...
        [UIView animateWithDuration:0.4 animations:^{
            ...
            [blockScrollView.parallaxView layoutIfNeeded];
            _ignoreInset = NO;
            _activity.alpha = 0;
            _activity.layer.transform = CATransform3DMakeScale(0.1, 0.1, 1);
