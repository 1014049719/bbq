...
@interface AGEmojiKeyboardView () <UIScrollViewDelegate,
                                   AGEmojiPageViewDelegate>
...
- (NSDictionary *)emojis {
  if (!_emojis) {
    NSString *plistPath =
        [[NSBundle mainBundle] pathForResource:@"emotion_list" ofType:@"plist"];
...
- (NSString *)categoryNameAtIndex:(NSUInteger)index {
  //    NSArray *categoryList = @[segmentRecentName, @"People", @"Objects",
  //    @"Nature", @"Places", @"Symbols"];
...
- (AGEmojiKeyboardViewCategoryImage)defaultSelectedCategory {
  if ([self.dataSource
          respondsToSelector:@selector(defaultCategoryForEmojiKeyboardView:)]) {
...
- (NSUInteger)recentEmojisMaintainedCount {
  if ([self.dataSource
          respondsToSelector:
              @selector(recentEmojisMaintainedCountForEmojiKeyboardView:)]) {
    return
        [self.dataSource recentEmojisMaintainedCountForEmojiKeyboardView:self];
  }
...
- (NSArray *)imagesForSelectedSegments {
  ...
  dispatch_once(&onceToken, ^{
    ...
    for (AGEmojiKeyboardViewCategoryImage i =
             AGEmojiKeyboardViewCategoryImageEmoji;
         i <= AGEmojiKeyboardViewCategoryImageMonkey_Gif; ++i) {
      [array addObject:[self.dataSource emojiKeyboardView:self
                                 imageForSelectedCategory:i]];
    }
  });
  return array;
}
...
- (NSMutableArray *)recentEmojis {
  NSArray *emojis = [[NSUserDefaults standardUserDefaults]
      arrayForKey:RecentUsedEmojiCharactersKey];
...
- (void)setRecentEmojis:(NSMutableArray *)recentEmojis {
  ...
  if ([recentEmojis count] > self.recentEmojisMaintainedCount) {
    NSIndexSet *indexesToBeRemoved = [NSIndexSet
        indexSetWithIndexesInRange:NSMakeRange(
                                       self.recentEmojisMaintainedCount,
                                       [recentEmojis count] -
                                           self.recentEmojisMaintainedCount)];
    [recentEmojis removeObjectsAtIndexes:indexesToBeRemoved];
  }
  [[NSUserDefaults standardUserDefaults]
      setObject:recentEmojis
         forKey:RecentUsedEmojiCharactersKey];
}
...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self) {
    ...
    if (!showBigEmotion) {
      self.easeTabBar = [[UIEaseTabBar alloc]
             initWithFrame:CGRectMake(0, (self_Height - easeTabBar_Height),
    ...
    self.pageControl.pageIndicatorImage =
        [UIImage imageNamed:@"keyboard_page_unselected"];
    self.pageControl.currentPageIndicatorImage =
        [UIImage imageNamed:@"keyboard_page_selected"];
    self.pageControl.hidesForSinglePage = YES;
    self.pageControl.backgroundColor = [UIColor clearColor];
    CGSize pageControlSize = [self.pageControl sizeForNumberOfPages:3];
    NSUInteger numberOfPages = [self
        numberOfPagesForCategory:self.category
                     inFrameSize:CGSizeMake(CGRectGetWidth(self.bounds),
                                            CGRectGetHeight(self.bounds) -
                                                CGRectGetHeight(
                                                    self.easeTabBar.bounds) -
                                                pageControlSize.height)];
    self.pageControl.numberOfPages = numberOfPages;
    pageControlSize = [self.pageControl sizeForNumberOfPages:numberOfPages];
    self.pageControl.frame = CGRectIntegral(
        CGRectMake((CGRectGetWidth(self.bounds) - pageControlSize.width) / 2,
                   CGRectGetHeight(self.bounds) - easeTabBar_Height -
                       pageControlSize.height - 10.0,
    ...
    self.scrollView = [[UIScrollView alloc]
        initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds),
                                 CGRectGetHeight(self.bounds) -
                                     CGRectGetHeight(self.easeTabBar.bounds) -
                                     pageControlSize.height - 20.0)];
...
- (void)layoutSubviews {
  ...
  NSUInteger numberOfPages = [self
      numberOfPagesForCategory:self.category
                   inFrameSize:CGSizeMake(
                                   CGRectGetWidth(self.bounds),
                                   CGRectGetHeight(self.bounds) -
                                       CGRectGetHeight(self.easeTabBar.bounds) -
                                       pageControlSize.height)];

  NSInteger currentPage = (self.pageControl.currentPage > numberOfPages)
                              ? numberOfPages
                              : self.pageControl.currentPage;

  // if (currentPage > numberOfPages) it is set implicitly to max pageNumber
  // available
  self.pageControl.numberOfPages = numberOfPages;
  pageControlSize = [self.pageControl sizeForNumberOfPages:numberOfPages];
  self.pageControl.frame = CGRectIntegral(CGRectMake(
      (CGRectGetWidth(self.bounds) - pageControlSize.width) / 2,
      CGRectGetHeight(self.bounds) - CGRectGetHeight(self.easeTabBar.bounds) -
          pageControlSize.height - 10.0,
      pageControlSize.width, pageControlSize.height));

  self.scrollView.frame = CGRectMake(
      0, 0, CGRectGetWidth(self.bounds),
      CGRectGetHeight(self.bounds) - CGRectGetHeight(self.easeTabBar.bounds) -
          pageControlSize.height - 20.0);
  [self.scrollView.subviews
      makeObjectsPerformSelector:@selector(removeFromSuperview)];
  self.scrollView.contentOffset =
      CGPointMake(CGRectGetWidth(self.scrollView.bounds) * currentPage, 0);
  self.scrollView.contentSize =
      CGSizeMake(CGRectGetWidth(self.scrollView.bounds) * numberOfPages,
...
- (BOOL)requireToSetPageViewForIndex:(NSUInteger)index {
  ...
  for (AGEmojiPageView *page in self.pageViews) {
    if ((page.frame.origin.x / CGRectGetWidth(self.scrollView.bounds)) ==
        index) {
      return NO;
    }
  }
...
- (AGEmojiPageView *)synthesizeEmojiPageView {
  ...
  NSUInteger columns =
      [self numberOfColumnsForFrameSize:self.scrollView.bounds.size];

  AGEmojiPageView *pageView = [[AGEmojiPageView alloc]
             initWithFrame:CGRectMake(0, 0,
                                      CGRectGetWidth(self.scrollView.bounds),
                                      CGRectGetHeight(self.scrollView.bounds))
      backSpaceButtonImage:[self.dataSource
                               backSpaceButtonImageForEmojiKeyboardView:self]
...
- (AGEmojiPageView *)usableEmojiPageView {
  ...
  for (AGEmojiPageView *page in self.pageViews) {
    NSUInteger pageNumber =
        page.frame.origin.x / CGRectGetWidth(self.scrollView.bounds);
...
- (void)setEmojiPageViewInScrollView:(UIScrollView *)scrollView
  ...
  NSUInteger columns =
      [self numberOfColumnsForFrameSize:scrollView.bounds.size];
  ...
  pageView.frame = CGRectMake(index * CGRectGetWidth(scrollView.bounds), 0,
...
- (NSUInteger)numberOfPagesForCategory:(NSString *)category
                           inFrameSize:(CGSize)frameSize {
  ...
  NSUInteger numberOfPages =
      (NSUInteger)ceil((float)emojiCount / numberOfEmojisOnAPage);
  DebugLog(@"%@ %@ %@ :: %@", @(numberOfRows), @(numberOfColumns),
           @(emojiCount), @(numberOfPages));
...
- (NSMutableArray *)emojiTextsForCategory:(NSString *)category
  ...
  NSIndexSet *index = [[NSIndexSet alloc]
      initWithIndexesInRange:NSMakeRange(start, end - start)];
...
- (void)emojiPageView:(AGEmojiPageView *)emojiPageView
...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  self = [super initWithFrame:frame];
  if (self) {
    [self addLineUp:YES andDown:NO andColor:[UIColor colorWithHexString:@"dddddd"]];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex {
  if (selectedIndex != _selectedIndex) {
    ...
    for (int i = 0; i < _numOfTabs; i++) {
      ...
      if (i == selectedIndex) {
        [tabButton setImage:self.selectedImages[i]
                   forState:UIControlStateNormal];
        [tabButton setBackgroundColor:[UIColor colorWithHexString:@"e5e5e5"]];
      } else {
        [tabButton setImage:self.unSelectedImages[i]
                   forState:UIControlStateNormal];
...
- (UIButton *)tabButtonWithIndex:(NSInteger)index {
  UIButton *button = [[UIButton alloc]
      initWithFrame:CGRectMake(_buttonWidth * index, 0, _buttonWidth,
                               CGRectGetHeight(self.frame))];
  UIView *lineView =
      [[UIView alloc] initWithFrame:CGRectMake(_buttonWidth - 0.5, 0, 0.5,
                                               CGRectGetHeight(self.frame))];
  lineView.backgroundColor = [UIColor colorWithHexString:@"dddddd"];
  [button addSubview:lineView];
  [button addTarget:self
                action:@selector(tabButtonClicked:)
...
- (void)configScrollView {
  self.scrollView = [[UIScrollView alloc]
      initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame) - _buttonWidth,
                               CGRectGetHeight(self.frame))];
...
- (void)configSendButton {
  self.sendButton = [[UIButton alloc]
      initWithFrame:CGRectMake(CGRectGetWidth(self.frame) - self.buttonWidth, 0,
                               self.buttonWidth, CGRectGetHeight(self.frame))];
  [self.sendButton setBackgroundColor:[UIColor colorWithHexString:@"ff6440"]];
