...
- (void)showFailure {
  ...
  if (_failureLabel == nil) {
    ...
    _failureLabel.textColor = [UIColor colorWithHexString:@"ff6440"];
    _failureLabel.backgroundColor = [UIColor clearColor];
    _failureLabel.autoresizingMask = UIViewAutoresizingFlexibleTopMargin |
...
- (void)setProgress:(float)progress {
  ...
  if (progress >= 1.0) {
    [_progressView performSelectorOnMainThread:@selector(removeFromSuperview)
                                    withObject:nil
                                 waitUntilDone:YES];
  }
}
