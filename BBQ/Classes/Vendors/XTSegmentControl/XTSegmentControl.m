...
- (id)initWithFrame:(CGRect)frame
              title:(NSString *)title
               type:(XTSegmentControlItemType)type {
  if (self = [super initWithFrame:frame]) {
    self.backgroundColor = [UIColor clearColor];
    _type = type;
    switch (_type) {
    case XTSegmentControlItemTypeIconUrl: {
//      _titleIconView = [[UIImageView alloc]
//          initWithFrame:CGRectMake((CGRectGetWidth(self.bounds) - 40) / 2,
//                                   (CGRectGetHeight(self.bounds) - 40) / 2, 40,
//                                   40)];
//      [_titleIconView doCircleFrame];
//      if (title) {
//          [_titleIconView setImageWithURL:[title urlImageWithCodePathResizeToView:_titleIconView] placeholder:kPlaceholderMonkeyRoundView(_titleIconView)];
//      } else {
...
//      [self addSubview:_titleIconView];
    ...
    case XTSegmentControlItemTypeTitleAndIcon: {
      _titleLabel = ({
        ...
        label.font = [UIFont
            systemFontOfSize:(kDevice_Is_iPhone6Plus)
                                 ? (XTSegmentControlItemFont + 1)
                                 : (kDevice_Is_iPhone6
                                        ? XTSegmentControlItemFont
                                        : XTSegmentControlItemFont - 2)];
        label.textAlignment = NSTextAlignmentCenter;
        label.text = title;
        label.textColor = [UIColor colorWithHexString:@"0x222222"];
        label.backgroundColor = [UIColor clearColor];
        [label sizeToFit];
        if (label.frame.size.width >
            CGRectGetWidth(self.bounds) - XTSegmentControlIconSpace - 10) {
      ...
      _titleIconView = [[UIImageView alloc]
          initWithFrame:CGRectMake(x, (CGRectGetHeight(self.bounds) - 10) * 0.5,
                                   10, 10)];
    ...
    default: {
      _titleLabel = ({
        UILabel *label = [[UILabel alloc]
            initWithFrame:CGRectMake(XTSegmentControlHspace, 0,
                                     CGRectGetWidth(self.bounds) -
                                         2 * XTSegmentControlHspace,
                                     CGRectGetHeight(self.bounds))];
...
- (void)setSelected:(BOOL)selected {
  ...
  case XTSegmentControlItemTypeTitleAndIcon: {
    if (_titleLabel) {
      [_titleLabel
          setTextColor:(selected ? [UIColor colorWithHexString:[[BBQThemeManager sharedInstance] getTheCurThemeType]]
                                 : [UIColor colorWithHexString:@"0x222222"])];
    }
    if (_titleIconView) {
      [_titleIconView setImage:[UIImage imageNamed:selected ? @"tag_list_down"
                                                            : @"tag_list_up"]];
    }
  } break;
  default: {
...
- (void)resetTitle:(NSString *)title {
  ...
  if (_type == XTSegmentControlItemTypeTitleAndIcon) {
    ...
    if (_titleLabel.frame.size.width >
        CGRectGetWidth(self.bounds) - XTSegmentControlIconSpace - 10) {
    ...
    _titleLabel.center = CGPointMake(
        (CGRectGetWidth(self.bounds) - XTSegmentControlIconSpace - 10) * 0.5,
        CGRectGetHeight(self.bounds) * 0.5);

    CGRect frame = _titleIconView.frame;
    frame.origin.x =
        CGRectGetMaxX(_titleLabel.frame) + XTSegmentControlIconSpace;
...
- (id)initWithFrame:(CGRect)frame
              Items:(NSArray *)titleItem
           withIcon:(BOOL)isIcon {
  if (self = [super initWithFrame:frame]) {
...
- (instancetype)initWithFrame:(CGRect)frame
                        Items:(NSArray *)titleItem
                     delegate:(id<XTSegmentControlDelegate>)delegate {
...
- (instancetype)initWithFrame:(CGRect)frame
                        Items:(NSArray *)titleItem
                     withIcon:(BOOL)isIcon
                selectedBlock:(XTSegmentControlBlock)selectedHandle {
...
- (void)transformAction:(NSInteger)index {
  if (self.delegate &&
      [self.delegate conformsToProtocol:@protocol(XTSegmentControlDelegate)] &&
      [self.delegate
          respondsToSelector:@selector(segmentControl:selectedIndex:)]) {
...
- (void)initItemsWithTitleArray:(NSArray *)titleArray withIcon:(BOOL)isIcon {
  ...
  if ([obj isKindOfClass:[NSString class]]) {
    ...
    for (int i = 0; i < titleArray.count; i++) {
      ...
      XTSegmentControlItem *item = [[XTSegmentControlItem alloc]
          initWithFrame:rect
                  title:title
                   type:isIcon ? XTSegmentControlItemTypeTitleAndIcon
                               : XTSegmentControlItemTypeTitle];
      if (!isIcon && i == 0) {
    ...
    //    } else if ([obj isKindOfClass:[ProjectMember class]] || [obj
    //    isKindOfClass:[Project class]]) {
    ////        全部任务的frame
    //        CGRect firstFrame = CGRectMake(5.0, 0, XTSegmentControlIconWidth,
    //        height);
    ...
    //            CGRect rect = CGRectMake(x, y, XTSegmentControlIconWidth,
    //            height);
    //            [_itemFrames addObject:[NSValue valueWithCGRect:rect]];
    //        }
    //
    ...
    //                item = [[XTSegmentControlItem alloc] initWithFrame:rect
    //                title:title.user.avatar
    //                type:XTSegmentControlItemTypeIconUrl];
    //            } else if ([obj isKindOfClass:[Project class]]){
    //                Project *title = titleArray[i];
    //                item = [[XTSegmentControlItem alloc] initWithFrame:rect
    //                title:title.icon type:XTSegmentControlItemTypeIconUrl];
    //            }
    //            if (item) {
    //                if (i == 0) {
    //                    [item setSelected:YES];
  ...
  [_contentView
      setContentSize:CGSizeMake(
                         CGRectGetMaxX([[_itemFrames lastObject] CGRectValue]),
                         CGRectGetHeight(self.bounds))];
  self.currentIndex = 0;
  [self selectIndex:0];
  if (isIcon) {
    ...
    for (int i = 1; i < _itemFrames.count; i++) {
      ...
      UIView *lineView = [[UIView alloc]
          initWithFrame:CGRectMake(CGRectGetMinX(rect),
                                   (CGRectGetHeight(rect) - 14) * 0.5, 1, 14)];
...
- (void)addRedLine {
  if (!_lineView) {
    ...
    _lineView.backgroundColor = [UIColor colorWithHexString:[[BBQThemeManager sharedInstance] getTheCurThemeType]];
    [_contentView addSubview:_lineView];

    UIView *bottomLineView = [[UIView alloc]
        initWithFrame:CGRectMake(0, CGRectGetHeight(rect) - 0.5,
                                 CGRectGetWidth(self.bounds), 0.5)];
    //        bottomLineView.backgroundColor = [UIColor
    //        colorWithHexString:@"0xc8c7cc"];
...
- (void)selectIndex:(NSInteger)index {
  ...
  if (index < 0) {
    ...
    if (index != _currentIndex) {
      ...
      CGRect lineRect =
          CGRectMake(CGRectGetMinX(rect) + XTSegmentControlHspace,
                     CGRectGetHeight(rect) - XTSegmentControlLineHeight,
                     CGRectGetWidth(rect) - 2 * XTSegmentControlHspace,
                     XTSegmentControlLineHeight);
      if (_currentIndex < 0) {
        ...
        [UIView animateWithDuration:XTSegmentControlAnimationTime
            animations:^{
              _lineView.frame = lineRect;
            }
            completion:^(BOOL finished) {
              [_items enumerateObjectsUsingBlock:^(XTSegmentControlItem *item,
...
- (void)moveIndexWithProgress:(float)progress {
  ...
  CGRect origionLineRect =
      CGRectMake(CGRectGetMinX(origionRect) + XTSegmentControlHspace,
                 CGRectGetHeight(origionRect) - XTSegmentControlLineHeight,
                 CGRectGetWidth(origionRect) - 2 * XTSegmentControlHspace,
                 XTSegmentControlLineHeight);
  ...
  if (delta > 0) {
    ...
    moveRect.size = CGSizeMake(CGRectGetWidth(origionLineRect) +
                                   delta * (CGRectGetWidth(lineRect) -
                                            CGRectGetWidth(origionLineRect)),
                               CGRectGetHeight(lineRect));
    moveRect.origin = CGPointMake(
        CGRectGetMidX(origionLineRect) +
            delta * (CGRectGetMidX(lineRect) - CGRectGetMidX(origionLineRect)) -
...
- (void)setScrollOffset:(NSInteger)index {
  if (_contentView.contentSize.width <= kScreenWidth) {
    return;
  }
  ...
  [UIView animateWithDuration:XTSegmentControlAnimationTime
                   animations:^{
                     [_contentView setContentOffset:CGPointMake(offset, 0)
                                           animated:NO];
                   }];
}
