...
- (void)setFetchContentViewAtIndex:
    (UIView * (^)(NSInteger index))fetchContentViewAtIndex {
...
- (id)initWithFrame:(CGRect)frame
  ...
  if (animationDuration > 0.0) {
    self.animationTimer = [NSTimer
        scheduledTimerWithTimeInterval:(self.animationDuration =
                                            animationDuration)
                                target:self
  ...
  if (self.totalPageCount > 0) {
    if (self.totalPageCount > 1) {
      ...
      self.scrollView.contentOffset =
          CGPointMake(CGRectGetWidth(self.scrollView.frame), 0);
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  if (self = [super initWithCoder:aDecoder]) {
    ...
    self.scrollView.contentSize =
        CGSizeMake(3 * CGRectGetWidth(self.scrollView.frame),
    ...
    self.animationTimer = [NSTimer
        scheduledTimerWithTimeInterval:(self.animationDuration = 3)
                                target:self
                              selector:@selector(animationTimerDidFired:)
                              userInfo:nil
                               repeats:YES];
...
- (void)configContentViews {
  ...
  for (UIView *contentView in self.contentViews) {
    ...
    rightRect.origin =
        CGPointMake(CGRectGetWidth(self.scrollView.frame) * (counter++), 0);
...
- (void)setScrollViewContentDataSource {
  NSInteger previousPageIndex =
      [self getValidNextPageIndexWithPageIndex:self.currentPageIndex - 1];
  ...
  if (self.fetchContentViewAtIndex) {
    id set =
        (self.totalPageCount == 1)
            ? [NSSet setWithObjects:@(previousPageIndex), @(_currentPageIndex),
                                    @(rearPageIndex), nil]
            : @[
              @(previousPageIndex),
              @(_currentPageIndex),
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  if (self.totalPageCount == 2) {
    if (scrollViewStartContentOffsetX < contentOffsetX) {
      ...
      tempView.frame = (CGRect){{2 * CGRectGetWidth(scrollView.frame), 0},
                                tempView.frame.size};
