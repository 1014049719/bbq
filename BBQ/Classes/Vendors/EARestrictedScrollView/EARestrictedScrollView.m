...
- (BOOL)checkIfScrollIndicator:(UIView *)view {
  return ((self.showsHorizontalScrollIndicator &&
           view.frame.size.height == 2.5f) ||
          (self.showsVerticalScrollIndicator &&
           view.frame.size.width == 2.5f)) &&
...
- (CGPoint)alignedContentOffset {
  ...
  CGPoint newOffset =
      CGPointMake(originalOffset.x + self.restrictionArea.origin.x,
                  originalOffset.y + self.restrictionArea.origin.y);
...
- (void)setAlignedContentOffset:(CGPoint)contentOffset {
  CGPoint newOffset =
      CGPointMake(contentOffset.x - self.restrictionArea.origin.x,
                  contentOffset.y - self.restrictionArea.origin.y);
...
- (void)setContentSize:(CGSize)contentSize {
  [self.containerView
      setFrame:CGRectMake(self.containerView.frame.origin.x,
                          self.containerView.frame.origin.y, contentSize.width,
                          contentSize.height)];
  [self setRestrictionArea:CGRectMake(self.restrictionArea.origin.x,
                                      self.restrictionArea.origin.y,
...
- (void)setRestrictionArea:(CGRect)restrictionArea {
  ...
  if (CGRectEqualToRect(restrictionArea, CGRectZero)) {
    [super setContentOffset:CGPointMake([super contentOffset].x -
                                            self.containerView.frame.origin.x,
                                        [super contentOffset].y -
                                            self.containerView.frame.origin.y)];
    [self.containerView
        setFrame:CGRectMake(0.f, 0.f, self.containerView.frame.size.width,
                            self.containerView.frame.size.height)];
    [super setContentSize:CGSizeMake(self.containerView.frame.size.width,
                                     self.containerView.frame.size.height)];
  } else {
    CGPoint currentOffset = [super contentOffset];

    [self.containerView
        setFrame:CGRectMake(-restrictionArea.origin.x,
                            -restrictionArea.origin.y,
                            self.containerView.frame.size.width,
                            self.containerView.frame.size.height)];
    [super setContentOffset:CGPointMake(
                                currentOffset.x - restrictionArea.origin.x,
                                currentOffset.y - restrictionArea.origin.y)];
    [super setContentSize:CGSizeMake(restrictionArea.size.width,
                                     restrictionArea.size.height)];
  }
}
