...
- (UIView *)viewForStateError;
- (UIView *)viewForStateNoMoreData;
...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler {
    if (!self.infiniteScrollingView) {
        ...
        [view setCustomView:[view viewForStateError] forState:SVInfiniteScrollingStateError];
        [view setCustomView:[view viewForStateLoading]
...
- (void)setScrollViewContentInsetForInfiniteScrolling {
    ...
    self.originalBottomInset + SVInfiniteScrollingViewHeight;
    [self setScrollViewContentInset:currentInsets];
}
...
- (UIView *)viewForStateError {
    ...
    button.size = CGSizeMake(customView.width - 20, customView.height - 20);
    button.origin = CGPointMake(10, 10);
    [customView addSubview:button];
    [button addTarget:self.scrollView action:@selector(triggerInfiniteScrolling) forControlEvents:UIControlEventTouchUpInside];
    customView.backgroundColor = UIColorHex(f5f5f5);
    return customView;
}

- (UIView *)viewForStateLoading {
    UIView *customView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, SVInfiniteScrollingViewHeight)];
    //小菊花
    UIActivityIndicatorView *activityIndicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    [activityIndicator startAnimating];
    [activityIndicator setCenter:CGPointMake(kScreenWidth/2-60, CGRectGetHeight(customView.frame)/2)];
    
    //提示文字
    UILabel *loadingLabel = [[UILabel alloc] initWithFrame:CGRectMake(kScreenWidth/2-30, (CGRectGetHeight(customView.frame)-25)/2, 100, 25)];
...
- (UIView *)viewForStateNoMoreData {
    ...
    label.size = CGSizeMake(customView.width - 20, customView.height - 20);
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:@"contentOffset"])
        ...
        CGRectMake(0, self.scrollView.contentSize.height,
                   self.bounds.size.width, SVInfiniteScrollingViewHeight);
    }
}
    ...
    if (self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        if (!self.scrollView.isDragging &&
            ...
            self.state = SVInfiniteScrollingStateTriggered;
                 ...
                 (self.state != SVInfiniteScrollingStateStopped || self.state != SVInfiniteScrollingStateError || self.state != SVInfiniteScrollingStateNoMoreData))
            self.state = SVInfiniteScrollingStateStopped;
    }
}
...
- (void)setCustomView:(UIView *)view forState:(SVInfiniteScrollingState)state {
    ...
    if (state == SVInfiniteScrollingStateAll)
        [self.viewForState
         replaceObjectsInRange:NSMakeRange(0, 5)
         withObjectsFromArray:
         @[ viewPlaceholder, viewPlaceholder, viewPlaceholder, viewPlaceholder , viewPlaceholder]];
    else
...
- (void)showErrorView {
    self.state = SVInfiniteScrollingStateError;
}

- (void)showNoMoreView {
    self.state = SVInfiniteScrollingStateNoMoreData;
}
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if (hasCustomView) {
        customView.size = self.size;
        customView.origin = CGPointMake(0, 0);
        ...
        switch (newState) {
            ...
            case SVInfiniteScrollingStateNoMoreData:
            case SVInfiniteScrollingStateError:
