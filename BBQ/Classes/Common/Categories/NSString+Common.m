...
//    [[[NSBundle mainBundle] infoDictionary] objectForKey:(__bridge NSString
//    *)kCFBundleExecutableKey] ?: [[[NSBundle mainBundle] infoDictionary]
//    objectForKey:(__bridge NSString *)kCFBundleIdentifierKey], (__bridge
//    id)CFBundleGetValueForInfoDictionaryKey(CFBundleGetMainBundle(),
//    kCFBundleVersionKey) ?: [[[NSBundle mainBundle] infoDictionary]
//    objectForKey:(__bridge NSString *)kCFBundleVersionKey], deviceString,
//    [[UIDevice currentDevice] systemVersion], ([[UIScreen mainScreen]
...
+ (NSString *)ageWithYear:(NSNumber *)birthyear
    ...
    NSInteger month = 0;
    month = [[NSDate date]
             monthsFrom:[NSDate dateWithYear:birthyear.integerValue month:birthmonth.integerValue day:birthday.integerValue]];
    year = month / 12;
    month %= 12;
    if (year >= 10 || (year == 0 && month == 0)) return @"";
    if (year * month) return [NSString stringWithFormat:@"%@岁%@个月", @(year), @(month)];
    if (month != 0) return [NSString stringWithFormat:@"%@个月", @(month)];
    if (year != 0) return [NSString stringWithFormat:@"%@岁", @(year)];
    return @"";
}

+ (NSString *)relationshipWithID:(NSNumber *)gxid gxname:(NSString *)gxname {
    NSString *relationship = @"";
    switch (gxid.integerValue) {
        case 1: {
            relationship = @"爸爸";
        } break;
        case 2: {
...
//            urlStr = [NSString stringWithFormat:@"%@%@", [NSObject
///baseURLStr], self];
//        }else{
//            urlStr = self;
//        }
...
//            stringByMatching:@"/static/fruit_avatar/([a-zA-Z0-9\\-._]+)$"
//            capture:1];
//            if (imageName && imageName.length > 0) {
//                urlStr = [NSString
//                stringWithFormat:@"http://coding-net-avatar.qiniudn.com/%@?imageMogr2/auto-orient/thumbnail/!%.0fx%.0fr",
...
//                        urlStr = [urlStr stringByAppendingString:[NSString
//                        stringWithFormat:@"/thumbnail/!%.0fx%.0fr/format/png",
//                        width, width]];
//                    }else{
//                        urlStr = [urlStr stringByAppendingString:[NSString
//                        stringWithFormat:@"?imageMogr2/auto-orient/thumbnail/!%.0fx%.0fr/format/png",
//                        width, width]];
//                    }
//                }else{
//                    if ([urlStr rangeOfString:@"?"].location != NSNotFound) {
//                        urlStr = [urlStr stringByAppendingString:[NSString
//                        stringWithFormat:@"/thumbnail/!%.0fx%.0fr", width,
//                        width]];
//                    }else{
//                        urlStr = [urlStr stringByAppendingString:[NSString
//                        stringWithFormat:@"?imageMogr2/auto-orient/thumbnail/!%.0fx%.0fr",
//                        width, width]];
//                    }
//                }
//                canCrop = YES;
//            }else if ([urlStr rangeOfString:@"www.gravatar.com"].location !=
//            NSNotFound){
//                urlStr = [urlStr
//                stringByReplacingOccurrencesOfString:@"s=[0-9]*"
//                withString:[NSString stringWithFormat:@"s=%.0f", width]
//                options:NSRegularExpressionSearch range:NSMakeRange(0, [urlStr
//                length])];
//            }else if ([urlStr hasSuffix:@"/imagePreview"]){
//                urlStr = [urlStr stringByAppendingFormat:@"?width=%.0f",
//                width];
//            }
//        }
//        if (needCrop && canCrop) {
//            urlStr = [urlStr
//            stringByAppendingFormat:@"/gravity/Center/crop/%.0fx%.0f", width,
//            width];
//        }
//        return [NSURL URLWithString:urlStr];
//    }
...
///showType:MediaShowTypeImageAndMonkey];
...
//     ^(NSString *substring, NSRange substringRange, NSRange enclosingRange,
//     BOOL *stop) {
//
...
//                 const int uc = ((hs - 0xd800) * 0x400) + (ls - 0xdc00) +
//                 0x10000;
...
//             0x3030 || hs == 0x2b55 || hs == 0x2b1c || hs == 0x2b1b || hs ==
//             0x2b50) {
//                 returnValue = YES;
//             }
//         }
...
//    if (sizeOfByte < 1024) {
//        sizeDisplayStr = [NSString stringWithFormat:@"%.2f bytes",
//        sizeOfByte];
//    }else{
...
//            if (sizeOfM < 1024) {
//                sizeDisplayStr = [NSString stringWithFormat:@"%.2f M",
//                sizeOfM];
//            }else{
//                CGFloat sizeOfG = sizeOfKB/1024;
//                sizeDisplayStr = [NSString stringWithFormat:@"%.2f G",
//                sizeOfG];
//            }
//        }
//    }
...
//}
//
//- (NSString *)stringByTrimmingLeftCharactersInSet:(NSCharacterSet
//*)characterSet {
//    return [self substringWithRange:[self
//    rangeByTrimmingLeftCharactersInSet:characterSet]];
//}
//
//- (NSString *)stringByTrimmingRightCharactersInSet:(NSCharacterSet
//*)characterSet {
//    return [self substringWithRange:[self
//    rangeByTrimmingRightCharactersInSet:characterSet]];
//}
//
////转换拼音
...
//    CFStringTransform((CFMutableStringRef)tempString, NULL,
//    kCFStringTransformToLatin, false);
//    tempString = (NSMutableString *)[tempString
//    stringByFoldingWithOptions:NSDiacriticInsensitiveSearch locale:[NSLocale
//    currentLocale]];
