...
- (UIImage *)addImage:(UIImage *)maskImage withSize:(CGSize)size {
    UIGraphicsBeginImageContext(size);
    [self drawInRect:CGRectMake(0, 0, size.width, size.height)];
    [maskImage drawInRect:CGRectMake((size.width - maskImage.size.width)/2,
                                     (size.height - maskImage.size.height)/2,
...
- (UIImage *)scaledToSize:(CGSize)targetSize {
    ...
    [sourceImage
     drawInRect:CGRectMake(0, 0, ceilf(targetWidth), ceilf(targetHeight))];
...
+ (UIImage *)fullResolutionImageFromALAsset:(ALAsset *)asset {
    ...
    UIImage *img =
    [UIImage imageWithCGImage:imgRef
