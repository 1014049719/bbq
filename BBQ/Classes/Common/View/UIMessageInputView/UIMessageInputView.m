...
@interface UIMessageInputView () <AGEmojiKeyboardViewDelegate,
...
- (void)setFrame:(CGRect)frame {
    CGFloat oldheightToBottom = kScreenHeight - CGRectGetMinY(self.frame);
    CGFloat newheightToBottom = kScreenHeight - CGRectGetMinY(frame);
    ...
    if (fabs(oldheightToBottom - newheightToBottom) > 0.1) {
        ...
        if (_delegate &&
            [_delegate respondsToSelector:@selector(messageInputView:
                                                    heightToBottomChanged:)]) {
            [self.delegate messageInputView:self
                      heightToBottomChanged:newheightToBottom];
        }
    }
}
    ...
    if (_inputState != inputState) {
        ...
        switch (_inputState) {
            case UIMessageInputViewStateSystem: {
                [self.addButton setImage:[UIImage imageNamed:@"keyboard_add"]
                                forState:UIControlStateNormal];
                [self.emotionButton setImage:[UIImage imageNamed:@"keyboard_emotion"]
                                    forState:UIControlStateNormal];
                [self.voiceButton setImage:[UIImage imageNamed:@"keyboard_voice"]
                                  forState:UIControlStateNormal];
            } break;
            case UIMessageInputViewStateEmotion: {
                [self.addButton setImage:[UIImage imageNamed:@"keyboard_add"]
                                forState:UIControlStateNormal];
                [self.emotionButton setImage:[UIImage imageNamed:@"keyboard_keyboard"]
                                    forState:UIControlStateNormal];
                [self.voiceButton setImage:[UIImage imageNamed:@"keyboard_voice"]
                                  forState:UIControlStateNormal];
            } break;
            case UIMessageInputViewStateAdd: {
                [self.addButton setImage:[UIImage imageNamed:@"keyboard_keyboard"]
                                forState:UIControlStateNormal];
                [self.emotionButton setImage:[UIImage imageNamed:@"keyboard_emotion"]
                                    forState:UIControlStateNormal];
                [self.voiceButton setImage:[UIImage imageNamed:@"keyboard_voice"]
            ...
            case UIMessageInputViewStateVoice: {
                ...
                [self.emotionButton setImage:[UIImage imageNamed:@"keyboard_emotion"]
                                    forState:UIControlStateNormal];
                [self.voiceButton setImage:[UIImage imageNamed:@"keyboard_keyboard"]
                                  forState:UIControlStateNormal];
            } break;
            default:
                break;
        ...
        _arrowKeyboardButton.center =
        CGPointMake(self.frame.size.width / 2, self.frame.size.height / 2);
    }
}
- (void)setPlaceHolder:(NSString *)placeHolder {
    if (_inputTextView &&
        ![_inputTextView.placeholder isEqualToString:placeHolder]) {
    ...
    if (_commentGuid) {
        switch (_contentType) {
            case UIMessageInputViewContentTypeTweet:
                inputKey = [NSString
                            stringWithFormat:@"tweet_%@_%@", _commentGuid,
                            _toUser.member.uid.stringValue.length > 0 ? _toUser.member.uid : @""];
                break;
            case UIMessageInputViewContentTypeTopic:
                inputKey = [NSString
                            stringWithFormat:@"topic_%@_%@", _commentGuid,
                            _toUser.member.uid.stringValue.length > 0 ? _toUser.member.uid : @""];
                break;
            case UIMessageInputViewContentTypeTask:
                inputKey = [NSString
                            stringWithFormat:@"task_%@_%@", _commentGuid,
                            _toUser.member.uid.stringValue.length > 0 ? _toUser.member.uid : @""];
                break;
            default:
                break;
...
- (void)setToUser:(User *)toUser {
    ...
    if (_inputTextView) {
        if (_contentType != UIMessageInputViewContentTypePriMsg) {
            self.placeHolder =
            _toUser ? [NSString stringWithFormat:@"回复 %@", _toUser.member.nickname]
            : @"添加评论";
        } else {
...
+ (instancetype)messageInputViewWithType:(UIMessageInputViewContentType)type
    ...
    UIMessageInputView *messageInputView = [[UIMessageInputView alloc]
                                            initWithFrame:CGRectMake(0, kScreenHeight, kScreenWidth,
                                                                     kMessageInputView_Height)];
...
- (void)customUIWithType:(UIMessageInputViewContentType)type {
    ...
    kMessageInputView_Height - 2 * kMessageInputView_PadingHeight;
    ...
    if (!_contentView) {
        ...
        [_contentView mas_makeConstraints:^(MASConstraintMaker *make) {
            CGFloat left = hasVoiceBtn ? (7 + kMessageInputView_Width_Tool + 7)
            : kPaddingLeftWidth;
            make.edges.equalTo(self).insets(UIEdgeInsetsMake(
                                                             kMessageInputView_PadingHeight, left, kMessageInputView_PadingHeight,
                                                             kPaddingLeftWidth + toolBtnNum * kMessageInputView_Width_Tool));
        }];
    }
    
    if (!_inputTextView) {
        _inputTextView = [[UIPlaceHolderTextView alloc]
                          initWithFrame:CGRectMake(0, 0,
                                                   kScreenWidth - kPaddingLeftWidth -
                                                   toolBtnNum * kMessageInputView_Width_Tool -
                                                   (hasVoiceBtn
                                                    ? 7 + kMessageInputView_Width_Tool + 7
                                                    : kPaddingLeftWidth),
                                                   contentViewHeight)];
    ...
    if (hasEmotionBtn && !_emotionButton) {
        _emotionButton = [[UIButton alloc]
                          initWithFrame:CGRectMake(kScreenWidth - kPaddingLeftWidth / 2 -
                                                   toolBtnNum * kMessageInputView_Width_Tool,
                                                   (kMessageInputView_Height -
                                                    kMessageInputView_Width_Tool) /
                                                   2,
                                                   kMessageInputView_Width_Tool,
                                                   kMessageInputView_Width_Tool)];
        [_emotionButton setImage:[UIImage imageNamed:@"keyboard_emotion"]
                        forState:UIControlStateNormal];
    ...
    //        initWithFrame:CGRectMake(kScreenWidth - kPaddingLeftWidth/2
    //        -kMessageInputView_Width_Tool, (kMessageInputView_Height -
    //        kMessageInputView_Width_Tool)/2, kMessageInputView_Width_Tool,
    //        kMessageInputView_Width_Tool)];
    //
    //        [_addButton setImage:[UIImage imageNamed:@"keyboard_add"]
    //        forState:UIControlStateNormal];
    //        [_addButton addTarget:self action:@selector(addButtonClicked:)
    //        forControlEvents:UIControlEventTouchUpInside];
    ...
    //        _photoButton = [[UIButton alloc]
    //        initWithFrame:CGRectMake(kScreenWidth - kPaddingLeftWidth/2
    //        -kMessageInputView_Width_Tool, (kMessageInputView_Height -
    //        kMessageInputView_Width_Tool)/2, kMessageInputView_Width_Tool,
    //        kMessageInputView_Width_Tool)];
    //
    //        [_photoButton setImage:[UIImage imageNamed:@"keyboard_photo"]
    //        forState:UIControlStateNormal];
    //        [_photoButton addTarget:self action:@selector(photoButtonClicked:)
    //        forControlEvents:UIControlEventTouchUpInside];
    ...
    if (hasEmotionBtn && !_emojiKeyboardView) {
        _emojiKeyboardView = [[AGEmojiKeyboardView alloc]
                              initWithFrame:CGRectMake(0, 0, kScreenWidth, kKeyboardView_Height)
                              dataSource:self
                              showBigEmotion:showBigEmotion];
        _emojiKeyboardView.delegate = self;
        [_emojiKeyboardView setY:kScreenHeight];
    }
    
    //    if (hasAddBtn && !_addKeyboardView) {
    //        _addKeyboardView = [[UIMessageInputView_Add alloc]
    //        initWithFrame:CGRectMake(0, kScreenHeight, kScreenWidth,
    //        kKeyboardView_Height)];
    //        _addKeyboardView.addIndexBlock = ^(NSInteger index){
    //            if ([weakSelf.delegate
    //            respondsToSelector:@selector(messageInputView:addIndexClicked:)])
    //            {
    //                [weakSelf.delegate messageInputView:weakSelf
    //                addIndexClicked:index];
    //            }
    //        };
    //    }
    //
    ...
    //        alloc] init];
    //        _mediaView = [[UICustomCollectionView alloc]
    //        initWithFrame:CGRectMake(0, CGRectGetHeight(_inputTextView.frame),
    //        CGRectGetWidth(_inputTextView.frame), 1)
    //        collectionViewLayout:layout];
    //        _mediaView.scrollEnabled = NO;
    //        [_mediaView setBackgroundView:nil];
    //        [_mediaView setBackgroundColor:[UIColor clearColor]];
    //        [_mediaView registerClass:[UIMessageInputView_CCell class]
    //        forCellWithReuseIdentifier:kCCellIdentifier_UIMessageInputView_CCell];
    ...
    if (_inputTextView) {
        [[RACObserve(self.inputTextView, contentSize)
          takeUntil:self.rac_willDeallocSignal]
         subscribeNext:^(NSValue *contentSize) {
    ...
    //        //监听-上传文件成功
    //        [[[[NSNotificationCenter defaultCenter]
    //        rac_addObserverForName:kNotificationUploadCompled object:nil]
    //        takeUntil:self.rac_willDeallocSignal] subscribeNext:^(NSNotification
    //        *aNotification) {
    //            //{NSURLResponse: response, NSError: error, ProjectFile: data}
    //            NSDictionary* userInfo = [aNotification userInfo];
    //            [self completionUploadWithResult:[userInfo objectForKey:@"data"]
    //            error:[userInfo objectForKey:@"error"]];
    //        }];
    //    }
    
    //    if (hasVoiceBtn && !_voiceButton) {
    //        _voiceButton = [[UIButton alloc] initWithFrame:CGRectMake(7,
    //        (kMessageInputView_Height - kMessageInputView_Width_Tool)/2,
    //        kMessageInputView_Width_Tool, kMessageInputView_Width_Tool)];
    //
    //        [_voiceButton setImage:[UIImage imageNamed:@"keyboard_voice"]
    //        forState:UIControlStateNormal];
    //        [_voiceButton addTarget:self action:@selector(voiceButtonClicked:)
    //        forControlEvents:UIControlEventTouchUpInside];
    //        [self addSubview:_voiceButton];
    //
    //        if ([[FunctionTipsManager shareManager]
    //        needToTip:kFunctionTipStr_VoiceMessage]) {
    //            _voiceRedpointView = [[UIView alloc] initWithFrame:CGRectMake(0,
    //            0, 7, 7)];
    //            _voiceRedpointView.center = CGPointMake(31, 5);
    //            _voiceRedpointView.backgroundColor = [UIColor
    //            colorWithRGBHex:0xf75288];
    //            _voiceRedpointView.layer.cornerRadius =
    //            _voiceRedpointView.frame.size.width/2;
    ...
    if (hasVoiceBtn && !_arrowKeyboardButton) {
        ...
        _arrowKeyboardButton.frame = CGRectMake(0, 0, kMessageInputView_Width_Tool,
                                                kMessageInputView_Width_Tool);
        [_arrowKeyboardButton setImage:[UIImage imageNamed:@"keyboard_arrow_down"]
                              forState:UIControlStateNormal];
        [_arrowKeyboardButton addTarget:self
    ...
    //        _voiceKeyboardView = [[UIMessageInputView_Voice alloc]
    //        initWithFrame:CGRectMake(0, kScreenHeight, kScreenWidth,
    //        kKeyboardView_Height)];
    //        _voiceKeyboardView.recordSuccessfully = ^(NSString *file,
    //        NSTimeInterval duration){
    //            if (weakSelf.delegate && [weakSelf.delegate
    //            respondsToSelector:@selector(messageInputView:sendVoice:duration:)])
    //            {
    //                [weakSelf.delegate messageInputView:weakSelf sendVoice:file
    //                duration:duration];
    //            }
    //        };
    //    }
...
- (void)updateContentViewBecauseOfMedia:(BOOL)becauseOfMedia {
    ...
    if (_mediaView) {
        ...
        if (_mediaList.count <= 0) {
            ...
            collectionItemHeight = (CGRectGetWidth(_inputTextView.frame) -
                                    kMessageInputView_MediaPadding) /
            3;
        } else {
            collectionItemHeight = (CGRectGetWidth(_inputTextView.frame) -
                                    2 * kMessageInputView_MediaPadding) /
            3;
        }
        CGFloat mediaHeight =
        ceilf(_mediaList.count / 3.0) *
        (collectionItemHeight + kMessageInputView_MediaPadding) -
        kMessageInputView_MediaPadding;
        mediaSize = CGSizeMake(CGRectGetWidth(_mediaView.frame), mediaHeight);
        
        CGRect mediaFrame = CGRectMake(0, CGRectGetMaxY(_inputTextView.frame),
    ...
    CGSize contentSize =
    CGSizeMake(textSize.width, textSize.height + mediaSize.height);
    CGFloat selfHeight =
    MAX(kMessageInputView_Height,
        contentSize.height + 2 * kMessageInputView_PadingHeight);
    
    CGFloat maxSelfHeight = kScreenHeight / 2;
    if (kDevice_Is_iPhone5) {
        maxSelfHeight = 230;
    } else if (kDevice_Is_iPhone6) {
        maxSelfHeight = 290;
    } else if (kDevice_Is_iPhone6Plus) {
        maxSelfHeight = kScreenHeight / 2;
    } else {
        maxSelfHeight = 140;
    }
    ...
    CGFloat offsetY = MAX(0, bottomY - (CGRectGetHeight(self.frame) -
                                        2 * kMessageInputView_PadingHeight));
...
//    UIMessageInputView_CCell *ccell = [collectionView
//    dequeueReusableCellWithReuseIdentifier:kCCellIdentifier_UIMessageInputView_CCell
//    forIndexPath:indexPath];
//    [ccell setCurMedia:[_mediaList objectAtIndex:indexPath.row]
//    andTotalCount:_mediaList.count];
//    @weakify(self);
...
//    }else if (_mediaList.count == 2){
//        ccellSize = CGSizeMake((contentWidth -
//        kMessageInputView_MediaPadding)/2, (contentWidth -
//        kMessageInputView_MediaPadding)/3);
//    }else{
//        ccellSize = CGSizeMake((contentWidth - 2*
//        kMessageInputView_MediaPadding)/3, (contentWidth - 2*
//        kMessageInputView_MediaPadding)/3);
//    }
//    return ccellSize;
//}
...
//        dispatch_queue_create("MJPhotoBrowserForAsset",
//        DISPATCH_QUEUE_SERIAL);
//        dispatch_async(queue, ^{
//            [assetsLibrary assetForURL:mediaItem.assetURL
...
//                photo.url = [NSURL URLWithString:mediaItem.urlStr]; //
//                图片路径
//                dispatch_semaphore_signal(semaphore);
//            }];
//        });
...
//    alloc] init];
//    imagePickerController.filterType =
...
//    alloc] initWithRootViewController:imagePickerController];
//    [[BaseViewController presentingVC]
...
//        endY = kScreenHeight - kKeyboardView_Height;
//    }
//    [UIView animateWithDuration:0.25 delay:0.0f
...
//
//        [[FunctionTipsManager shareManager]
//        markTiped:kFunctionTipStr_VoiceMessage];
//    }
//}
...
//- (void)qb_imagePickerController:(QBImagePickerController
...
//        [_uploadMediaList addObject:[UIMessageInputView_Media
//        mediaWithAsset:asset urlStr:nil]];
//    }
//    [self doUploadMediaList];
//}
//- (void)qb_imagePickerControllerDidCancel:(QBImagePickerController
//*)imagePickerController{
//    [[BaseViewController presentingVC] dismissViewControllerAnimated:YES
//    completion:nil];
//}
...
//    [_uploadMediaList enumerateObjectsUsingBlock:^(UIMessageInputView_Media
...
//    //保存到app内
//    NSString* originalFileName = [[media.curAsset defaultRepresentation]
//    filename];
//    NSString *fileName = [NSString stringWithFormat:@"%@|||%@|||%@",
//    self.curProject.id.stringValue, @"0", originalFileName];
//
//    if ([Coding_FileManager writeUploadDataWithName:fileName
//    andAsset:media.curAsset]) {
//        [self hudTipWillShow:[NSString stringWithFormat:@"正在上传第 %ld
//        张图片...", (long)index +1]];
...
//        addUploadTaskWithFileName:fileName
//        projectIsPublic:_curProject.is_public.boolValue];
//        @weakify(self)
//        [RACObserve(uploadTask, progress.fractionCompleted)
//        subscribeNext:^(NSNumber *fractionCompleted) {
//            @strongify(self);
//            dispatch_async(dispatch_get_main_queue(), ^{
//                if (self.HUD) {
//                    self.HUD.progress = MAX(0,
//                    fractionCompleted.floatValue-0.05);
//                    DebugLog(@"uploadingPhotoName - %@ : %.2f",
//                    self.uploadingPhotoName, fractionCompleted.floatValue);
//                }
//            });
//        }];
//    }else{
//        media.state = UIMessageInputView_MediaStateUploadFailed;
//        [NSObject showHudTipStr:[NSString stringWithFormat:@"%@ 文件处理失败",
//        originalFileName]];
//    }
//}
...
//    //移除文件（共有项目不能自动移除）
//    NSString *diskFileName = [NSString stringWithFormat:@"%@|||%@|||%@",
//    self.curProject.id.stringValue, @"0", self.uploadingPhotoName];
...
//            isEqualToString:self.uploadingPhotoName]) {
...
- (void)sendTextStr {
    ...
    [NSMutableString stringWithString:self.inputTextView.text];
    //    if (_mediaList.count > 0) {
    //        [_mediaList enumerateObjectsUsingBlock:^(UIMessageInputView_Media
    //        *obj, NSUInteger idx, BOOL *stop) {
    //            [sendStr appendFormat:@"\n![图片](%@)", obj.urlStr];
    //        }];
    //    }
    if (sendStr && [sendStr isNotBlank] && _delegate &&
        [_delegate respondsToSelector:@selector(messageInputView:sendText:)]) {
...
shouldChangeTextInRange:(NSRange)range
 replacementText:(NSString *)text {
    ...
    //        if (self.curProject) {
    //            //@项目成员
    //            [ProjectMemberListViewController showATSomeoneWithBlock:^(User
    //            *curUser) {
    //                [weakSelf atSomeUser:curUser inTextView:textView
    //                andRange:range];
...
- (void)keyboardChange:(NSNotification *)aNotification {
    ...
    if (self.inputState == UIMessageInputViewStateSystem &&
        ...
        CGFloat selfOriginY =
        keyboardY == kScreenHeight
        ? self.isAlwaysShow ? kScreenHeight - CGRectGetHeight(self.frame)
        : kScreenHeight
        : keyboardY - CGRectGetHeight(self.frame);
        //        if (keyboardY == kScreenHeight) {
        //            if (self.isAlwaysShow) {
        //                selfOriginY = kScreenHeight- CGRectGetHeight(self.frame);
        //            }else{
        //                selfOriginY = kScreenHeight;
        //            }
        //        }else{
...
- (void)emojiKeyBoardView:(AGEmojiKeyboardView *)emojiKeyBoardView
    ...
    if (emotion_monkey) {
        ...
        if (_delegate &&
            [_delegate
             respondsToSelector:@selector(messageInputView:sendBigEmotion:)]) {
