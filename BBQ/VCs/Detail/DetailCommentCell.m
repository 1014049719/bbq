...
- (void)setModel:(Comment *)model {
    ...
    self.replyTimeLabel.text = [NSDate
                                timeAgoSinceDate:[NSDate
                                                  dateWithTimeIntervalSince1970:model.dateline
                                                  .integerValue]];
...
- (void)loadImages {
    [self.replierHeadView setImageWithURL:[NSURL URLWithString:self.model.fbztx]
...
- (void)didTapOnUserHeadView:(UITapGestureRecognizer *)recognizer {
    if ([self.delegate respondsToSelector:@selector(didClickUserWithID:)]) {
