...
#import "ChoosePhotoAlbumTableViewController.h"
...
@interface ChoosePhotoAlbumTableViewController ()
...
@implementation ChoosePhotoAlbumTableViewController
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  //    self.navigationItem.title = NSLocalizedStringFromTableInBundle(@"title",
  //    @"QBImagePicker", self.imagePickerController.assetBundle, nil);
  //
  //
...
//{
//    QBAssetsViewController *assetsViewController =
//    segue.destinationViewController;
//    assetsViewController.imagePickerController = self.imagePickerController;
//    assetsViewController.assetsGroup =
//    self.assetsGroups[self.tableView.indexPathForSelectedRow.row];
//}
...
//{
//    if ([self.imagePickerController.delegate
//    respondsToSelector:@selector(qb_imagePickerController:didSelectAssets:)])
//    {
//        [self fetchAssetsFromSelectedAssetURLsWithCompletion:^(NSArray
//        *assets) {
//            [self.imagePickerController.delegate
...
//    initWithTitle:@"" style:UIBarButtonItemStylePlain target:nil action:NULL];
//    infoButtonItem.enabled = NO;
//    [infoButtonItem setTitleTextAttributes:attributes
...
//            format = NSLocalizedStringFromTableInBundle(@"items_selected",
//            @"QBImagePicker", bundle, nil);
//        } else {
//            format = NSLocalizedStringFromTableInBundle(@"item_selected",
//            @"QBImagePicker", bundle, nil);
//        }
//
//        NSString *title = [NSString stringWithFormat:format,
//        selectedAssetURLs.count];
...
- (void)updateAssetsGroupsWithCompletion:(void (^)(void))completion {
  [self
      fetchAssetsGroupsWithTypes:self.uploadVC.groupTypes
                      completion:^(NSArray *assetsGroups) {
                        ...
                        NSMutableArray *sortedAssetsGroups = [NSMutableArray
                            arrayWithCapacity:self.uploadVC.groupTypes.count];

                        for (NSValue *groupType in self.uploadVC.groupTypes) {
...
- (void)fetchAssetsGroupsWithTypes:(NSArray *)types
  ...
  ALAssetsLibrary *assetsLibrary = TheGlobal.assetsLibrary;
  ;
  ALAssetsFilter *assetsFilter;

  switch (self.uploadVC.itemType) {
...
- (void)fetchAssetsFromSelectedAssetURLsWithCompletion:
  ...
  ALAssetsLibrary *assetsLibrary = TheGlobal.assetsLibrary;
  NSMutableOrderedSet *selectedAssetURLs = self.uploadVC.selectedAssetURLs;
...
//{
//    NSUInteger minimumNumberOfSelection = MAX(1,
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  BBQAlbumCell *cell =
      [tableView dequeueReusableCellWithIdentifier:@"BBQAlbumCell"
...
- (void)tableView:(UITableView *)tableView
  ...
  if (self.block) {
    self.block(self.assetsGroups[indexPath.row]);
  }
