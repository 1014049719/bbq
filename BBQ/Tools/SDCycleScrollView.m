...
- (void)loadImageAtIndex:(NSInteger)index
{
    ...
    UIImage *image = [[YYImageCache sharedCache] getImageForKey:urlStr withType:YYImageCacheTypeAll ];
    if (image) {
        [self.imagesGroup setObject:image atIndexedSubscript:index];
    } else {
        [[YYWebImageManager sharedManager] requestImageWithURL:url options:YYWebImageOptionSetImageWithFadeAnimation progress:nil transform:nil completion:^(UIImage *image, NSURL *url, YYWebImageFromType from, YYWebImageStage stage, NSError *error) {
            if (image) {
