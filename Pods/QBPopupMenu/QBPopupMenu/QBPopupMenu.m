...
static const NSTimeInterval kQBPopupMenuAnimationDuration = 0.4;
...
- (instancetype)initWithItems:(NSArray *)items {
    ...
    if (self) {
        ...
        [[UIColor darkGrayColor] colorWithAlphaComponent:0.8];
    }
...
- (void)showInView:(UIView *)view
    ...
    if (arrowDirection == QBPopupMenuArrowDirectionDefault) {
        if ((targetRect.origin.y - (self.height + self.arrowSize)) >=
            self.popupMenuInsets.top) {
            arrowDirection = QBPopupMenuArrowDirectionDown;
        } else if ((targetRect.origin.y + targetRect.size.height +
                    (self.height + self.arrowSize)) <
                   (view.bounds.size.height - self.popupMenuInsets.bottom)) {
            arrowDirection = QBPopupMenuArrowDirectionUp;
        } else {
            CGFloat left = targetRect.origin.x - self.popupMenuInsets.left;
            CGFloat right = view.bounds.size.width -
            (targetRect.origin.x + targetRect.size.width +
             self.popupMenuInsets.right);
            
            arrowDirection = (left > right) ? QBPopupMenuArrowDirectionLeft
    ...
    switch (arrowDirection) {
        ...
        case QBPopupMenuArrowDirectionUp:
            maximumWidth = view.bounds.size.width -
            (self.popupMenuInsets.left + self.popupMenuInsets.right);
    ...
    // Create overlay view
    self.overlayView = ({
        QBPopupMenuOverlayView *overlayView =
        [[QBPopupMenuOverlayView alloc] initWithFrame:view.bounds];
    ...
    // Delegate
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(popupMenuWillAppear:)]) {
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:kQBPopupMenuAnimationDuration
                         animations:^(void) {
                             self.alpha = 1.0;
                         }
                         completion:^(BOOL finished) {
                             ...
                             if (self.delegate &&
                                 [self.delegate
                                  respondsToSelector:@selector(popupMenuDidAppear:)]) {
        ...
        // Delegate
        if (self.delegate &&
            [self.delegate respondsToSelector:@selector(popupMenuDidAppear:)]) {
            [self.delegate popupMenuDidAppear:self];
        }
    }
}
...
- (void)dismissAnimated:(BOOL)animated {
    ...
    // Delegate
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(popupMenuWillDisappear:)]) {
    ...
    if (animated) {
        [UIView animateWithDuration:kQBPopupMenuAnimationDuration
                         ...
                         completion:^(BOOL finished) {
                             ...
                             if (self.delegate &&
                                 [self.delegate
                                  respondsToSelector:@selector(popupMenuDidDisappear:)]) {
        ...
        // Delegate
        if (self.delegate &&
            [self.delegate respondsToSelector:@selector(popupMenuDidDisappear:)]) {
            [self.delegate popupMenuDidDisappear:self];
        }
    }
}
...
- (void)createItemViews {
    ...
    for (QBPopupMenuItem *item in self.items) {
        QBPopupMenuItemView *itemView =
        [[[self class] itemViewClass] itemViewWithItem:item];
...
- (void)resetItemViewState:(QBPopupMenuItemView *)itemView {
    // NOTE: Reset properties related to the size of the button before colling
    // sizeThatFits: of item view,
...
- (void)groupItemViewsWithMaximumWidth:(CGFloat)maximumWidth {
    ...
    if (self.actualArrorDirection == QBPopupMenuArrowDirectionLeft ||
        self.actualArrorDirection == QBPopupMenuArrowDirectionRight) {
    ...
    for (QBPopupMenuItemView *itemView in self.itemViews) {
        ...
        if (itemViews.count > 0 &&
            width + itemViewSize.width + pagenatorWidth > maximumWidth) {
...
- (void)updateVisibleItemViewsWithPage:(NSUInteger)page {
    ...
    if (numberOfPages > 1 && page != 0) {
        QBPopupMenuPagenatorView *leftPagenatorView =
        [[[self class] pagenatorViewClass]
         leftPagenatorViewWithTarget:self
    ...
    if (numberOfPages > 1 && page != numberOfPages - 1) {
        QBPopupMenuPagenatorView *rightPagenatorView =
        [[[self class] pagenatorViewClass]
         rightPagenatorViewWithTarget:self
...
- (void)layoutVisibleItemViews {
    ...
    if (self.actualArrorDirection == QBPopupMenuArrowDirectionDown ||
        self.actualArrorDirection == QBPopupMenuArrowDirectionUp) {
    ...
    for (NSInteger i = 0; i < self.visibleItemViews.count; i++) {
        ...
        if (i == 0 && self.actualArrorDirection == QBPopupMenuArrowDirectionLeft) {
            itemView.button.contentEdgeInsets =
            UIEdgeInsetsMake(0, self.arrowSize, 0, 0);
        } else if (i == self.visibleItemViews.count - 1 &&
                   self.actualArrorDirection == QBPopupMenuArrowDirectionRight) {
            itemView.button.contentEdgeInsets =
            UIEdgeInsetsMake(0, 0, 0, self.arrowSize);
        } else if (self.actualArrorDirection == QBPopupMenuArrowDirectionDown) {
            itemView.button.contentEdgeInsets =
            UIEdgeInsetsMake(0, 0, self.arrowSize, 0);
        } else if (self.actualArrorDirection == QBPopupMenuArrowDirectionUp) {
            itemView.button.contentEdgeInsets =
            UIEdgeInsetsMake(self.arrowSize, 0, 0, 0);
        }
        ...
        if ((i == 0 &&
             self.actualArrorDirection == QBPopupMenuArrowDirectionLeft) ||
            (i == self.visibleItemViews.count - 1 &&
             self.actualArrorDirection == QBPopupMenuArrowDirectionRight)) {
                width += self.arrowSize;
            }
...
- (void)updatePopupMenuFrameAndArrowPosition {
    ...
    switch (self.actualArrorDirection) {
        case QBPopupMenuArrowDirectionDown: {
            popupMenuFrame = CGRectMake(
                                        self.targetRect.origin.x + (self.targetRect.size.width - width) / 2.0,
                                        self.targetRect.origin.y - (height + self.margin), width, height);
            
            if (popupMenuFrame.origin.x + popupMenuFrame.size.width >
                self.view.frame.size.width - self.popupMenuInsets.right) {
                popupMenuFrame.origin.x = self.view.frame.size.width -
                self.popupMenuInsets.right -
                popupMenuFrame.size.width;
            }
            if (popupMenuFrame.origin.x < self.popupMenuInsets.left) {
                popupMenuFrame.origin.x = self.popupMenuInsets.left;
            }
            
            CGFloat centerOfTargetRect =
            self.targetRect.origin.x + self.targetRect.size.width / 2.0;
            arrowPoint =
            CGPointMake(MAX(self.cornerRadius,
                            MIN(popupMenuFrame.size.width - self.cornerRadius,
                                centerOfTargetRect - popupMenuFrame.origin.x)),
        ...
        case QBPopupMenuArrowDirectionLeft: {
            popupMenuFrame = CGRectMake(
                                        self.targetRect.origin.x + (self.targetRect.size.width + self.margin),
                                        self.targetRect.origin.y + (self.targetRect.size.height - height) / 2.0,
                                        width, height);
            
            if (popupMenuFrame.origin.y + popupMenuFrame.size.height >
                self.view.frame.size.height - self.popupMenuInsets.bottom) {
                popupMenuFrame.origin.y = self.view.frame.size.height -
                self.popupMenuInsets.bottom -
                popupMenuFrame.size.height;
            }
            ...
            CGFloat centerOfTargetRect =
            self.targetRect.origin.y + self.targetRect.size.height / 2.0;
            arrowPoint =
            CGPointMake(0, MAX(self.cornerRadius,
                               MIN(popupMenuFrame.size.height - self.cornerRadius,
                                   centerOfTargetRect - popupMenuFrame.origin.y)));
        } break;
            
        case QBPopupMenuArrowDirectionRight: {
            popupMenuFrame = CGRectMake(
                                        self.targetRect.origin.x - (width + self.margin),
                                        self.targetRect.origin.y + (self.targetRect.size.height - height) / 2.0,
                                        width, height);
            ...
            self.targetRect.origin.y + self.targetRect.size.height / 2.0;
            arrowPoint =
            CGPointMake(popupMenuFrame.size.width,
                        MAX(self.cornerRadius,
                            MIN(popupMenuFrame.size.height - self.cornerRadius,
                                centerOfTargetRect - popupMenuFrame.origin.y)));
        } break;
    ...
    // Round coordinates
    popupMenuFrame = CGRectMake(
                                round(popupMenuFrame.origin.x), round(popupMenuFrame.origin.y),
                                round(popupMenuFrame.size.width), round(popupMenuFrame.size.height));
    arrowPoint = CGPointMake(round(arrowPoint.x), round(arrowPoint.y));
...
- (void)updatePopupMenuImage {
    ...
    for (NSInteger i = 0; i < self.visibleItemViews.count; i++) {
        ...
        UIImage *image =
        [self cropImageFromImage:popupMenuImage inRect:itemView.frame];
        UIImage *highlightedImage =
        [self cropImageFromImage:popupMenuHighlightedImage
                          inRect:itemView.frame];
...
- (UIImage *)popupMenuImageWithHighlighted:(BOOL)highlighted {
    ...
    // Draw body
    CGFloat y = (self.actualArrorDirection == QBPopupMenuArrowDirectionUp)
    ? self.arrowSize
    : 0;
    CGFloat height = self.height;
        ...
        if (i == 0) {
            if (self.visibleItemViews.count == 1) {
                ...
                if (self.actualArrorDirection == QBPopupMenuArrowDirectionLeft) {
                    ...
                    bodyRect = CGRectMake(self.arrowSize + self.cornerRadius, y,
                                          frame.size.width -
                                          (self.arrowSize + self.cornerRadius * 2.0),
                                          height);
                    tailRect = CGRectMake(frame.size.width - self.cornerRadius, y,
                                          self.cornerRadius, height);
                } else if (self.actualArrorDirection ==
                           QBPopupMenuArrowDirectionRight) {
                    headRect = CGRectMake(0, y, self.cornerRadius, height);
                    bodyRect = CGRectMake(self.cornerRadius, y,
                                          frame.size.width -
                                          (self.arrowSize + self.cornerRadius * 2.0),
                                          height);
                    tailRect = CGRectMake(frame.size.width -
                                          (self.arrowSize + self.cornerRadius),
                                          y, self.cornerRadius, height);
                } else {
                    headRect = CGRectMake(0, y, self.cornerRadius, height);
                    bodyRect =
                    CGRectMake(self.cornerRadius, y,
                               frame.size.width - self.cornerRadius * 2.0, height);
                    tailRect = CGRectMake(frame.size.width - self.cornerRadius, y,
                                          self.cornerRadius, height);
                }
                
                // Draw head
                [self drawHeadInRect:headRect
                        cornerRadius:self.cornerRadius
                         highlighted:highlighted];
                
                // Draw body
                [self drawBodyInRect:bodyRect
                           firstItem:YES
                            lastItem:YES
                         highlighted:highlighted];
                
                // Draw tail
                [self drawTailInRect:tailRect
                        cornerRadius:self.cornerRadius
                         highlighted:highlighted];
            } else {
                CGRect headRect;
                ...
                if (self.actualArrorDirection == QBPopupMenuArrowDirectionLeft) {
                    headRect = CGRectMake(self.arrowSize, y, self.cornerRadius, height);
                    bodyRect = CGRectMake(
                                          self.arrowSize + self.cornerRadius, y,
                                          frame.size.width - (self.arrowSize + self.cornerRadius), height);
                } else {
                    headRect = CGRectMake(0, y, self.cornerRadius, height);
                    bodyRect = CGRectMake(self.cornerRadius, y,
                                          frame.size.width - self.cornerRadius, height);
                }
            ...
            if (self.actualArrorDirection == QBPopupMenuArrowDirectionRight) {
                bodyRect = CGRectMake(
                                      frame.origin.x, y,
                                      frame.size.width - (self.cornerRadius + self.arrowSize), height);
                tailRect = CGRectMake(frame.origin.x + frame.size.width -
                                      (self.cornerRadius + self.arrowSize),
                                      y, self.cornerRadius, height);
            } else {
                bodyRect = CGRectMake(frame.origin.x, y,
                                      frame.size.width - self.cornerRadius, height);
                tailRect =
                CGRectMake(frame.origin.x + frame.size.width - self.cornerRadius, y,
                           self.cornerRadius, height);
            }
    ...
    // Draw arrow
    [self drawArrowAtPoint:self.arrowPoint
                 arrowSize:self.arrowSize
            arrowDirection:self.actualArrorDirection
               highlighted:highlighted];
...
- (CGMutablePathRef)arrowPathInRect:(CGRect)rect
                          direction:(QBPopupMenuArrowDirection)direction {
    // Create arrow path
    ...
    switch (direction) {
        case QBPopupMenuArrowDirectionDown: {
            ...
            CGPathAddLineToPoint(path, NULL, rect.origin.x + rect.size.width,
                                 rect.origin.y);
            CGPathAddLineToPoint(path, NULL, rect.origin.x + rect.size.width / 2.0,
        ...
        case QBPopupMenuArrowDirectionLeft: {
            CGPathMoveToPoint(path, NULL, rect.origin.x + rect.size.width,
                              rect.origin.y);
            CGPathAddLineToPoint(path, NULL, rect.origin.x + rect.size.width,
                                 rect.origin.y + rect.size.height);
...
- (CGMutablePathRef)headPathInRect:(CGRect)rect
    ...
    CGPathAddLineToPoint(path, NULL, rect.origin.x + rect.size.width,
                         rect.origin.y + rect.size.height);
    CGPathAddLineToPoint(path, NULL, rect.origin.x + cornerRadius,
                         rect.origin.y + rect.size.height);
    CGPathAddArcToPoint(path, NULL, rect.origin.x,
...
- (CGMutablePathRef)tailPathInRect:(CGRect)rect
                      cornerRadius:(CGFloat)cornerRadius {
    CGMutablePathRef path = CGPathCreateMutable();
    CGPathMoveToPoint(path, NULL, rect.origin.x, rect.origin.y);
    CGPathAddLineToPoint(path, NULL,
                         ...
                         rect.origin.y);
    CGPathAddArcToPoint(path, NULL, rect.origin.x + rect.size.width,
...
- (void)drawArrowAtPoint:(CGPoint)point
               arrowSize:(CGFloat)arrowSize
          arrowDirection:(QBPopupMenuArrowDirection)arrowDirection
             highlighted:(BOOL)highlighted {
    CGRect arrowRect = CGRectZero;
    
    switch (arrowDirection) {
        case QBPopupMenuArrowDirectionDown: {
            arrowRect = CGRectMake(point.x - arrowSize + 1.0, point.y - arrowSize,
                                   arrowSize * 2.0 - 1.0, arrowSize);
            
            arrowRect.origin.x =
            MIN(MAX(arrowRect.origin.x, self.cornerRadius),
                self.frame.size.width - self.cornerRadius - arrowRect.size.width);
        } break;
            
        case QBPopupMenuArrowDirectionUp: {
            arrowRect = CGRectMake(point.x - arrowSize + 1.0, 0, arrowSize * 2.0 - 1.0,
                                   arrowSize);
        ...
        case QBPopupMenuArrowDirectionLeft: {
            arrowRect = CGRectMake(0, point.y - arrowSize + 1.0, arrowSize,
                                   arrowSize * 2.0 - 1.0);
        } break;
            
        case QBPopupMenuArrowDirectionRight: {
            arrowRect = CGRectMake(point.x - arrowSize, point.y - arrowSize + 1.0,
                                   arrowSize, arrowSize * 2.0 - 1.0);
        } break;
    ...
    [self drawArrowInRect:arrowRect
...
- (void)drawArrowInRect:(CGRect)rect
    ...
    // Separator
    if (direction == QBPopupMenuArrowDirectionDown ||
        direction == QBPopupMenuArrowDirectionUp) {
        for (QBPopupMenuItemView *itemView in self.visibleItemViews) {
            [self drawSeparatorInRect:CGRectMake(itemView.frame.origin.x +
                                                 itemView.frame.size.width - 1,
                                                 rect.origin.y, 1, rect.size.height)];
        }
    }
}

- (void)drawHeadInRect:(CGRect)rect
          cornerRadius:(CGFloat)cornerRadius
           highlighted:(BOOL)highlighted {
    CGContextRef context = UIGraphicsGetCurrentContext();
    ...
    CGContextSaveGState(context);
    {
        ...
        [self headPathInRect:rect cornerRadius:cornerRadius];
        CGContextAddPath(context, path);
...
- (void)drawTailInRect:(CGRect)rect
          cornerRadius:(CGFloat)cornerRadius
           highlighted:(BOOL)highlighted {
    CGContextRef context = UIGraphicsGetCurrentContext();
    ...
    {
        CGMutablePathRef path =
        [self tailPathInRect:rect cornerRadius:cornerRadius];
        CGContextAddPath(context, path);
...
- (void)drawBodyInRect:(CGRect)rect
    ...
    // Separator
    if (!lastItem) {
        [self drawSeparatorInRect:CGRectMake(rect.origin.x + rect.size.width - 1,
                                             rect.origin.y, 1, rect.size.height)];
    }
}
