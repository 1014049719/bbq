...
- (instancetype)init {
  @throw [NSException exceptionWithName:@"Must initialize with a name"
                                 reason:@"PINCache must be initialized with a "
...
- (instancetype)initWithName:(NSString *)name rootPath:(NSString *)rootPath {
  ...
  if (self = [super init]) {
    ...
    NSString *queueName =
        [[NSString alloc] initWithFormat:@"%@.%p", PINCachePrefix, self];
    _concurrentQueue = dispatch_queue_create(
        [[NSString
            stringWithFormat:@"%@ Asynchronous Queue", queueName] UTF8String],
...
- (NSString *)description {
  return [[NSString alloc]
      initWithFormat:@"%@.%@.%p", PINCachePrefix, _name, self];
}
    ...
    [strongSelf->_memoryCache
        objectForKey:key
               block:^(PINMemoryCache *cache, NSString *key, id object) {
                 ...
                 if (object) {
                   [strongSelf->_diskCache
                       fileURLForKey:key
                               block:^(PINDiskCache *cache, NSString *key,
                                       id<NSCoding> object, NSURL *fileURL){
                   ...
                   [strongSelf->_diskCache
                       objectForKey:key
                              block:^(PINDiskCache *cache, NSString *key,
                                ...
                                [strongSelf->_memoryCache setObject:object
                                                             forKey:key
                                                              block:nil];
...
- (void)setObject:(id<NSCoding>)object
           forKey:(NSString *)key
            block:(PINCacheObjectBlock)block {
  if (!key || !object)
    return;
  ...
  if (block) {
    ...
    diskBlock = ^(PINDiskCache *cache, NSString *key, id<NSCoding> object,
                  NSURL *fileURL) {
      dispatch_group_leave(group);
    };
  }
...
- (NSUInteger)diskByteCount {
  ...
  [_diskCache synchronouslyLockFileAccessWhileExecutingBlock:^(PINDiskCache
                                                                   *diskCache) {
