...
@property(nonatomic, retain, setter=bk_setResponseData:)
...
- (void)bk_setResponseData:(NSMutableData *)responseData {
  objc_setAssociatedObject(self, BKResponseDataKey, responseData,
...
- (void)bk_setResponse:(NSURLResponse *)response {
  objc_setAssociatedObject(self, BKResponseKey, response,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSUInteger)bk_responseLength {
  return [objc_getAssociatedObject(self, BKResponseLengthKey)
          unsignedIntegerValue];
}

- (void)bk_setResponseLength:(NSUInteger)responseLength {
  objc_setAssociatedObject(self, BKResponseLengthKey, @(responseLength),
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
@interface A2DynamicBKURLConnectionInformalDelegate
    : A2DynamicDelegate <BKURLConnectionInformalDelegate>
...
didReceiveResponse:(NSURLResponse *)response {
  ...
  if (realDelegate &&
      [realDelegate
          respondsToSelector:@selector(connection:didReceiveResponse:)])
  ...
  void (^block)(NSURLConnection *, NSURLResponse *) =
      [self blockImplementationForMethod:_cmd];
  if (block)
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
  ...
  if (block && connection.bk_response &&
      connection.bk_response.expectedContentLength !=
          NSURLResponseUnknownLength)
    block((double)connection.bk_responseLength /
          (double)connection.bk_response.expectedContentLength);

  id realDelegate = self.realDelegate;
  if (realDelegate &&
      [realDelegate respondsToSelector:@selector(connection:didReceiveData:)]) {
...
- (void)connection:(NSURLConnection *)connection
  ...
  id realDelegate = self.realDelegate;
  if (realDelegate &&
      [realDelegate respondsToSelector:@selector(connection:
                                                     didSendBodyData:
                                                   totalBytesWritten:
                                           totalBytesExpectedToWrite:)])
    [realDelegate connection:connection
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
  ...
  void (^block)(NSURLConnection *, NSURLResponse *, NSData *) =
      connection.bk_successBlock;
  if (block)
...
- (void)connection:(NSURLConnection *)connection
  ...
  id realDelegate = self.realDelegate;
  if (realDelegate &&
      [realDelegate respondsToSelector:@selector(connection:didFailWithError:)])
    [realDelegate connection:connection didFailWithError:error];

  connection.bk_responseLength = 0;
  [connection.bk_responseData setLength:0];

  void (^block)(NSURLConnection *, NSError *) =
      [self blockImplementationForMethod:_cmd];
  if (block)
  ...
  if (realDelegate &&
      [realDelegate
          respondsToSelector:@selector(connection:
                                 canAuthenticateAgainstProtectionSpace:)])
    return [realDelegate connection:connection
...
@interface A2DynamicNSURLConnectionDelegate
    : A2DynamicDelegate <NSURLConnectionDelegate>
...
- (BOOL)conformsToProtocol:(Protocol *)protocol {
  Protocol *dataDelegateProtocol =
      objc_getProtocol("NSURLConnectionDataDelegate");
  return (protocol_isEqual(protocol, dataDelegateProtocol) ||
          protocol_isEqual(protocol, self.protocol) ||
...
- (NSURLRequest *)connection:(NSURLConnection *)connection
  ...
  id realDelegate = self.realDelegate;
  if (realDelegate && [realDelegate respondsToSelector:@selector(connection:
                                                            willSendRequest:
                                                           redirectResponse:)])
    return [realDelegate connection:connection
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection
  ...
  if (realDelegate && [realDelegate respondsToSelector:@selector(connection:
                                                           willCacheResponse:)])
    return
        [realDelegate connection:connection willCacheResponse:cachedResponse];
...
+ (void)load {
  @autoreleasepool {
    ...
    [self bk_linkDelegateMethods:@{
      @"bk_responseBlock" : @"connection:didReceiveResponse:",
      @"bk_failureBlock" : @"connection:didFailWithError:"
    }];
  }
}
...
+ (NSURLConnection *)
bk_startConnectionWithRequest:(NSURLRequest *)request
               successHandler:(void (^)(NSURLConnection *, NSURLResponse *,
                                        NSData *))success
               failureHandler:(void (^)(NSURLConnection *, NSError *))failure {
  ...
  if (failure)
    [dd implementMethod:@selector(connection:didFailWithError:)
              withBlock:[failure copy]];
...
- (id)bk_initWithRequest:(NSURLRequest *)request
       completionHandler:
           (void (^)(NSURLConnection *, NSURLResponse *, NSData *))block {
  Protocol *delegateProtocol = objc_getProtocol("NSURLConnectionDelegate");
  if (!delegateProtocol)
    delegateProtocol = @protocol(BKURLConnectionInformalDelegate);
  ...
  return (self =
              [self initWithRequest:request delegate:dd startImmediately:NO]);
}

- (void)bk_startWithCompletionBlock:(void (^)(NSURLConnection *,
                                              NSURLResponse *, NSData *))block {
...
- (void)bk_setSuccessBlock:(void (^)(NSURLConnection *, NSURLResponse *,
                                     NSData *))block {
  if (block)
