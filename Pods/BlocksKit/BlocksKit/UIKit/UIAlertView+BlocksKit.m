...
@interface A2DynamicUIAlertViewDelegate
    : A2DynamicDelegate <UIAlertViewDelegate>
...
- (void)alertViewCancel:(UIAlertView *)alertView {
  id realDelegate = self.realDelegate;
  if (realDelegate &&
      [realDelegate respondsToSelector:@selector(alertViewCancel:)])
...
- (void)willPresentAlertView:(UIAlertView *)alertView {
  id realDelegate = self.realDelegate;
  if (realDelegate &&
      [realDelegate respondsToSelector:@selector(willPresentAlertView:)])
...
- (void)didPresentAlertView:(UIAlertView *)alertView {
  id realDelegate = self.realDelegate;
  if (realDelegate &&
      [realDelegate respondsToSelector:@selector(didPresentAlertView:)])
...
- (void)alertView:(UIAlertView *)alertView
  ...
  if (realDelegate &&
      [realDelegate
          respondsToSelector:@selector(alertView:willDismissWithButtonIndex:)])
    [realDelegate alertView:alertView willDismissWithButtonIndex:buttonIndex];

  void (^block)(UIAlertView *, NSInteger) =
...
@dynamic bk_willShowBlock, bk_didShowBlock, bk_willDismissBlock,
    bk_didDismissBlock, bk_shouldEnableFirstOtherButtonBlock;
...
+ (UIAlertView *)bk_showAlertViewWithTitle:(NSString *)title
  ...
  UIAlertView *alertView =
      [[[self class] alloc] initWithTitle:title
                                  message:message
                                 delegate:self.bk_dynamicDelegate
                        cancelButtonTitle:cancelButtonTitle
                        otherButtonTitles:nil];

  // Set other buttons
  [otherButtonTitles enumerateObjectsUsingBlock:^(NSString *button,
...
- (NSInteger)bk_addButtonWithTitle:(NSString *)title
  ...
  NSAssert(title.length,
           @"A button without a title cannot be added to the alert view.");
