...
- (BOOL)saveToURL:(NSURL *)url
                    uti:(CFStringRef)uti
    backgroundFillColor:(UIColor *)fillColor {
  if (!url)
    return NO;
  ...
  CFDictionaryAddValue(
      ...
      (__bridge CFNumberRef)[NSNumber numberWithFloat:1.0f]); // No compression
  if (fillColor)
    CFDictionaryAddValue(options, kCGImageDestinationBackgroundColor,
                         fillColor.CGColor);
...
- (BOOL)saveToURL:(NSURL *)url
                   type:(NYXImageType)type
    backgroundFillColor:(UIColor *)fillColor {
  return
      [self saveToURL:url uti:[self utiForType:type]
...
- (BOOL)saveToPath:(NSString *)path {
  ...
  NSURL *url = [[NSURL alloc] initFileURLWithPath:path];
  const BOOL ret =
      [self saveToURL:url type:NYXImageTypePNG backgroundFillColor:nil];
  return ret;
}
...
+ (NSString *)extensionForUTI:(CFStringRef)uti {
  ...
  NSDictionary *declarations =
      (__bridge_transfer NSDictionary *)UTTypeCopyDeclaration(uti);
  if (!declarations)
    return nil;

  id extensions = [(NSDictionary *)[declarations
      objectForKey:(__bridge NSString *)kUTTypeTagSpecificationKey]
      objectForKey:(__bridge NSString *)kUTTagClassFilenameExtension];
  NSString *extension = ([extensions isKindOfClass:[NSArray class]])
                            ? [extensions objectAtIndex:0]
                            : extensions;
