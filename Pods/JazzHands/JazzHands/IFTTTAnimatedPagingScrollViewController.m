...
- (void)viewDidLoad {
  ...
  [self.view
      addConstraint:[NSLayoutConstraint constraintWithItem:self.scrollView
  ...
  [self.view addConstraint:[NSLayoutConstraint
                               constraintWithItem:self.contentView
                                        ...
                                        relatedBy:NSLayoutRelationEqual
                                           toItem:self.view
                                        attribute:NSLayoutAttributeWidth
                                       multiplier:(CGFloat)self.numberOfPages
                                         constant:0.f]];
  [self.view addConstraint:[NSLayoutConstraint
                               constraintWithItem:self.contentView
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  for (IFTTTScrollViewPageConstraintAnimation *animation in self
...
- (void)keepView:(UIView *)view
  ...
  NSAssert((pages.count == times.count),
           @"Make sure you set a time for each position.");
  ...
  IFTTTScrollViewPageConstraintAnimation *xPositionAnimation =
      [IFTTTScrollViewPageConstraintAnimation
          animationWithSuperview:self.contentView
                      constraint:xPositionConstraint
                       pageWidth:self.pageWidth];

  for (NSUInteger i = 0; i < pages.count; i++) {
    [xPositionAnimation
        addKeyframeForTime:(CGFloat)[(NSNumber *)times[i] floatValue]
