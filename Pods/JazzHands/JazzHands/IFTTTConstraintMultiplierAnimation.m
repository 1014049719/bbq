...
- (instancetype)initWithSuperview:(UIView *)superview
                       constraint:(NSLayoutConstraint *)constraint
                        attribute:(IFTTTLayoutAttribute)attribute
...
+ (instancetype)animationWithSuperview:(UIView *)superview
                            constraint:(NSLayoutConstraint *)constraint
                             attribute:(IFTTTLayoutAttribute)attribute
                         referenceView:(UIView *)referenceView {
  return [self animationWithSuperview:superview
                           constraint:constraint
                            attribute:attribute
                        referenceView:referenceView
                             constant:0.f];
...
- (void)animate:(CGFloat)time {
  if (!self.hasKeyframes)
    return;
  CGFloat multiplier =
      (CGFloat)[(NSNumber *)[self valueAtTime:time] floatValue];
  ...
  case IFTTTLayoutAttributeCenterX:
    referenceAttributeValue = CGRectGetMinX(self.referenceView.frame) +
                              (CGRectGetWidth(self.referenceView.frame) / 2.f);
    break;
  case IFTTTLayoutAttributeCenterY:
    referenceAttributeValue = CGRectGetMinY(self.referenceView.frame) +
                              (CGRectGetHeight(self.referenceView.frame) / 2.f);
    break;
  ...
  self.constraint.constant =
      (multiplier * referenceAttributeValue) + self.constant;
