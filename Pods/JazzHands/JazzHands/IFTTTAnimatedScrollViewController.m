...
CG_INLINE CGFloat
IFTTTMaxContentOffsetXForScrollView(UIScrollView *scrollView) {
  return scrollView.contentSize.width + scrollView.contentInset.right -
...
- (void)scrollViewDidScroll:(UIScrollView *)aScrollView {
  ...
  self.isAtEnd = (aScrollView.contentOffset.x >=
                  IFTTTMaxContentOffsetXForScrollView(aScrollView));

  id delegate = self.delegate;

  if (self.isAtEnd &&
      [delegate respondsToSelector:
                    @selector(animatedScrollViewControllerDidScrollToEnd:)]) {
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView
  ...
  if (self.isAtEnd &&
      [delegate
          respondsToSelector:
              @selector(animatedScrollViewControllerDidEndDraggingAtEnd:)]) {
