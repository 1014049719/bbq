...
- (instancetype)initWithView:(UIView *)view
                  maskEffect:(IFTTTMaskEffect)maskEffect {
  if ((self = [super init])) {
    _maskedView = view;
...
- (void)animate:(CGFloat)time {
  ...
  case IFTTTMaskEffectRevealFromBottom: {
    maskedRect.size.height *= visibilityPercent;
    maskedRect.origin.y =
        CGRectGetMaxY(self.maskedView.bounds) - CGRectGetHeight(maskedRect);
    maskPath = [UIBezierPath bezierPathWithRect:maskedRect];
    break;
  }
  case IFTTTMaskEffectRevealFromRight: {
    maskedRect.size.width *= visibilityPercent;
    maskedRect.origin.x =
        CGRectGetMaxX(self.maskedView.bounds) - CGRectGetWidth(maskedRect);
    maskPath = [UIBezierPath bezierPathWithRect:maskedRect];
    break;
  }
  case IFTTTMaskEffectRevealFromCenterToCircle: {
    CGPoint center = CGPointMake((CGRectGetWidth(maskedRect) / 2.f),
                                 (CGRectGetHeight(maskedRect) / 2.f));
