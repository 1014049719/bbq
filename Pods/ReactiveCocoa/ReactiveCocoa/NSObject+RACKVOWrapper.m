...
- (RACDisposable *)rac_observeKeyPath:(NSString *)keyPath
                             ...
                             observer:(__weak NSObject *)weakObserver
                                block:(void (^)(id, NSDictionary *, BOOL,
                                                BOOL))block {
  ...
  RACSerialDisposable *firstComponentSerialDisposable =
  ...
  objc_property_t property =
      class_getProperty(object_getClass(self), keyPathHead.UTF8String);
  if (property != NULL) {
    ...
    if (attributes != NULL) {
      ...
      BOOL isProtocol =
          attributes->objectClass == NSClassFromString(@"Protocol");
      ...
      initWithTarget:self
            observer:strongObserver
             ...
             options:trampolineOptions
               block:^(id trampolineTarget, id trampolineObserver,
                       NSDictionary *change) {
                 // If this is a prior notification, clean up all the callbacks
                 // added to the
                 // previous value and call the callback block. Everything else
                 // is deferred
                 ...
                 // If the value has changed but is nil, there is no need to add
                 // callbacks to
                 ...
                 // From here the notification is not prior and the value is not
                 // nil.

                 // Create a new firstComponentDisposable while getting rid of
                 // the old one at
                 // the same time, in case this is being called concurrently.
                 RACDisposable *oldFirstComponentDisposable =
                     [firstComponentSerialDisposable
                         swapInDisposable:[RACCompoundDisposable
                                                  compoundDisposable]];
                 [oldFirstComponentDisposable dispose];

                 addDeallocObserverToPropertyValue(value);

                 // If there are no further key path components, there is no
                 // need to add the
                 // other callbacks, just call the callback block with the value
                 // itself.
                 ...
                 // The value has changed, is not nil, and there are more key
                 // path components
                 // to consider. Add the callbacks to the value for the
                 // remaining key path
                 // components and call the callback block with the current
                 // value of the full
                 // key path.
                 addObserverToValue(value);
                 block([value valueForKeyPath:keyPathTail], change, NO,
  ...
  RACCompoundDisposable *observerDisposable =
      strongObserver.rac_deallocDisposable;
...
- (RACKVOTrampoline *)rac_addObserver:(NSObject *)observer
  ...
  return [[RACKVOTrampoline alloc] initWithTarget:self
                                         observer:observer
                                          keyPath:keyPath
                                          options:options
                                            block:block];
