...
- (RACSignal *)rac_liftSelector:(SEL)selector
          withSignalOfArguments:(RACSignal *)arguments {
  ...
  return [[[[arguments takeUntil:self.rac_willDeallocSignal]
      ...
      setNameWithFormat:@"%@ -rac_liftSelector: %s withSignalsOfArguments: %@",
                        self.rac_description, sel_getName(selector), arguments];
}

- (RACSignal *)rac_liftSelector:(SEL)selector
           withSignalsFromArray:(NSArray *)signals {
  ...
  NSUInteger numberOfArguments __attribute__((unused)) =
  ...
  return [[self rac_liftSelector:selector
           withSignalOfArguments:[RACSignal combineLatest:signals]]
      setNameWithFormat:@"%@ -rac_liftSelector: %s withSignalsFromArray: %@",
                        self.rac_description, sel_getName(selector), signals];
}

- (RACSignal *)rac_liftSelector:(SEL)selector
                    withSignals:(RACSignal *)firstSignal, ... {
  ...
  for (id currentSignal = firstSignal; currentSignal != nil;
       currentSignal = va_arg(args, id)) {
    NSCAssert([currentSignal isKindOfClass:RACSignal.class],
              @"Argument %@ is not a RACSignal", currentSignal);
  ...
  return [[self rac_liftSelector:selector withSignalsFromArray:signals]
      setNameWithFormat:@"%@ -rac_liftSelector: %s withSignals: %@",
                        self.rac_description, sel_getName(selector), signals];
}
...
- (RACSignal *)rac_liftSelector:(SEL)selector
           withObjectsFromArray:(NSArray *)args {
  return [self rac_liftSelector:selector
           withSignalsFromArray:RACMapArgumentsToSignals(args)];
}
...
- (RACSignal *)rac_liftBlock:(id)block withArguments:(id)arg, ... {
  ...
  va_list args;
  va_start(args, arg);
  for (id currentObject = arg; currentObject != nil;
...
- (RACSignal *)rac_liftBlock:(id)block withArgumentsFromArray:(NSArray *)args {
  return [[[[RACSignal combineLatest:RACMapArgumentsToSignals(args)]
      reduceEach:block] takeUntil:self.rac_willDeallocSignal] replayLast];
}
