...
rac_propertyAttributes *rac_copyPropertyAttributes(objc_property_t property) {
  ...
  if (attrString[0] != 'T') {
    fprintf(stderr, "ERROR: Expected attribute string \"%s\" for property %s "
                    "to start with 'T'\n",
  ...
  if (!next) {
    fprintf(stderr, "ERROR: Could not read past type in attribute string "
                    "\"%s\" for property %s\n",
            attrString, property_getName(property));
    return NULL;
  }
  ...
  rac_propertyAttributes *attributes =
      calloc(1, sizeof(rac_propertyAttributes) + typeLength + 1);
  if (!attributes) {
    fprintf(stderr, "ERROR: Could not allocate rac_propertyAttributes "
  ...
  if (typeString[0] == *(@encode(id)) && typeString[1] == '"') {
    ...
    if (!next) {
      fprintf(stderr, "ERROR: Could not read class name in attribute string "
                      "\"%s\" for property %s\n",
              attrString, property_getName(property));
      return NULL;
    }
  ...
  while (next && *next == ',') {
    ...
    case 'S': {
      ...
      if (!nextFlag) {
        ...
        if (!selectorLength) {
          fprintf(stderr, "ERROR: Found zero length selector name in attribute "
    ...
    case 't':
      fprintf(stderr, "ERROR: Old-style type encoding is unsupported in "
    ...
    default:
      fprintf(stderr, "ERROR: Unrecognized attribute string flag '%c' in "
                      "attribute string \"%s\" for property %s\n",
  ...
  if (next && *next != '\0') {
    fprintf(stderr, "Warning: Unparsed data \"%s\" in attribute string \"%s\" "
                    "for property %s\n",
