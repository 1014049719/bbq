...
startEagerlyWithScheduler:(RACScheduler *)scheduler
  ...
  return [signal
      setNameWithFormat:@"+startEagerlyWithScheduler: %@ block:", scheduler];
}

+ (RACSignal *)startLazilyWithScheduler:(RACScheduler *)scheduler
                                  block:(void (^)(id<RACSubscriber> subscriber))
                                            block {
  NSCParameterAssert(scheduler != nil);
  NSCParameterAssert(block != NULL);

  RACMulticastConnection *connection =
      [[RACSignal createSignal:^id(id<RACSubscriber> subscriber) {
        block(subscriber);
        return nil;
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p> name: %@", self.class, self, self.name];
}
...
- (RACSignal *)bind:(RACStreamBindBlock (^)(void))block {
   ...
   * 2. Any time the original signal sends a value, transform it using the
   *binding block.
   * 3. If the binding block returns a signal, subscribe to it, and pass all of
   *its values through to the subscriber as they're received.
   * 4. If the binding block asks the bind to terminate, complete the _original_
   *signal.
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    void (^completeSignal)(RACSignal *, RACDisposable *) =
        ^(RACSignal *signal, RACDisposable *finishedDisposable) {
    ...
    @autoreleasepool {
      ...
      RACDisposable *bindingDisposable = [self subscribeNext:^(id x) {
        ...
        if (compoundDisposable.disposed)
          return;
...
- (RACDisposable *)subscribeNext:(void (^)(id x))nextBlock {
  ...
  RACSubscriber *o =
      [RACSubscriber subscriberWithNext:nextBlock error:NULL completed:NULL];
...
- (RACDisposable *)subscribeError:(void (^)(NSError *error))errorBlock {
  ...
  RACSubscriber *o =
      [RACSubscriber subscriberWithNext:NULL error:errorBlock completed:NULL];
  return [self subscribe:o];
}
...
- (id)asynchronousFirstOrDefault:(id)defaultValue
  ...
  NSCAssert([NSThread isMainThread],
            @"%s should only be used from the main thread", __func__);
  ...
  [[[[self take:1] timeout:RACSignalAsynchronousWaitTimeout
...
+ (RACSignal *)startWithScheduler:(RACScheduler *)scheduler
                     subjectBlock:(void (^)(RACSubject *subject))block {
  NSCParameterAssert(block != NULL);

  RACReplaySubject *subject = [[RACReplaySubject subject]
      setNameWithFormat:@"+startWithScheduler:subjectBlock:"];
...
+ (RACSignal *)start:(id (^)(BOOL *success, NSError **error))block {
  return [[self startWithScheduler:[RACScheduler scheduler] block:block]
      setNameWithFormat:@"+start:"];
}

+ (RACSignal *)startWithScheduler:(RACScheduler *)scheduler
                            block:
                                (id (^)(BOOL *success, NSError **error))block {
  return [[self startWithScheduler:scheduler
                      subjectBlock:^(id<RACSubscriber> subscriber) {
                        BOOL success = YES;
                        NSError *error = nil;
