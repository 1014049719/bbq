...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p> %@", self.class, self, self.allObjects];
}
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
  ...
  return [self.backingArray countByEnumeratingWithState:state
                                                objects:buffer
                                                  count:len];
}
...
+ (instancetype)tupleWithObjectsFromArray:(NSArray *)array
  ...
  if (convert) {
    ...
    for (id object in array) {
      [newArray
          addObject:(object == NSNull.null ? RACTupleNil.tupleNil : object)];
    }
...
+ (instancetype)tupleWithObjects:(id)object, ... {
  ...
  NSUInteger count = 0;
  for (id currentObject = object; currentObject != nil;
...
- (NSArray *)allObjects {
  NSMutableArray *newArray =
      [NSMutableArray arrayWithCapacity:self.backingArray.count];
  for (id object in self.backingArray) {
    [newArray
        addObject:(object == RACTupleNil.tupleNil ? NSNull.null : object)];
  }
...
- (instancetype)tupleByAddingObject:(id)obj {
  NSArray *newArray =
      [self.backingArray arrayByAddingObject:obj ?: RACTupleNil.tupleNil];
...
- (RACSequence *)rac_sequence {
  return [RACTupleSequence sequenceWithTupleBackingArray:self.backingArray
                                                  offset:0];
}
...
- (void)setObject:(RACTuple *)tuple forKeyedSubscript:(NSArray *)variables {
  ...
  [variables enumerateObjectsUsingBlock:^(NSValue *value, NSUInteger index,
                                          BOOL *stop) {
