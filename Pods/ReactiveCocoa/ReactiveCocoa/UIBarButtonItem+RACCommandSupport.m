...
- (void)setRac_command:(RACCommand *)command {
  ...
  RACDisposable *disposable =
      objc_getAssociatedObject(self, UIControlEnabledDisposableKey);
  [disposable dispose];
  ...
  disposable =
      [command.enabled setKeyPath:@keypath(self.enabled) onObject:self];
...
- (void)rac_hijackActionAndTargetIfNeeded {
  ...
  if (self.target != nil)
    NSLog(@"WARNING: UIBarButtonItem.rac_command hijacks the control's "
          @"existing target and action.");
