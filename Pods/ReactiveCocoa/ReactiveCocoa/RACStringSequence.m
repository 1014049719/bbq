...
- (RACSequence *)tail {
  RACSequence *sequence =
      [self.class sequenceWithString:self.string offset:self.offset + 1];
...
- (NSArray *)array {
  ...
  [self.string
      enumerateSubstringsInRange:NSMakeRange(self.offset, substringLength)
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p>{ name = %@, string = %@ }", self.class, self,
