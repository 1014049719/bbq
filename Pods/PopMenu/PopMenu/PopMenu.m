...
@property (nonatomic, assign) BOOL needReturnFrame;
@property (nonatomic, assign) BOOL hasHideGesture;
@end
 ...
 */
- (void)showButtons {
    self.needReturnFrame = YES;
    self.hasHideGesture = NO;
...
- (CGRect)getFrameWithItemCount:(NSInteger)itemCount
    ...
    NSUInteger rowCount = ceil(itemCount / (float)perColumItemCount);
    //    CGFloat insetY = (CGRectGetHeight(self.bounds) - (itemHeight + paddingY) * rowCount) / 2.0;
    CGFloat insetY = kScreenHeight - 100 - (itemHeight + paddingY) * rowCount;
...
- (void)initailzerAnimationWithToPostion:(CGRect)toRect formPostion:(CGRect)fromRect atView:(UIView *)view beginTime:(CFTimeInterval)beginTime {
    ...
    if (self.returnFrameBlock != nil) {
        if (self.needReturnFrame) {
            self.returnFrameBlock(toRect,YES);
        }else if(!self.hasHideGesture ){
            self.returnFrameBlock(toRect,NO);
            self.hasHideGesture = YES;
        }
      
    }
}
- (void)returnFrame:(ReturnMenuButtomFrameBlock)block {
