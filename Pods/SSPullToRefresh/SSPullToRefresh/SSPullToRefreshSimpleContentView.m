...
- (id)initWithFrame:(CGRect)frame {
  if ((self = [super initWithFrame:frame])) {
    ...
    _statusLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 14.0f, width, 20.0f)];
...
- (void)layoutSubviews {
  ...
  self.statusLabel.frame = CGRectMake(
      20.0f, roundf((size.height - 30.0f) / 2.0f), size.width - 40.0f, 30.0f);
  self.activityIndicatorView.frame =
      CGRectMake(roundf((size.width - 20.0f) / 2.0f),
                 roundf((size.height - 20.0f) / 2.0f), 20.0f, 20.0f);
}
...
- (void)setState:(SSPullToRefreshViewState)state
  ...
  case SSPullToRefreshViewStateReady: {
    self.statusLabel.text = NSLocalizedString(@"松开即可刷新", nil);
  ...
  case SSPullToRefreshViewStateNormal: {
    self.statusLabel.text = NSLocalizedString(@"下拉刷新", nil);
  ...
  case SSPullToRefreshViewStateClosing: {
    self.statusLabel.text = NSLocalizedString(@"刷新成功", nil);
    self.statusLabel.alpha = 1.0f;
    self.activityIndicatorView.alpha = 0.0f;
    break;
  }
