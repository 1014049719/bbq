...
- (void)setState:(SSPullToRefreshViewState)state {
  ...
  if (wasLoading && _state != SSPullToRefreshViewStateLoading) {
    if ([delegate
            respondsToSelector:@selector(pullToRefreshViewDidFinishLoading:)]) {
    ...
    if ([delegate
            respondsToSelector:@selector(pullToRefreshViewDidStartLoading:)]) {
...
- (void)setScrollView:(UIScrollView *)scrollView {
  ...
  if ([_scrollView
          respondsToSelector:@selector(removeObserver:forKeyPath:context:)]) {
...
- (UIView<SSPullToRefreshContentView> *)contentView {
  ...
  if (!_contentView) {
    self.contentView =
        [[SSPullToRefreshSimpleContentView alloc] initWithFrame:CGRectZero];
  }
  return _contentView;
}
...
- (void)layoutSubviews {
  ...
  case SSPullToRefreshViewStyleStatic:
    contentFrame.origin.y = size.height + self.defaultContentInset.top +
  ...
  if ((self = [super initWithFrame:frame])) {
    for (UIView *view in self.scrollView.subviews) {
      if ([view isKindOfClass:[SSPullToRefreshView class]]) {
        [[NSException
            exceptionWithName:@"SSPullToRefreshViewAlreadyAdded"
                       reason:@"There is already a SSPullToRefreshView added "
                              @"to this scroll view. Unexpected things will "
                              @"happen. Don't do this."
                     userInfo:nil] raise];
      }
    }
...
- (void)startLoadingAndExpand:(BOOL)shouldExpand
  ...
  [self _setState:SSPullToRefreshViewStateLoading
         animated:animated
...
- (void)finishLoadingAnimated:(BOOL)animated completion:(void (^)())block {
  ...
  [self _setState:SSPullToRefreshViewStateClosing
         animated:animated
         expanded:NO
       completion:^{
...
- (void)refreshLastUpdatedAt {
  ...
  id<SSPullToRefreshViewDelegate> delegate = self.delegate;
  if ([delegate
          respondsToSelector:@selector(pullToRefreshViewLastUpdatedAt:)]) {
  ...
  // Forward to content view
  if ([self.contentView respondsToSelector:@selector(setLastUpdatedAt:
                                                withPullToRefreshView:)]) {
...
- (void)_setContentInsetTop:(CGFloat)topInset {
  ...
  // If scrollView is on top, scroll again to the top (needed for scrollViews
  // with content > scrollView).
  if (self.scrollView.contentOffset.y <= 0.0f) {
    [self.scrollView scrollRectToVisible:CGRectMake(0.0f, 0.0f, 1.0f, 1.0f)
                                animated:NO];
  }

  // Tell the delegate
  id<SSPullToRefreshViewDelegate> delegate = self.delegate;
  if ([delegate respondsToSelector:@selector(pullToRefreshView:
                                         didUpdateContentInset:)]) {
    [delegate pullToRefreshView:self
          didUpdateContentInset:self.scrollView.contentInset];
  }
}

- (void)_setState:(SSPullToRefreshViewState)state
         animated:(BOOL)animated
          ...
          completion:^(BOOL finished) {
            ...
            if ([delegate respondsToSelector:@selector(pullToRefreshView:
                                                    didTransitionToState:
                                                               fromState:
                                                                animated:)]) {
              [delegate pullToRefreshView:weakSelf
                     didTransitionToState:state
                                fromState:fromState
                                 animated:animated];
            }
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if (self.scrollView.isDragging) {
    ...
    if (self.state == SSPullToRefreshViewStateReady) {
      ...
      CGFloat insetAdjustment =
          y < 0 ? fmaxf(0, self.expandedHeight + y) : self.expandedHeight;
  ...
  id<SSPullToRefreshViewDelegate> delegate = self.delegate;
  if ([delegate
          respondsToSelector:@selector(pullToRefreshViewShouldStartLoading:)]) {
