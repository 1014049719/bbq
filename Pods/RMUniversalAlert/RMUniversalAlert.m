...
+ (instancetype)showAlertInViewController:(UIViewController *)viewController
                   ...
                   destructiveButtonTitle:(NSString *)destructiveButtonTitle
                        otherButtonTitles:(NSArray *)otherButtonTitles
                                 tapBlock:
                                     (RMUniversalAlertCompletionBlock)tapBlock {
  ...
  if ([UIAlertController class]) {
    alert.alertController = [UIAlertController
        showAlertInViewController:viewController
                        withTitle:title
                          message:message
                   ...
                   if (tapBlock) {
                     if (buttonIndex == alertView.cancelButtonIndex) {
                       ...
                       if (buttonIndex == alertView.firstOtherButtonIndex) {
                         tapBlock(alert,
                                  RMUniversalAlertDestructiveButtonIndex);
                       } else if (otherButtonTitles.count) {
                         NSInteger otherOffset =
                             buttonIndex - alertView.firstOtherButtonIndex;
                         tapBlock(alert, RMUniversalAlertFirstOtherButtonIndex +
                                             otherOffset - 1);
                       }
                     } else if (otherButtonTitles.count) {
                       NSInteger otherOffset =
...
popoverPresentationControllerBlock:
    (void (^)(RMPopoverPresentationController *popover))
        popoverPresentationControllerBlock
  ...
  if ([UIAlertController class]) {
    ...
    alert.alertController =
        [UIAlertController showActionSheetInViewController:viewController
            ...
            destructiveButtonTitle:destructiveButtonTitle
            otherButtonTitles:otherButtonTitles
            popoverPresentationControllerBlock:^(UIPopoverPresentationController
                                                     *popover) {
              if (popoverPresentationControllerBlock) {
                RMPopoverPresentationController *configuredPopover =
                    [RMPopoverPresentationController new];
            ...
            tapBlock:^(UIAlertController *controller, UIAlertAction *action,
                       NSInteger buttonIndex) {
              if (tapBlock) {
                tapBlock(alert, buttonIndex);
              }
    ...
    void (^actionSheetTapBlock)(UIActionSheet *actionSheet,
                                NSInteger buttonIndex) =
        ^(UIActionSheet *actionSheet, NSInteger buttonIndex) {
          if (tapBlock) {
            if (buttonIndex == actionSheet.cancelButtonIndex) {
              ...
              tapBlock(alert,
                       RMUniversalAlertFirstOtherButtonIndex + otherOffset);
            }
          }
        };
    ...
    if (popoverPresentationControllerBlock) {
      ...
      if (configuredPopover.barButtonItem) {
        alert.actionSheet =
            [UIActionSheet showFromBarButtonItem:configuredPopover.barButtonItem
                                        animated:YES
                                       withTitle:title
                               cancelButtonTitle:cancelButtonTitle
                          destructiveButtonTitle:destructiveButtonTitle
        ...
        alert.actionSheet =
            [UIActionSheet showFromRect:configuredPopover.sourceRect
