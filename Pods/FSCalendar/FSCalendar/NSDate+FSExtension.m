...
- (NSInteger)fs_weekOfYear {
  NSCalendar *calendar = [NSCalendar fs_sharedCalendar];
  NSDateComponents *component =
      [calendar components:NSCalendarUnitWeekOfYear fromDate:self];
...
- (NSDate *)fs_firstDayOfWeek {
  ...
  componentsToSubtract.day =
      -(weekdayComponents.weekday - calendar.firstWeekday);
...
+ (instancetype)fs_dateWithYear:(NSInteger)year
...
- (BOOL)fs_isEqualToDateForWeek:(NSDate *)date {
  return self.fs_year == date.fs_year &&
         self.fs_weekOfYear == date.fs_weekOfYear;
}

- (BOOL)fs_isEqualToDateForDay:(NSDate *)date {
  return self.fs_year == date.fs_year && self.fs_month == date.fs_month &&
         self.fs_day == date.fs_day;
}
