...
- (instancetype)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    ...
    [self.contentView.layer insertSublayer:backgroundLayer
                                     below:_titleLabel.layer];
    ...
    eventLayer.path =
        [UIBezierPath bezierPathWithOvalInRect:eventLayer.bounds].CGPath;
...
- (void)setBounds:(CGRect)bounds {
  ...
  CGFloat diameter =
      MIN(self.bounds.size.height * 5.0 / 6.0, self.bounds.size.width);
  ...
  _eventLayer.frame =
      CGRectMake((_backgroundLayer.frame.size.width - eventSize) / 2 +
                     _backgroundLayer.frame.origin.x,
                 CGRectGetMaxY(_backgroundLayer.frame) + eventSize * 0.2,
                 eventSize * 0.8, eventSize * 0.8);
  _eventLayer.path =
      [UIBezierPath bezierPathWithOvalInRect:_eventLayer.bounds].CGPath;
...
#define m_adjustLabelFrame                                                     \
  if (_subtitle) {                                                             \
    ...
    _subtitleLabel.font =                                                      \
        [UIFont systemFontOfSize:_appearance.subtitleTextSize];                \
    CGFloat subtitleHeight = [_subtitleLabel.text sizeWithAttributes:@{        \
                               NSFontAttributeName : self.subtitleLabel.font   \
                             }].height;                                        \
    CGFloat height = titleHeight + subtitleHeight;                             \
    _titleLabel.frame =                                                        \
        CGRectMake(0, (self.contentView.fs_height * 5.0 / 6.0 - height) * 0.5, \
                   self.fs_width, titleHeight);                                \
                                                                               \
    _subtitleLabel.frame =                                                     \
        CGRectMake(0, _titleLabel.fs_bottom - (_titleLabel.fs_height -         \
                                               _titleLabel.font.pointSize),    \
                   self.fs_width, subtitleHeight);                             \
    _subtitleLabel.textColor = self.colorForSubtitleLabel;                     \
  } else {                                                                     \
    _titleLabel.frame = CGRectMake(                                            \
        0, 0, self.fs_width, floor(self.contentView.fs_height * 5.0 / 6.0));   \
    _subtitleLabel.hidden = YES;                                               \
  }

  if (self.calendar.ibEditing) {
    m_calculateTitleHeight m_adjustLabelFrame
  } else {
    dispatch_async(
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          m_calculateTitleHeight dispatch_async(dispatch_get_main_queue(), ^{
                                                    m_adjustLabelFrame});
        });
  }
  ...
  _backgroundLayer.hidden = !self.selected && !self.dateIsToday &&
                            !self.dateIsSelected && !borderColor;
  if (!_backgroundLayer.hidden) {
    _backgroundLayer.path =
        self.cellShape == FSCalendarCellShapeCircle
  ...
  if (!_eventLayer.hidden) {
    _eventLayer.fillColor =
        self.preferedEventColor.CGColor ?: _appearance.eventColor.CGColor;
  }
}
...
- (UIColor *)colorForCurrentStateInDictionary:(NSDictionary *)dictionary {
  if (self.hasEvent &&
      [[dictionary allKeys] containsObject:@(FSCalendarCellStateEvent)]) {
    return dictionary[@(FSCalendarCellStateEvent)];
  }
  if (self.isSelected || self.dateIsSelected) {
    if (self.dateIsToday) {
      return dictionary[
                 @(FSCalendarCellStateSelected | FSCalendarCellStateToday)]
                 ?: dictionary[@(FSCalendarCellStateSelected)];
    }
  ...
  if (self.dateIsToday &&
      [[dictionary allKeys] containsObject:@(FSCalendarCellStateToday)]) {
    return dictionary[@(FSCalendarCellStateToday)];
  }
  if (self.dateIsPlaceholder &&
      [[dictionary allKeys] containsObject:@(FSCalendarCellStatePlaceholder)]) {
    return dictionary[@(FSCalendarCellStatePlaceholder)];
  }
  if (self.isWeekend &&
      [[dictionary allKeys] containsObject:@(FSCalendarCellStateWeekend)]) {
...
//    return self.preferedSelectionColor
//               ?: [self colorForCurrentStateInDictionary:_appearance
//                                                             .backgroundColors];
//  }
...
- (UIColor *)colorForTitleLabel {
  if (self.dateIsSelected || self.isSelected) {
    return self.preferedTitleSelectionColor
               ?: [self
                      colorForCurrentStateInDictionary:_appearance.titleColors];
  }
  return self.preferedTitleDefaultColor
             ?: [self colorForCurrentStateInDictionary:_appearance.titleColors];
}

- (UIColor *)colorForSubtitleLabel {
  if (self.dateIsSelected || self.isSelected) {
    return self.preferedSubtitleSelectionColor
               ?: [self colorForCurrentStateInDictionary:_appearance
                                                             .subtitleColors];
  }
  return self.preferedSubtitleDefaultColor
             ?: [self colorForCurrentStateInDictionary:_appearance
                                                           .subtitleColors];
}
