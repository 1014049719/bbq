...
#define m_calculate                                                            \
  ...
  CGFloat weekdayHeight =                                                      \
      [@"1" sizeWithAttributes:@{                                              \
        NSFontAttributeName : [_weekdayLabels.lastObject font]                 \
      }].height;                                                               \
  CGFloat weekdayMargin =                                                      \
      (weekdayHeight * 0.4 + _contentView.fs_height * 0.2) * 0.5;              \
  CGFloat titleWidth = _contentView.fs_width;                                  \
  CGFloat titleHeight =                                                        \
      [@"1" sizeWithAttributes:@{                                              \
        NSFontAttributeName :                                                  \
            [UIFont systemFontOfSize:self.appearance.headerTitleTextSize]      \
      }].height;                                                               \
  CGFloat titleMargin =                                                        \
      (titleHeight * 0.2 + _contentView.fs_height * 0.1) * 0.5;

#define m_adjust                                                               \
  [_weekdayLabels                                                              \
      enumerateObjectsUsingBlock:^(UILabel * label, NSUInteger index,          \
                                   BOOL * stop) {                              \
    label.frame = CGRectMake(index * weekdayWidth,                             \
                             _contentView.fs_height - weekdayHeight,           \
                             weekdayWidth, weekdayHeight);                     \
      }];                                                                      \
  _separator.frame =                                                           \
      CGRectMake(0, _contentView.fs_height - weekdayHeight - weekdayMargin,    \
                 _contentView.fs_width, 1.0);                                  \
  _titleLabel.frame =                                                          \
      CGRectMake(0, _separator.fs_top - titleMargin - titleHeight, titleWidth, \
                 titleHeight);

  if (_calendar.ibEditing) {
    m_calculate m_adjust
  } else {
    dispatch_async(
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          m_calculate dispatch_async(dispatch_get_main_queue(), ^{
                                         m_adjust});
        });
  }
...
- (void)reloadData {
  NSArray *weekdaySymbols =
      self.appearance.useVeryShortWeekdaySymbols
          ? self.calendar.calendar.veryShortStandaloneWeekdaySymbols
          : self.calendar.calendar.shortStandaloneWeekdaySymbols;
  [_weekdayLabels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger index,
                                               BOOL *stop) {
