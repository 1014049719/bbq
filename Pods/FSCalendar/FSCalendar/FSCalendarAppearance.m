...
#define kPink                                                                  \
  [UIColor colorWithRed:198 / 255.0 green:51 / 255.0 blue:42 / 255.0 alpha:1.0]
#define kBlue                                                                  \
  [UIColor colorWithRed:31 / 255.0 green:119 / 255.0 blue:219 / 255.0 alpha:1.0]
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _subtitleColors[@(FSCalendarCellStatePlaceholder)] =
...
- (void)setTitleDefaultColor:(UIColor *)color {
  ...
  [_calendar.collectionView.visibleCells
...
- (void)setTodaySelectionColor:(UIColor *)todaySelectionColor {
  if (todaySelectionColor) {
    _backgroundColors[@(FSCalendarCellStateToday |
                        FSCalendarCellStateSelected)] = todaySelectionColor;
  } else {
    [_backgroundColors removeObjectForKey:@(FSCalendarCellStateToday |
...
- (void)adjustTitleIfNecessary {
  ...
  [_calendar.collectionView.visibleCells
      makeObjectsPerformSelector:@selector(setNeedsLayout)];
  [_calendar.header.collectionView reloadData];
  [_calendar.weekdays setValue:[UIFont systemFontOfSize:_weekdayTextSize]
                    forKeyPath:@"font"];
}
...
- (void)invalidateAppearance {
  ...
  [_calendar.header.collectionView.visibleCells
      makeObjectsPerformSelector:@selector(setNeedsLayout)];
  [_calendar.visibleStickyHeaders
      makeObjectsPerformSelector:@selector(setNeedsLayout)];
}
