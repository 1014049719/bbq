...
@interface FSCalendarHeader () <UICollectionViewDataSource,
...
- (void)layoutSubviews {
  ...
  if (_collectionView) {
    _collectionView.frame = CGRectMake(0, self.fs_height * 0.1, self.fs_width,
                                       self.fs_height * 0.9);
...
- (NSInteger)collectionView:(UICollectionView *)collectionView
    ...
    case UICollectionViewScrollDirectionVertical: {
      NSInteger count =
          [self.calendar.maximumDate
              fs_monthsFrom:self.calendar.minimumDate.fs_firstDayOfMonth] +
          1;
      return count;
    }
    case UICollectionViewScrollDirectionHorizontal: {
      // 这里需要默认多出两项，否则当contentOffset为负时，切换到其他页面时会自动归零
      // 2 more pages to prevent scrollView from auto bouncing while
      // push/present to other UIViewController
      NSInteger count =
          [self.calendar.maximumDate
              fs_monthsFrom:self.calendar.minimumDate.fs_firstDayOfMonth] +
          1;
  ...
  case FSCalendarScopeWeek: {
    NSInteger count =
        [self.calendar.maximumDate
            fs_weeksFrom:self.calendar.minimumDate.fs_firstDayOfWeek] +
        1;
    return count + 2;
  }
  default: { break; }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  FSCalendarHeaderCell *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:@"cell"
                                                forIndexPath:indexPath];
  cell.titleLabel.font =
      [UIFont systemFontOfSize:_appearance.headerTitleTextSize];
  ...
  case FSCalendarScopeMonth: {
    if (_scrollDirection == UICollectionViewScrollDirectionHorizontal) {
      // 多出的两项需要制空
      if ((indexPath.item == 0 ||
           indexPath.item == [collectionView numberOfItemsInSection:0] - 1)) {
        cell.titleLabel.text = nil;
      } else {
        NSDate *date =
            [self.calendar.minimumDate fs_dateByAddingMonths:indexPath.item - 1]
                .fs_dateByIgnoringTimeComponents;
        cell.titleLabel.text = [_dateFormatter stringFromDate:date];
      }
    } else {
      NSDate *date =
          [self.calendar.minimumDate fs_dateByAddingMonths:indexPath.item]
              .fs_dateByIgnoringTimeComponents;
      cell.titleLabel.text = [_dateFormatter stringFromDate:date];
    }
    break;
  ...
  case FSCalendarScopeWeek: {
    if ((indexPath.item == 0 ||
      ...
      NSDate *date = [self.calendar.minimumDate.fs_firstDayOfWeek
                         fs_dateByAddingWeeks:indexPath.item - 1]
                         .fs_dateByIgnoringTimeComponents;
      cell.titleLabel.text = [_dateFormatter stringFromDate:date];
    }
    break;
...
- (void)layoutSubviews {
  ...
  if (self.header.scrollDirection ==
      UICollectionViewScrollDirectionHorizontal) {
    CGFloat position =
        [self.contentView
            convertPoint:CGPointMake(CGRectGetMidX(self.contentView.bounds),
    ...
    if (self.header.scrollEnabled) {
      self.contentView.alpha =
          1.0 -
          (1.0 - self.header.appearance.headerMinimumDissolvedAlpha) *
              ABS(center - position) / self.fs_width;
    } else {
      self.contentView.alpha =
          (position > 0 && position < self.header.fs_width * 0.75);
    }
  } else if (self.header.scrollDirection ==
    ...
    self.contentView.alpha =
        1.0 -
        (1.0 - self.header.appearance.headerMinimumDissolvedAlpha) *
            ABS(center - position) / self.fs_height;
  }
}
