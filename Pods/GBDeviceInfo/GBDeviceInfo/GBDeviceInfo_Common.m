...
- (BOOL)isOperatingSystemAtLeastVersion:(NSString *)versionString {
  return [self
      isOperatingSystemAtLeastOSVersion:GBOSVersionFromString(versionString)];
}
...
+ (NSString *)_sysctlStringForKey:(NSString *)key {
  ...
  if (length) {
    ...
    answer = [NSString stringWithCString:answerCString
...
+ (GBCPUInfo)_cpuInfo {
  return GBCPUInfoMake(
      [self _sysctlCGFloatForKey:kHardwareCPUFrequencyKey] /
          1000000000., // giga
