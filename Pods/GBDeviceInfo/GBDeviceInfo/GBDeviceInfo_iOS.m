...
- (BOOL)isJailbroken {
  @throw [NSException
      exceptionWithName:NSInternalInconsistencyException
                 reason:@"You have to include the Jailbreak subspec in order "
                        @"to access this property. Add `pod "
                        @"'GBDeviceInfo/Jailbreak'` to your Podfile."
               userInfo:nil];
}
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"%@\nrawSystemInfoString: %@\nmodel: %ld\nfamily: "
                       ...
                       [super description], self.rawSystemInfoString,
                       (long)self.model, (long)self.family, (long)self.display,
                       ...
                       self.cpuInfo.l2CacheSize, self.physicalMemory];
}
...
+ (GBDeviceVersion)_deviceVersion {
  ...
  NSUInteger positionOfFirstInteger =
      [systemInfoString
  ...
  if (positionOfComma != NSNotFound) {
    major = [[systemInfoString
        substringWithRange:NSMakeRange(
                               positionOfFirstInteger,
                               positionOfComma -
                                   positionOfFirstInteger)] integerValue];
    minor = [
...
+ (NSArray *)_modelNuances {
  ...
  if (TARGET_IPHONE_SIMULATOR) {
    ...
    model =
        iPadScreen ? GBDeviceModelSimulatoriPad : GBDeviceModelSimulatoriPhone;
  ...
  // Actual device
    ...
    NSDictionary *modelManifest = @{
      @"iPhone" : @{
        ...
        // 3GS
        @[ @(2), @(1) ] :
            @[ @(GBDeviceModeliPhone3GS), @"iPhone 3GS", @(163) ],
        ...
        // 6 Plus
        @[ @(7), @(1) ] :
            @[ @(GBDeviceModeliPhone6Plus), @"iPhone 6 Plus", @(401) ],
        ...
        // 6S Plus
        @[ @(8), @(2) ] :
            @[ @(GBDeviceModeliPhone6SPlus), @"iPhone 6S Plus", @(401) ],
      },
      @"iPad" : @{
        // 1
        ...
        // Mini
        @[ @(2), @(5) ] :
            @[ @(GBDeviceModeliPadMini1), @"iPad Mini 1", @(163) ],
        @[ @(2), @(6) ] :
            @[ @(GBDeviceModeliPadMini1), @"iPad Mini 1", @(163) ],
        @[ @(2), @(7) ] :
            @[ @(GBDeviceModeliPadMini1), @"iPad Mini 1", @(163) ],
        ...
        // Mini 2
        @[ @(4), @(4) ] :
            @[ @(GBDeviceModeliPadMini2), @"iPad Mini 2", @(326) ],
        @[ @(4), @(5) ] :
            @[ @(GBDeviceModeliPadMini2), @"iPad Mini 2", @(326) ],
        @[ @(4), @(6) ] :
            @[ @(GBDeviceModeliPadMini2), @"iPad Mini 2", @(326) ],
        ...
        // Mini 3
        @[ @(4), @(7) ] :
            @[ @(GBDeviceModeliPadMini3), @"iPad Mini 3", @(326) ],
        @[ @(4), @(8) ] :
            @[ @(GBDeviceModeliPadMini3), @"iPad Mini 3", @(326) ],
        @[ @(4), @(9) ] :
            @[ @(GBDeviceModeliPadMini3), @"iPad Mini 3", @(326) ],
    ...
    for (NSString *familyString in familyManifest) {
      if ([systemInfoString hasPrefix:familyString]) {
        ...
        NSArray *modelNuances = modelManifest[familyString][
            @[ @(deviceVersion.major), @(deviceVersion.minor) ]];
        if (modelNuances) {
...
+ (GBOSVersion)_osVersion {
  ...
  NSArray *decomposedOSVersion =
      [[UIDevice currentDevice].systemVersion componentsSeparatedByString:@"."];
  if (decomposedOSVersion.count > 0)
    majorVersion = [decomposedOSVersion[0] integerValue];
  if (decomposedOSVersion.count > 1)
    minorVersion = [decomposedOSVersion[1] integerValue];
  if (decomposedOSVersion.count > 2)
    patchVersion = [decomposedOSVersion[2] integerValue];
