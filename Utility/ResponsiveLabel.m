...
- (void)setCustomTruncationEnabled:(BOOL)customTruncationEnabled {
  ...
  [self shouldAppendTruncationToken] ? [self appendTokenIfNeeded]
                                     : [self removeTokenIfPresent];
}

- (void)setTruncationToken:(NSString *)truncationToken {
  self.attributedTruncationToken = [[NSAttributedString alloc]
      initWithString:truncationToken
          attributes:[self attributesFromProperties]];
...
/**
 This method calculates text offset to draw the given glyph range such that text
 is center aligned veritically
...
- (CGPoint)textOffsetForGlyphRange:(NSRange)glyphRange {
  ...
  CGFloat paddingHeight =
      (self.bounds.size.height - textBounds.size.height) / 2.0f;
  if (paddingHeight > 0)
...
- (void)redrawTextForRange:(NSRange)range {
  ...
  [self.layoutManager characterRangeForGlyphRange:range
...
- (CGRect)textRectForBounds:(CGRect)bounds
  ...
  NSInteger savedTextContainerNumberOfLines =
      self.textContainer.maximumNumberOfLines;
  ...
  @try {
    ...
    if (numberOfLines > 0 && (numberOfLines < totalLines)) {
      textBounds.size.height -=
          (totalLines - numberOfLines) * self.font.lineHeight;
    } else if (numberOfLines > 0 && (numberOfLines > totalLines)) {
      textBounds.size.height +=
          (numberOfLines - totalLines) * self.font.lineHeight;
    }
...
- (void)appendTokenIfNeeded {
  if ([self shouldAppendTruncationToken] && ![self truncationTokenAppended]) {
    if ([self.textStorage isNewLinePresent]) {
      ...
      [self.textStorage
          replaceCharactersInRange:
              [self rangeForTokenInsertionForStringWithNewLine]
    ...
    if (tokenRange.location != NSNotFound) {
      [self.textStorage
          replaceCharactersInRange:tokenRange
              withAttributedString:self.attributedTruncationToken];
    ...
    if (truncationRange.location != NSNotFound) {
      ...
      NSString *key =
          [NSString stringWithFormat:kRegexFormatForSearchWord,
                                     self.attributedTruncationToken.string];
      PatternDescriptor *descriptor =
          [self.patternDescriptorDictionary objectForKey:key];
      if (descriptor) {
        [self.textStorage addAttributes:descriptor.patternAttributes
...
- (void)removeTokenIfPresent {
  if ([self truncationTokenAppended]) {
    ...
    NSRange truncationRange = [self.textStorage.string
        rangeOfString:self.attributedTruncationToken.string];
    NSMutableAttributedString *finalString = [[NSMutableAttributedString alloc]
        initWithAttributedString:self.textStorage];

    if (truncationRange.location != NSNotFound) {
      NSRange rangeOfTuncatedString =
          NSMakeRange(truncationRange.location,
                      self.attributedText.length - truncationRange.location);
      NSAttributedString *truncatedString = [self.attributedText
          attributedSubstringFromRange:rangeOfTuncatedString];
      [finalString replaceCharactersInRange:truncationRange
                       withAttributedString:truncatedString];
    }
...
- (NSRange)rangeForTokenInsertion {
  ...
  NSInteger glyphIndex = [self.layoutManager
      glyphIndexForCharacterAtIndex:self.textStorage.length - 1];
  NSRange range = [self.layoutManager
      truncatedGlyphRangeInLineFragmentForGlyphAtIndex:glyphIndex];
...
- (NSRange)rangeForTokenInsertionForStringWithNewLine {
  ...
  NSRange rangeOfText = NSMakeRange(
...
- (NSRange)rangeOfTruncationToken {
  ...
  if (self.attributedTruncationToken && self.customTruncationEnabled) {
    truncationRange = [self.textStorage.string
        rangeOfString:self.attributedTruncationToken.string];
  } else {
...
- (BOOL)shouldAppendTruncationToken {
  return (self.textStorage.length > 0 && self.customTruncationEnabled &&
          self.attributedTruncationToken.length > 0);
}
...
- (void)updateTruncationToken:(NSAttributedString *)attributedTruncationToken
                   withAction:(PatternTapResponder)action {
  NSError *error;
  if (self.attributedTruncationToken.length > 0) {
    NSString *patternKey =
        [NSString stringWithFormat:kRegexFormatForSearchWord,
                                   self.attributedTruncationToken.string];
    [self disablePatternDetection:[self.patternDescriptorDictionary
                                      objectForKey:patternKey]];
  }
  ...
  NSRegularExpression *regex =
      [[NSRegularExpression alloc] initWithPattern:pattern
                                           options:0
                                             error:&error];
  PatternDescriptor *descriptor = [[PatternDescriptor alloc]
              initWithRegex:regex
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (index < self.textStorage.length) {
    rangeOfTappedText =
        [self.layoutManager rangeOfNominallySpacedGlyphsContainingIndex:index];
  }

  if (rangeOfTappedText.location != NSNotFound &&
      ![self patternTouchInProgress] && [self shouldHandleTouchAtIndex:index]) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  if ([self patternTouchInProgress] &&
      [self shouldHandleTouchAtIndex:self.selectedRange.location]) {
...
- (void)handleTouchBeginForRange:(NSRange)range {
  if (![self patternTouchInProgress]) {
    ...
    self.currentAttributedString = [[NSMutableAttributedString alloc]
        initWithAttributedString:self.textStorage];
...
/**
 ...
 Touch will be handled if any of these attributes are set:
 RLTapResponderAttributeName
                                             or
...
- (BOOL)shouldHandleTouchAtIndex:(NSInteger)index {
  ...
  NSDictionary *dictionary =
      [self.textStorage attributesAtIndex:index effectiveRange:&range];
  BOOL touchAttributesSet =
      (dictionary &&
       ([dictionary.allKeys containsObject:RLTapResponderAttributeName] ||
        [dictionary.allKeys
            containsObject:RLHighlightedBackgroundColorAttributeName] ||
        [dictionary.allKeys
            containsObject:RLHighlightedForegroundColorAttributeName]));
...
- (void)performActionAtIndex:(NSInteger)index {
  ...
  if (index < self.textStorage.length) {
    PatternTapResponder tapResponder =
        [self.textStorage attribute:RLTapResponderAttributeName
                            atIndex:index
...
- (void)addHighlightingForIndex:(NSInteger)index {
  ...
  if (index < self.textStorage.length) {
    backgroundcolor =
        [self.textStorage attribute:RLHighlightedBackgroundColorAttributeName
                            atIndex:index
                     effectiveRange:&patternRange];
    foregroundcolor =
        [self.textStorage attribute:RLHighlightedForegroundColorAttributeName
                            atIndex:index
                     effectiveRange:&patternRange];
...
- (void)removeHighlightingForIndex:(NSInteger)index {
  if (self.selectedRange.location != NSNotFound &&
    ...
    if (index < self.textStorage.length) {
      backgroundcolor =
          [self.currentAttributedString attribute:NSBackgroundColorAttributeName
                                          atIndex:index
                                   effectiveRange:&patternRange];
      foregroundcolor =
          [self.currentAttributedString attribute:NSForegroundColorAttributeName
                                          atIndex:index
                                   effectiveRange:&patternRange];
...
- (void)addAttributesForPatternDescriptor:
    (PatternDescriptor *)patternDescriptor {
  // Generate ranges for attributed text of the label
  NSArray *patternRanges =
      [patternDescriptor patternRangesForString:self.attributedText.string];

  // Apply attributes to the ranges conditionally
  [patternRanges
      enumerateObjectsUsingBlock:^(NSValue *obj, NSUInteger idx, BOOL *stop) {
        if ([self shouldAddAttributesAtRange:obj.rangeValue]) {
          [self.textStorage addAttributes:patternDescriptor.patternAttributes
...
- (void)removeAttributesForPatternDescriptor:
    (PatternDescriptor *)patternDescriptor {
  // Generate ranges for current text of textStorage
  NSArray *patternRanges =
      [patternDescriptor patternRangesForString:self.textStorage.string];

  // Remove attributes from the ranges conditionally
  [patternRanges enumerateObjectsUsingBlock:^(NSValue *obj, NSUInteger idx,
                                              BOOL *stop) {
    if ([self shouldRemoveAttributesFromRange:obj.rangeValue]) { // Do nothing
                                                                 // if range is
                                                                 // truncated
      [patternDescriptor.patternAttributes
          enumerateKeysAndObjectsUsingBlock:^(NSString *attributeName,
                                              id attributeValue, BOOL *stop) {
            [self.textStorage removeAttribute:attributeName
                                        range:obj.rangeValue];
          }];
      [self redrawTextForRange:obj.rangeValue];
    }
...
- (BOOL)shouldAddAttributesAtRange:(NSRange)range {
  ...
  BOOL doesIntersectTruncationRange =
      NSIntersectionRange(range, truncationRange).length == 0;
  BOOL isRangeOutOfBound =
      (range.location + range.length) > self.textStorage.length;
  return ((!isRangeOutOfBound && doesIntersectTruncationRange) ||
          isTruncationRange);
}
...
- (void)removeAttributeForTruncatedRange {
  ...
  [self.patternDescriptorDictionary
      enumerateKeysAndObjectsUsingBlock:^(id key, PatternDescriptor *descriptor,
                                          BOOL *stop) {

        // Generate ranges for attributed text of the label
        NSArray *ranges =
            [descriptor patternRangesForString:self.attributedText.string];

        [ranges enumerateObjectsUsingBlock:^(NSValue *obj, NSUInteger idx,
                                             BOOL *stop) {
          NSRange interSectionRange =
              NSIntersectionRange(obj.rangeValue, truncationRange);
          BOOL doesIntersectTruncationRange = (interSectionRange.length > 0);
          BOOL isTruncationRange =
          ...
          if (doesIntersectTruncationRange && !isTruncationRange) {
            NSRange visibleRange =
                NSMakeRange(obj.rangeValue.location,
                            obj.rangeValue.length - interSectionRange.length);
            [descriptor.patternAttributes
                enumerateKeysAndObjectsUsingBlock:^(id key, id obj,
                                                    BOOL *stop) {
...
/**
This method returns the key for the given PatternDescriptor stored in
patternDescriptorDictionary.
In patternDescriptorDictionary, each entry has the format (NSString,
PatternDescriptor).
@param: PatternDescriptor
@return: NSString
*/

- (NSString *)patternNameKeyForPatternDescriptor:
    (PatternDescriptor *)patternDescriptor {
  NSString *key;
  if ([patternDescriptor.patternExpression
          isKindOfClass:[NSDataDetector class]]) {
    ...
    NSTextCheckingTypes types =
        ((NSDataDetector *)patternDescriptor.patternExpression).checkingTypes;
...
- (void)initialTextConfiguration {
  ...
  if (self.attributedText.length > 0) {
    ...
    currentText = [[NSAttributedString alloc]
...
- (NSUInteger)characterIndexAtLocation:(CGPoint)location {
  ...
  if (self.textStorage.string.length > 0) {
    ...
    NSRange lineRange;
    CGRect lineRect =
        [self.layoutManager lineFragmentUsedRectForGlyphAtIndex:glyphIndex
                                                 effectiveRange:&lineRange];
    lineRect.size.height = 60; // Adjustment to increase tap area
    if (CGRectContainsPoint(lineRect, location)) {
      chracterIndex =
          [self.layoutManager characterIndexForGlyphAtIndex:glyphIndex];
    }
  }
...
- (NSUInteger)glyphIndexForLocation:(CGPoint)location {
  ...
  // Get the touch location and use text offset to convert to text cotainer
  // coords
...
- (void)setAttributedText:(NSAttributedString *)attributedText
           withTruncation:(BOOL)truncation {
...
- (void)setTruncationIndicatorImage:(UIImage *)image
                           withSize:(CGSize)size
                          andAction:(PatternTapResponder)action {
  ...
  textAttachment.bounds =
      CGRectMake(0, -self.font.descender - self.font.lineHeight / 2, size.width,
                 size.height);
  ...
  NSMutableAttributedString *finalString = [[NSMutableAttributedString alloc]
      initWithAttributedString:paddingString];
...
- (void)enableHashTagDetectionWithAttributes:(NSDictionary *)dictionary {
  NSError *error;
  NSRegularExpression *regex =
      [[NSRegularExpression alloc] initWithPattern:kRegexStringForHashTag
                                           options:0
                                             error:&error];
  PatternDescriptor *descriptor =
      [[PatternDescriptor alloc] initWithRegex:regex
...
- (void)disableHashTagDetection {
  [self disablePatternDetection:[self.patternDescriptorDictionary
                                    objectForKey:kRegexStringForHashTag]];
}

- (void)enableUserHandleDetectionWithAttributes:(NSDictionary *)dictionary {
  NSError *error;
  NSRegularExpression *regex =
      [[NSRegularExpression alloc] initWithPattern:kRegexStringForUserHandle
                                           options:0
                                             error:&error];
  PatternDescriptor *descriptor =
      [[PatternDescriptor alloc] initWithRegex:regex
...
- (void)disableUserHandleDetection {
  [self disablePatternDetection:[self.patternDescriptorDictionary
                                    objectForKey:kRegexStringForUserHandle]];
}

- (void)enableStringDetection:(NSString *)string
               withAttributes:(NSDictionary *)dictionary {
  NSError *error;
  NSString *pattern =
      [NSString stringWithFormat:kRegexFormatForSearchWord, string];
  NSRegularExpression *regex =
      [[NSRegularExpression alloc] initWithPattern:pattern
                                           options:0
                                             error:&error];
...
- (void)enableDetectionForStrings:(NSArray *)stringsArray
                   withAttributes:(NSDictionary *)dictionary {
  [stringsArray enumerateObjectsUsingBlock:^(NSString *string, NSUInteger idx,
                                             BOOL *stop) {
...
- (void)enablePatternDetection:(PatternDescriptor *)patternDescriptor {
  NSString *patternkey =
      [self patternNameKeyForPatternDescriptor:patternDescriptor];
...
- (void)enableURLDetectionWithAttributes:(NSDictionary *)dictionary {
  ...
  NSDataDetector *detector =
      [[NSDataDetector alloc] initWithTypes:NSTextCheckingTypeLink
                                      error:&error];
  PatternDescriptor *descriptor =
      [[PatternDescriptor alloc] initWithRegex:detector
                                withSearchType:PatternSearchTypeAll
                         withPatternAttributes:dictionary];
  [self enablePatternDetection:descriptor];
}
