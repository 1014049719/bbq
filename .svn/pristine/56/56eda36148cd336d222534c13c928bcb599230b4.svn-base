...
- (instancetype)bind:(RACStreamBindBlock (^)(void))block {
  ...
  RACSequence *result = (id)
      [bindBlock(self.head, &stop) setNameWithFormat:@"[%@] -bind:", self.name];
...
- (void)encodeWithCoder:(NSCoder *)coder {
  if (self.head != nil)
    [coder encodeObject:self.head forKey:@keypath(self.head)];
}

#pragma mark NSObject

- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p>{ name = %@, head = %@ }",
                                    self.class, self, self.name, self.head];
}
...
- (BOOL)isEqual:(RACUnarySequence *)seq {
  if (self == seq)
    return YES;
  if (![seq isKindOfClass:RACUnarySequence.class])
    return NO;
