...
RACCompareScheduledActions(const void *ptr1, const void *ptr2, void *info) {
  RACTestSchedulerAction *action1 = (__bridge id)ptr1;
  RACTestSchedulerAction *action2 = (__bridge id)ptr2;
  return CFDateCompare((__bridge CFDateRef)action1.date,
                       (__bridge CFDateRef)action2.date, NULL);
}

static const void *RACRetainScheduledAction(CFAllocatorRef allocator,
                                            const void *ptr) {
  return CFRetain(ptr);
}

static void RACReleaseScheduledAction(CFAllocatorRef allocator,
                                      const void *ptr) {
  CFRelease(ptr);
}
...
- (instancetype)init {
  self =
      [super initWithName:@"org.reactivecocoa.ReactiveCocoa.RACTestScheduler"];
  if (self == nil)
    return nil;

  CFBinaryHeapCallBacks callbacks =
      (CFBinaryHeapCallBacks){.version = 0,
    ...
    for (NSUInteger i = 0; i < ticks; i++) {
      ...
      if (previousScheduler != nil) {
        NSThread.currentThread
            .threadDictionary[RACSchedulerCurrentSchedulerKey] =
            previousScheduler;
      } else {
...
- (RACDisposable *)schedule:(void (^)(void))block {
  ...
  @synchronized(self) {
    NSDate *uniqueDate = [NSDate dateWithTimeIntervalSinceReferenceDate:
                                     self.numberOfDirectlyScheduledBlocks];
    self.numberOfDirectlyScheduledBlocks++;

    RACTestSchedulerAction *action =
        [[RACTestSchedulerAction alloc] initWithDate:uniqueDate block:block];
...
- (RACDisposable *)after:(NSDate *)date schedule:(void (^)(void))block {
  ...
  @synchronized(self) {
    RACTestSchedulerAction *action =
        [[RACTestSchedulerAction alloc] initWithDate:date block:block];
    CFBinaryHeapAddValue(self.scheduledActions, (__bridge void *)action);
  ...
  @synchronized(self) {
    ...
    void (^reschedulingBlock)(void) = ^{
      ...
      RACDisposable *schedulingDisposable =
          [self after:[date dateByAddingTimeInterval:interval]
    ...
    RACTestSchedulerAction *action =
        [[RACTestSchedulerAction alloc] initWithDate:date
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p>{ date: %@ }", self.class, self, self.date];
}
