...
CGContextRef NYXCreateARGBBitmapContext(const size_t width, const size_t height,
                                        const size_t bytesPerRow,
                                        BOOL withAlpha) {
  /// Use the generic RGB color space
  /// We avoid the NULL check because CGColorSpaceRelease() NULL check the value
  /// anyway, and worst case scenario = fail to create context
  /// Create the bitmap context, we want pre-multiplied ARGB, 8-bits per
  /// component
  CGImageAlphaInfo alphaInfo = (withAlpha ? kCGImageAlphaPremultipliedFirst
                                          : kCGImageAlphaNoneSkipFirst);
  CGContextRef bmContext = CGBitmapContextCreate(
      NULL, width, height, 8 /*Bits per component*/, bytesPerRow,
      NYXGetRGBColorSpace(), kCGBitmapByteOrderDefault | alphaInfo);
...
CGImageRef NYXCreateGradientImage(const size_t pixelsWide,
  ...
  CGContextRef gradientBitmapContext =
      CGBitmapContextCreate(NULL, pixelsWide, pixelsHigh, 8, 0, colorSpace,
...
CIContext *NYXGetCIContext(void) {
  if (!__ciContext) {
    ...
    NSDictionary *opts = [[NSDictionary alloc]
        initWithObjectsAndKeys:num, kCIContextUseSoftwareRenderer, nil];
...
BOOL NYXImageHasAlpha(CGImageRef imageRef) {
  ...
  BOOL hasAlpha = (alpha == kCGImageAlphaFirst || alpha == kCGImageAlphaLast ||
