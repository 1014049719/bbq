...
// A block used to evaluate head. This should be set to nil after `_head` has
// been
// initialized.
//
...
// A block used to evaluate tail. This should be set to nil after `_tail` has
// been
// initialized.
//
// This is marked `strong` instead of `copy` because of some bizarre block
...
- (NSString *)description {
  ...
  return
      [NSString stringWithFormat:@"<%@: %p>{ name = %@, head = %@, tail = %@ }",
                                 self.class, self, self.name, head, tail];
}
