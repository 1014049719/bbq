...
#define CNP_SYSTEM_VERSION_LESS_THAN(v)                                        \
([[[UIDevice currentDevice] systemVersion] compare:v                         \
options:NSNumericSearch] ==       \
NSOrderedAscending)
#define CNP_IS_IPAD (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)

static inline UIViewAnimationOptions
UIViewAnimationCurveToAnimationOptions(UIViewAnimationCurve curve) {
    return curve << 16;
}
...
- (instancetype)initWithContents:(NSArray *)contents {
    ...
    if (self) {
        ...
        self.maskView =
        [[UIView alloc] initWithFrame:self.applicationWindow.bounds];
        self.maskView.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.4];
        self.backgroundTapRecognizer = [[UITapGestureRecognizer alloc]
                                        initWithTarget:self
                                        action:@selector(handleBackgroundTapGesture:)];
...
- (void)orientationChanged {
    ...
    CGFloat angle =
    UIInterfaceOrientationAngleOfOrientation(statusBarOrientation);
...
- (void)applyTheme {
    ...
    self.popupView.layer.cornerRadius =
    self.theme.popupStyle == CNPPopupStyleCentered ? self.theme.cornerRadius
    : 0;
    ...
    if (self.theme.popupStyle == CNPPopupStyleFullscreen) {
        ...
        maskBackgroundColor = self.theme.maskType == CNPPopupMaskTypeClear
        ? [UIColor clearColor]
...
- (CGSize)calculateContentSizeThatFits:(CGSize)size
                       andUpdateLayout:(BOOL)update {
    ...
    for (UIView *view in self.popupView.subviews) {
        ...
        if (!view.hidden) {
            ...
            if (CGSizeEqualToSize(_size, CGSizeZero)) {
                ...
                if (update)
                    view.frame =
                    CGRectMake(inset.left, result.height, _size.width, _size.height);
            } else {
                if (update) {
                    //                    view.frame =
                    //                    CGRectMake((size.width-_size.width+inset.left+inset.right)*0.5,
                    //                    result.height, _size.width, _size.height);
...
- (void)keyboardWillShow:(NSNotification *)notification {
    if (self.theme.movesAboveKeyboard) {
        ...
        UIViewAnimationCurve curve = [[notification.userInfo
                                       objectForKey:UIKeyboardAnimationCurveUserInfoKey] integerValue];
        
        [self keyboardWithEndFrame:frame
             willShowAfterDuration:duration
                       withOptions:UIViewAnimationCurveToAnimationOptions(curve)];
    }
}

- (void)keyboardWithEndFrame:(CGRect)keyboardFrame
       willShowAfterDuration:(NSTimeInterval)duration
                 withOptions:(UIViewAnimationOptions)options {
    CGRect popupViewIntersection =
    CGRectIntersection(self.popupView.frame, keyboardFrame);
    
    if (popupViewIntersection.size.height > 0) {
        CGRect maskViewIntersection =
        CGRectIntersection(self.maskView.frame, keyboardFrame);
        ...
        [UIView animateWithDuration:duration
                         ...
                         animations:^{
                             ...
                             CGPointMake(self.popupView.center.x,
                                         (CGRectGetHeight(self.maskView.frame) -
                                          maskViewIntersection.size.height) /
                                         2);
                         }
                         completion:nil];
    }
...
- (void)keyboardWillHide:(NSNotification *)notification {
    if (self.theme.movesAboveKeyboard) {
        ...
        [self keyboardWithStartFrame:frame
               willHideAfterDuration:duration
                         withOptions:UIViewAnimationCurveToAnimationOptions(curve)];
    }
}

- (void)keyboardWithStartFrame:(CGRect)keyboardFrame
         willHideAfterDuration:(NSTimeInterval)duration
                   withOptions:(UIViewAnimationOptions)options {
    [UIView animateWithDuration:duration
                          delay:0.0f
                        options:options
...
- (void)presentPopupControllerAnimated:(BOOL)flag {
    if ([self.delegate
         respondsToSelector:@selector(popupControllerWillPresent:)]) {
    ...
    [self applyTheme];
    [self
     calculateContentSizeThatFits:CGSizeMake([self popupWidth],
                                             self.maskView.bounds.size.height)
     andUpdateLayout:YES];
    self.popupView.center = [self originPoint];
    [self.applicationWindow addSubview:self.maskView];
    self.maskView.alpha = 0;
    [UIView animateWithDuration:flag ? 0.3 : 0.0
                     animations:^{
                     ...
                     completion:^(BOOL finished) {
                         ...
                         if ([self.delegate
                              respondsToSelector:@selector(popupControllerDidPresent:)]) {
...
- (void)dismissPopupControllerAnimated:(BOOL)flag {
    if ([self.delegate
         respondsToSelector:@selector(popupControllerWillDismiss:)]) {
    ...
    [UIView animateWithDuration:flag ? 0.3 : 0.0
                     ...
                     completion:^(BOOL finished) {
                         ...
                         if ([self.delegate
                              respondsToSelector:@selector(popupControllerDidDismiss:)]) {
...
- (CGPoint)originPoint {
    ...
    switch (self.theme.presentationStyle) {
        ...
        case CNPPopupPresentationStyleSlideInFromBottom:
            origin = CGPointMake(self.maskView.center.x,
                                 self.maskView.bounds.size.height +
                                 self.popupView.bounds.size.height);
            break;
        case CNPPopupPresentationStyleSlideInFromLeft:
            origin =
            CGPointMake(-self.popupView.bounds.size.width, self.maskView.center.y);
            break;
        case CNPPopupPresentationStyleSlideInFromRight:
            origin = CGPointMake(self.maskView.bounds.size.width +
                                 self.popupView.bounds.size.width,
...
- (CGPoint)endingPoint {
    CGPoint center;
    if (self.theme.popupStyle == CNPPopupStyleActionSheet) {
        center = CGPointMake(self.maskView.center.x,
                             self.maskView.bounds.size.height -
                             (self.popupView.bounds.size.height * 0.5));
    } else {
...
- (CGPoint)dismissedPoint {
    ...
    switch (self.theme.presentationStyle) {
        case CNPPopupPresentationStyleFadeIn:
            ...
            break;
        case CNPPopupPresentationStyleSlideInFromBottom:
            dismissed = self.theme.dismissesOppositeDirection
            ? CGPointMake(self.maskView.center.x,
                          -self.popupView.bounds.size.height)
            : CGPointMake(self.maskView.center.x,
                          self.maskView.bounds.size.height +
                          self.popupView.bounds.size.height);
            if (self.theme.popupStyle == CNPPopupStyleActionSheet) {
                dismissed = CGPointMake(self.maskView.center.x,
                                        self.maskView.bounds.size.height +
                                        self.popupView.bounds.size.height);
            }
            break;
        case CNPPopupPresentationStyleSlideInFromLeft:
            dismissed = self.theme.dismissesOppositeDirection
            ? CGPointMake(self.maskView.bounds.size.width +
                          self.popupView.bounds.size.width,
                          self.maskView.center.y)
            : CGPointMake(-self.popupView.bounds.size.width,
                          self.maskView.center.y);
            break;
        case CNPPopupPresentationStyleSlideInFromRight:
            dismissed = self.theme.dismissesOppositeDirection
...
- (CGFloat)popupWidth {
    ...
    if ((self.theme.popupStyle == CNPPopupStyleActionSheet && !CNP_IS_IPAD) ||
...
+ (CNPPopupTheme *)defaultTheme {
    ...
    defaultTheme.popupContentInsets =
    UIEdgeInsetsMake(16.0f, 16.0f, 16.0f, 16.0f);
