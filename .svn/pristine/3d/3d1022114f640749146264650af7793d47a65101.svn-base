...
@interface RACMulticastConnection () {
  ...
  // When connecting, a caller should attempt to atomically swap the value of
  // this
  // from `0` to `1`.
  //
  // If the swap is successful the caller is resposible for subscribing
  // `_signal`
  // to `sourceSignal` and storing the returned disposable in
  // `serialDisposable`.
  //
