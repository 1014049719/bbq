...
- (void)addKeyframeForTime:(CGFloat)time strokeStart:(CGFloat)strokeStart {
  if (![self validStrokeStart:strokeStart])
    return;
...
- (BOOL)validStrokeStart:(CGFloat)strokeStart {
  NSAssert((strokeStart >= 0.f) && (strokeStart <= 1.f),
           @"Stroke Start values must be between zero and one.");
  if ((strokeStart < 0.f) || (strokeStart > 1.f))
    return NO;
  return YES;
}
...
- (void)createBasicAnimation {
  ...
  [self.layer addAnimation:[self strokeStartAnimation]
