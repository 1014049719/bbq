...
typedef NS_ENUM(NSUInteger, BBQTableViewControllerViewMode) {
    BBQTableViewControllerViewModeStatic = 1,
    BBQTableViewControllerViewModeTable,
};
...
- (instancetype)initWithStyle:(UITableViewStyle)style {
    if ((self = [super initWithStyle:style])) {
        ...
        // iOS 9 -initWithStyle does not call -initWithNibName so we'll call -onInit
        // here.
        if (kiOS9Later) {
            [self onInit];
        }
...
- (void)viewDidLoad {
    ...
    [[UIView alloc] initWithFrame:self.tableView.bounds];
...
- (void)setStatefulState:(BBQTableViewControllerState)state {
...
- (void)setStatefulState:(BBQTableViewControllerState)state
    ...
    if (state == BBQTableViewControllerStateInitialLoading) {
    ...
    if (state == BBQTableViewControllerStateIdle) {
    ...
    if (updateViewMode) {
        BBQTableViewControllerViewMode viewMode;
        switch (state) {
            case BBQTableViewControllerStateInitialLoading:
            case BBQTableViewControllerStateEmptyOrInitialLoadError:
                viewMode = BBQTableViewControllerViewModeStatic;
                break;
            case BBQTableViewControllerStateInitialLoadingTableView:
            default:
                viewMode = BBQTableViewControllerViewModeTable;
                break;
        }
...
- (BOOL)triggerInitialLoadShowingTableView:(BOOL)showTableView {
    ...
    if (showTableView)
        [self setStatefulState:
         BBQTableViewControllerStateInitialLoadingTableView];
    else
        [self setStatefulState:BBQTableViewControllerStateInitialLoading];
    
    __weak typeof(self) wSelf = self;
    if ([self.statefulDelegate
         respondsToSelector:
         @selector(statefulTableViewControllerWillBeginInitialLoad:
                   completion:)]) {
             [self.statefulDelegate
              statefulTableViewControllerWillBeginInitialLoad:
...
- (void)setHasFinishedInitialLoad:(BOOL)tableIsEmpty
    ...
    // We will only show the error page if the table is empty or there is an error
    // and the table is
    // empty.
    if (tableIsEmpty) {
        [self setStatefulState:
        ...
        [self setStatefulState:BBQTableViewControllerStateIdle];
    }
}
...
- (UIView *)viewForInitialLoad {
    if ([self.statefulDelegate
         respondsToSelector:
         @selector(statefulTableViewControllerViewForInitialLoad:)]) {
        return [self.statefulDelegate
                statefulTableViewControllerViewForInitialLoad:self];
    } else {
        ...
        activityIndicatorView.frame = ({
            ...
            f.origin.x = self.staticContainerView.frame.size.width * 0.5f -
            f.size.width * 0.5f;
            f.origin.y = self.staticContainerView.frame.size.height * 0.5f -
            f.size.height * 0.5f;
            f;
        });
...
- (UIView *)viewForEmptyInitialLoadWithError:(NSError *)errorOrNil {
    if ([self.statefulDelegate
         respondsToSelector:@selector(statefulTableViewController:
                                      viewForEmptyInitialLoadWithError:)]) {
             return [self.statefulDelegate statefulTableViewController:self
                                      viewForEmptyInitialLoadWithError:errorOrNil];
         } else {
             UIView *container =
             [[UIView alloc] initWithFrame:({
                 CGRect f = CGRectMake(
                                       0.f, 0.f,
                                       self.staticContainerView.bounds.size.width,
                                       120.f);
                 f.origin.y =
                 self.staticContainerView.bounds.size.height *
                 0.5f -
                 f.size.height * 0.5f;
                 f;
             })];
             ...
             label.text =
             errorOrNil ? errorOrNil.localizedDescription : @"No records found.";
             [label sizeToFit];
             label.frame = ({
                 CGRect f = label.frame;
                 f.origin.x =
                 container.bounds.size.width * 0.5f - label.bounds.size.width * 0.5f;
                 f;
             });
...
- (BOOL)triggerPullToRefresh {
    ...
    // We don't want to change the view mode since pulling may come from the
    // static view mode as well.
    [self setStatefulState:
     BBQTableViewControllerStateLoadingFromPullToRefresh
            updateViewMode:NO
                     error:nil];
    ...
    if ([self.statefulDelegate
         ...
         @selector(
             ...
             [self.statefulDelegate
              statefulTableViewControllerWillBeginLoadingFromPullToRefresh:
              self completion:^(BOOL tableIsEmpty, NSError *errorOrNil) {
                  [wSelf setHasFinishedLoadingFromPullToRefresh:tableIsEmpty
...
- (void)triggerLoadMore {
    ...
    [self setStatefulState:BBQTableViewControllerStateLoadingMore];
    
    __weak typeof(self) wSelf = self;
    if ([self.statefulDelegate
         respondsToSelector:
         @selector(statefulTableViewControllerWillBeginLoadingMore:
                   completion:)]) {
             [self.statefulDelegate
              statefulTableViewControllerWillBeginLoadingMore:
              self completion:^(BOOL canLoadMore, NSError *errorOrNil,
                                BOOL showErrorView) {
...
- (void)updateLoadMoreView {
    if (self.watchForLoadMore) {
        UIView *loadMoreView =
        [self viewForLoadingMoreWithError:self.loadMoreViewIsErrorView
         ? self.lastLoadMoreError
                                         : nil];
...
- (UIView *)viewForLoadingMoreWithError:(NSError *)error {
    if ([self.statefulDelegate
         respondsToSelector:@selector(statefulTableViewController:
                                      viewForLoadMoreWithError:)]) {
             return [self.statefulDelegate statefulTableViewController:self
                                              viewForLoadMoreWithError:error];
         } else {
             UIView *container = [[UIView alloc]
                                  initWithFrame:CGRectMake(0.f, 0.f, self.tableView.bounds.size.width,
                                                           44.f)];
             if (error) {
                 UILabel *label = [[UILabel alloc] init];
...
- (void)updateIdleOrEmptyOrInitialLoadState:(BOOL)tableIsEmpty
                                  withError:(NSError *)errorOrNil {
    if (self.statefulState != BBQTableViewControllerStateIdle &&
        self.statefulState !=
        BBQTableViewControllerStateEmptyOrInitialLoadError) {
        return;
    }
...
- (void)setViewMode:(BBQTableViewControllerViewMode)mode {
    BOOL hidden = mode == BBQTableViewControllerViewModeTable;
...
- (void)resetStaticContentViewWithChildView:(UIView *)childView {
    [self.staticContainerView.subviews
