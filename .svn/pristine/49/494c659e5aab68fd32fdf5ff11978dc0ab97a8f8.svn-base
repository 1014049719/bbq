...
#define PULL_AND_SET(type, selector)                                           \
  ...
  if (strcmp(argType, @encode(id)) == 0 ||
    ...
    NSCAssert(valueSize == argSize, @"Value size does not match argument size "
                                    @"in -rac_setArgument: %@ atIndex: %lu",
...
- (RACTuple *)rac_argumentsTuple {
  ...
  NSMutableArray *argumentsArray =
      [NSMutableArray arrayWithCapacity:numberOfArguments - 2];
  for (NSUInteger index = 2; index < numberOfArguments; index++) {
    [argumentsArray
        addObject:[self rac_argumentAtIndex:index] ?: RACTupleNil.tupleNil];
  }
...
- (void)setRac_argumentsTuple:(RACTuple *)arguments {
  NSCAssert(arguments.count == self.methodSignature.numberOfArguments - 2,
            @"Number of supplied arguments (%lu), does not match the number "
            @"expected by the signature (%lu)",
            (unsigned long)arguments.count,
            (unsigned long)self.methodSignature.numberOfArguments - 2);

  NSUInteger index = 2;
  for (id arg in arguments) {
    [self rac_setArgument:(arg == RACTupleNil.tupleNil ? nil : arg)
                  atIndex:index];
    index++;
  }
}
