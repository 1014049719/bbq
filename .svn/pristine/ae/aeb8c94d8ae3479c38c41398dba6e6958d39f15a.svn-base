...
- (void)viewDidLoad {
  [super viewDidLoad];
  [self.collectionView registerClass:[MWGridCell class]
...
- (void)adjustOffsetsAsRequired {
  ...
  if (_browser.numberOfPhotos > 0) {
    NSIndexPath *currentPhotoIndexPath =
        [NSIndexPath indexPathForItem:_browser.currentIndex inSection:0];
    ...
    if (!currentVisible) {
      [self.collectionView
          scrollToItemAtIndexPath:currentPhotoIndexPath
...
- (void)performLayout {
  ...
  self.collectionView.contentInset = UIEdgeInsetsMake(
      navBar.frame.origin.y + navBar.frame.size.height + [self getGutter], 0, 0,
      0);
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  CGFloat value = floorf(
      ((self.view.bounds.size.width - (columns - 1) * gutter - 2 * margin) /
       columns));
