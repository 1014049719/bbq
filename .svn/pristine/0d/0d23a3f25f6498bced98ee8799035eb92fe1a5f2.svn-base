...
/* vDSP kernel */
static float __f_edgedetect_kernel_3x3[9] = {-1.0f, -1.0f, -1.0f, -1.0f, 8.0f,
                                             -1.0f, -1.0f, -1.0f, -1.0f};

#pragma mark - Emboss kernels
/* vImage kernel */
static int16_t __s_emboss_kernel_3x3[9] = {-2, 0, 0, 0, 1, 0, 0, 0, 2};

#pragma mark - Sharpen kernels
/* vImage kernel */
static int16_t __s_sharpen_kernel_3x3[9] = {-1, -1, -1, -1, 9, -1, -1, -1, -1};

#pragma mark - Unsharpen kernels
/* vImage kernel */
static int16_t __s_unsharpen_kernel_3x3[9] = {-1, -1, -1, -1, 17,
                                              -1, -1, -1, -1};
...
- (UIImage *)brightenWithValue:(float)value {
  ...
  CGContextRef bmContext = NYXCreateARGBBitmapContext(
      width, height, width * kNyxNumberOfComponentsPerARBGPixel,
  ...
  float min = (float)kNyxMinPixelComponentValue,
        max = (float)kNyxMaxPixelComponentValue;
...
- (UIImage *)contrastAdjustmentWithValue:(float)value {
  ...
  const float factor =
      (259.0f * (value + 255.0f)) / (255.0f * (259.0f - value));
...
#pragma unused(bias)
  ...
  CGContextRef bmContext = NYXCreateARGBBitmapContext(
      width, height, bytesPerRow, NYXImageHasAlpha(self.CGImage));
  if (!bmContext)
    return nil;
  ...
  /*if ((&vImageConvolveWithBias_ARGB8888))
  {
          ...
          vImageConvolveWithBias_ARGB8888(&src, &dest, NULL, 0, 0,
  __s_edgedetect_kernel_3x3, 3, 3, 1, bias, NULL, kvImageCopyInPlace);

          CGDataProviderRef dp = CGDataProviderCreateWithData(NULL, data, n,
  NULL);

          CGColorSpaceRef cs = CGColorSpaceCreateDeviceRGB();
          CGImageRef edgedImageRef = CGImageCreate(width, height, 8, 32,
  bytesPerRow, cs, kCGBitmapByteOrderDefault | kCGImageAlphaNoneSkipFirst, dp,
  ...
  /// Red components
  vDSP_vfltu8(data + 1, 4, dataAsFloat, 1, pixelsCount);
  vDSP_f3x3(dataAsFloat, height, width, __f_edgedetect_kernel_3x3,
            resultAsFloat);
...
- (UIImage *)embossWithBias:(NSInteger)bias {
  ...
  vImage_Buffer dest = {outt, height, width, bytesPerRow};
  vImageConvolveWithBias_ARGB8888(&src, &dest, NULL, 0, 0,
                                  __s_emboss_kernel_3x3, 3, 3, 1 /*divisor*/,
                                  bias, NULL, kvImageCopyInPlace);
...
- (UIImage *)gammaCorrectionWithValue:(float)value {
  const size_t width = (size_t)self.size.width;
  const size_t height = (size_t)self.size.height;
  /// Number of bytes per row, each pixel in the bitmap will be represented by 4
  /// bytes (ARGB), 8 bits of alpha/red/green/blue
  const size_t bytesPerRow = width * kNyxNumberOfComponentsPerARBGPixel;
...
- (UIImage *)grayscale {
  /* const UInt8 luminance = (red * 0.2126) + (green * 0.7152) + (blue *
   * 0.0722); // Good luminance value */
  ...
  CGContextRef bmContext =
      CGBitmapContextCreate(NULL, width, height, 8 /*Bits per component*/,
                            width * kNyxNumberOfComponentsPerGreyPixel,
...
- (UIImage *)invert {
  ...
  vDSP_vsmsa(dataAsFloat, 1, &__negativeMultiplier, &max, dataAsFloat, 1,
             pixelsCount);
  vDSP_vclip(dataAsFloat, 1, &min, &max, dataAsFloat, 1, pixelsCount);
...
- (UIImage *)opacity:(float)value {
  ...
  const size_t height = (size_t)self.size.height;
  CGContextRef bmContext = NYXCreateARGBBitmapContext(
      width, height, width * kNyxNumberOfComponentsPerARBGPixel, YES);
  if (!bmContext)
    return nil;
...
- (UIImage *)sepia {
  if ([CIImage class]) {
    /// The sepia output from Core Image is nicer than manual method and 1.6x
    /// faster than vDSP
    ...
    CIImage *output =
        [CIFilter filterWithName:@"CISepiaTone"
            ...
            .outputImage;
    CGImageRef cgImage =
        [NYXGetCIContext() createCGImage:output fromRect:[output extent]];
    ...
    free(reds), free(greens), free(blues), free(tmpRed), free(tmpGreen),
        free(tmpBlue), free(finalRed), free(finalGreen), free(finalBlue);
    CGContextRelease(bmContext);
...
- (UIImage *)sharpenWithBias:(NSInteger)bias {
  ...
  vImage_Buffer dest = {outt, height, width, bytesPerRow};
  vImageConvolveWithBias_ARGB8888(&src, &dest, NULL, 0, 0,
                                  __s_sharpen_kernel_3x3, 3, 3, 1 /*divisor*/,
                                  bias, NULL, kvImageCopyInPlace);
...
- (UIImage *)unsharpenWithBias:(NSInteger)bias {
  ...
  vImage_Buffer dest = {outt, height, width, bytesPerRow};
  vImageConvolveWithBias_ARGB8888(&src, &dest, NULL, 0, 0,
                                  __s_unsharpen_kernel_3x3, 3, 3, 9 /*divisor*/,
                                  bias, NULL, kvImageCopyInPlace);
