...
- (void)jumpToNewUploadVC {
    NSString *newGuidekey = [NSString stringWithFormat:@"%@ShowedGuide",TheCurUser.member.uid];
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"firstLaunch"]) {
        ...
        vc.showNewGuiderBlock = ^(){
            if (![[NSUserDefaults standardUserDefaults] boolForKey:newGuidekey]){
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:newGuidekey];
                [self showNewGuideController];
            }
        };
