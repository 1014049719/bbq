...
- (void)setDelegate:(id<NYXProgressiveImageViewDelegate>)delegate {
  if (delegate != _delegate) {
    _delegate = delegate;
    _delegateFlags.delegateImageDidLoadWithImage = (unsigned)
        [delegate respondsToSelector:@selector(imageDidLoadWithImage:)];
    _delegateFlags.delegateImageDownloadCompletedWithImage = (unsigned)[delegate
        respondsToSelector:@selector(imageDownloadCompletedWithImage:)];
    _delegateFlags.delegateImageDownloadFailedWithData = (unsigned)
        [delegate respondsToSelector:@selector(imageDownloadFailedWithData:)];
  }
}
...
- (void)loadImageAtURL:(NSURL *)url {
  ...
  if (_caching) {
    ...
    NSString *cachedImagePath =
        [cacheDir stringByAppendingPathComponent:[self cachedImageSystemName]];
    if ([fileManager fileExistsAtPath:cachedImagePath]) {
      NSDate *mofificationDate =
          [[fileManager attributesOfItemAtPath:cachedImagePath error:nil]
      ...
      if (-[mofificationDate timeIntervalSinceNow] > _cacheTime) {
        ...
        UIImage *localImage =
            [[UIImage alloc] initWithContentsOfFile:cachedImagePath];
  ...
  dispatch_async(_queue, ^{
    NSURLRequest *request =
        [[NSURLRequest alloc] initWithURL:url
                          ...
                          timeoutInterval:kNyxDefaultTimeoutValue];
...
+ (void)resetImageCache {
  [[NSFileManager defaultManager]
      removeItemAtPath:[NYXProgressiveImageView cacheDirectoryAddress]
                 error:nil];
}
...
- (void)connection:(__unused NSURLConnection *)connection
  ...
  CGImageSourceUpdateData(_imageSource, (__bridge CFDataRef)_dataTemp,
                          (len == _expectedSize) ? true : false);
  ...
  if (_imageHeight > 0 && _imageWidth > 0) {
    ...
    if (cgImage) {
      ...
      //{
      /// iOS 4.x fix to correctly handle JPEG images (
      /// http://www.cocoaintheshell.com/2011/05/progressive-images-download-imageio/
      /// )
      /// If the image doesn't have a transparency layer, the background is
      /// black-filled
      ...
      if (imgTmp) {
        __block UIImage *img =
            [[UIImage alloc] initWithCGImage:imgTmp
                                       scale:1.0f
    ...
    if (dic) {
      ...
      if (val) {
        int orientation; // Note: This is an EXIF int for orientation, a number
                         // between 1 and 8
        CFNumberGetValue(val, kCFNumberIntType, &orientation);
        _imageOrientation = [NYXProgressiveImageView
            exifOrientationToiOSOrientation:orientation];
      } else
...
- (void)connectionDidFinishLoading:(__unused NSURLConnection *)connection {
  if (_dataTemp) {
    ...
    if (_caching) {
      ...
      NSString *path = [cacheDir
          stringByAppendingPathComponent:[self cachedImageSystemName]];
      //[img saveToPath:path uti:CGImageSourceGetType(_imageSource)
...
- (void)connection:(__unused NSURLConnection *)connection
  didFailWithError:(__unused NSError *)error {
...
- (CGImageRef)createTransitoryImage:(CGImageRef)partialImage {
  ...
  CGContextRef bmContext = NYXCreateARGBBitmapContext(
      (size_t)_imageWidth, (size_t)_imageHeight, (size_t)_imageWidth * 4,
      NYXImageHasAlpha(partialImage));
  if (!bmContext)
    return NULL;
...
+ (NSString *)cacheDirectoryAddress {
  ...
  return [documentsDirectoryPath
      stringByAppendingPathComponent:@"NYXProgressiveImageViewCache"];
}
...
- (void)resetCache {
  [[[NSFileManager alloc] init]
      removeItemAtPath:
          [[NYXProgressiveImageView cacheDirectoryAddress]
              stringByAppendingPathComponent:[self cachedImageSystemName]]
                 error:nil];
}
