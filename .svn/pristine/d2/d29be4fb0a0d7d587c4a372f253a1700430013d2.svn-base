...
- (void)doBorderWidth:(CGFloat)width
                color:(UIColor *)color
...
- (void)addBadgePoint:(NSInteger)pointRadius
         withPosition:(BadgePositionType)type {
    ...
    switch (type) {
        ...
        case BadgePositionTypeMiddle:
            badgeView.frame = CGRectMake(0, self.frame.size.height / 2 - pointRadius,
                                         2 * pointRadius, 2 * pointRadius);
            break;
            
        default:
            badgeView.frame = CGRectMake(self.frame.size.width - 2 * pointRadius, 0,
                                         2 * pointRadius, 2 * pointRadius);
            break;
    }
...
- (void)addGradientLayerWithColors:(NSArray *)cgColorArray {
    [self addGradientLayerWithColors:cgColorArray
                           locations:nil
...
+ (UIView *)lineViewWithPointYY:(CGFloat)pointY {
    return [self lineViewWithPointYY:pointY
                            andColor:[UIColor colorWithHexString:@"0xc8c7cc"]];
}
...
+ (UIView *)lineViewWithPointYY:(CGFloat)pointY
                   ...
                   andLeftSpace:(CGFloat)leftSpace {
    UIView *lineView =
    [[UIView alloc] initWithFrame:CGRectMake(leftSpace, pointY,
                                             kScreen_Width - leftSpace, 0.5)];
...
+ (void)outputTreeInView:(UIView *)view withSeparatorCount:(NSInteger)count {
    NSString *outputStr = @"";
    outputStr =
    [outputStr stringByReplacingCharactersInRange:NSMakeRange(0, count)
                                       withString:@"-"];
...
- (void)addLineUp:(BOOL)hasUp andDown:(BOOL)hasDown andColor:(UIColor *)color {
    ...
    if (hasDown) {
        UIView *downView =
        [UIView lineViewWithPointYY:CGRectGetMaxY(self.bounds) - 0.5
                           andColor:color];
...
- (void)addLineUp:(BOOL)hasUp
    ...
    if (hasUp) {
        UIView *upView =
        [UIView lineViewWithPointYY:0 andColor:color andLeftSpace:leftSpace];
        upView.tag = kTagLineView;
        [self addSubview:upView];
    }
    if (hasDown) {
...
- (void)addRoundingCorners:(UIRectCorner)corners
    ...
    maskLayer.rasterizationScale = kScreenScale;
...
- (void)setBlankPageView:(EaseBlankPageView *)blankPageView {
    ...
    objc_setAssociatedObject(self, &BlankPageViewKey, blankPageView,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)configBlankPage:(EaseBlankPageType)blankPageType
    ...
    if (hasData) {
        ...
        if (!self.blankPageView) {
            self.blankPageView =
            [[EaseBlankPageView alloc] initWithFrame:self.bounds];
        }
        self.blankPageView.hidden = NO;
        [self.blankPageContainer addSubview:self.blankPageView];
        
        //        [self.blankPageContainer insertSubview:self.blankPageView
        //        atIndex:0];
        //        [self.blankPageView mas_makeConstraints:^(MASConstraintMaker
        //        *make) {
...
@property(nonatomic, assign) CGFloat loopAngle, monkeyAlpha, angleStep,
alphaStep;
@end
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _loopView = [[UIImageView alloc]
                     initWithImage:[UIImage imageNamed:@"loading_loop"]];
        _monkeyView = [[UIImageView alloc]
                       initWithImage:[UIImage imageNamed:@"loading_panda"]];
...
- (void)loadingAnimation {
    ...
    [UIView animateWithDuration:duration
                     ...
                     animations:^{
                         CGAffineTransform loopAngleTransform =
                         CGAffineTransformMakeRotation(_loopAngle * (M_PI / 180.0f));
...
- (void)configWithType:(EaseBlankPageType)blankPageType
    ...
    if (hasError) {
        ...
        if (!_reloadButton) {
            ...
            [_reloadButton setImage:[UIImage imageNamed:@"blankpage_button_reload"]
        ...
        [_monkeyView setImage:[UIImage imageNamed:@"error_failed"]];
