...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        CGFloat weekdayWidth = self.width / 7.0;
        CGFloat weekdayHeight = _calendar.preferedWeekdayHeight;
        CGFloat weekdayMargin = weekdayHeight * 0.1;
        CGFloat titleWidth = _contentView.width;
        
        [_weekdayLabels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger index, BOOL *stop) { \
            label.frame = CGRectMake(index*weekdayWidth, _contentView.height-weekdayHeight-weekdayMargin, weekdayWidth, weekdayHeight);
        }];
        ...
        _separator.frame = CGRectMake(0, _contentView.height-weekdayHeight-weekdayMargin*2, _contentView.width, 1.0);
        _titleLabel.frame = CGRectMake(0, _separator.bottom-titleHeight-weekdayMargin, titleWidth,titleHeight);
