...
+ (instancetype)leftPagenatorViewWithTarget:(id)target action:(SEL)action {
  return [[self alloc] initWithArrowDirection:QBPopupMenuPagenatorDirectionLeft
                                       target:target
                                       action:action];
}

+ (instancetype)rightPagenatorViewWithTarget:(id)target action:(SEL)action {
  return [[self alloc] initWithArrowDirection:QBPopupMenuPagenatorDirectionRight
                                       target:target
                                       action:action];
}

- (instancetype)initWithArrowDirection:
                    (QBPopupMenuPagenatorDirection)arrowDirection
                                target:(id)target
                                action:(SEL)action {
  ...
  if (self) {
    ...
    UIImage *highlightedArrowImage = [self arrowImageWithSize:CGSizeMake(10, 10)
                                                    direction:arrowDirection
                                                  highlighted:YES];
    [self.button setImage:highlightedArrowImage
...
- (UIImage *)arrowImageWithSize:(CGSize)size
                      direction:(QBPopupMenuPagenatorDirection)direction
                    highlighted:(BOOL)highlighted {
  UIGraphicsBeginImageContextWithOptions(size, NO, 0);

  // Draw arrow
  [self drawArrowInRect:CGRectMake(0, 0, size.width, size.height)
              direction:direction
