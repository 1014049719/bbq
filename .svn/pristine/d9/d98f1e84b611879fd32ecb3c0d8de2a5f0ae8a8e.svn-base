...
- (void)startQueueWithName:(NSString *)queueId {
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(netWorkUpdate)
             name:SlateReachabilityDidChangeNotification
           object:nil];
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(saveUncompletedTask)
...
- (void)enumerateUndoneTasksWithBlock:
    (void (^)(SlateOfflineMessageQueueTask *task, NSUInteger idx,
              BOOL *stop))block {
  ...
  if (taskArray) {
    for (id obj in [taskArray copy]) {
      if ([obj isKindOfClass:[SlateOfflineMessageQueueTask class]]) {
        SlateOfflineMessageQueueTask *task =
            (SlateOfflineMessageQueueTask *)obj;
...
- (NSDictionary *)taskDictionaryWithTask:(SlateOfflineMessageQueueTask *)task {
  if (task.identifier) {
    ...
    if ([task
            respondsToSelector:@selector(serializingObjectWithTaskUserInfo:)]) {
      userInfo = [task serializingObjectWithTaskUserInfo:task.userInfo];
    }
    return @{
      task.
      identifier :
          @{@"userInfo" : userInfo, @"class" : NSStringFromClass([task class])}
    };
  }
  return nil;
}
        ...
        if (userInfo && className) {
          ...
          if ([task
                  respondsToSelector:@selector(
                                         taskUserInfoWithSerializedObject:)]) {
...
- (void)saveUncompletedTask {
  if (taskArray) {
    ...
    [array writeToFile:[self tasksfilePathWithQueueId:currentQueueId]
            atomically:YES];
  }
}
...
- (NSString *)tasksfilePathWithQueueId:(NSString *)queueId {
  ...
  // 创建父目录
  NSString *fatherPath =
      [cacheDirectoryPath stringByAppendingFormat:@"/OfflineMessageQueue"];

  if (![fileManger fileExistsAtPath:fatherPath]) {
    [fileManger createDirectoryAtPath:fatherPath
