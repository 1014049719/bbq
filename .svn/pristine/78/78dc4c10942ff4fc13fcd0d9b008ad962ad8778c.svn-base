...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    NSLog(@"视频地址。。。。%@",[info objectForKey:UIImagePickerControllerMediaURL]);
    
    NSDictionary *opts = [NSDictionary dictionaryWithObject:[NSNumber numberWithBool:NO]
                                                     forKey:AVURLAssetPreferPreciseDurationAndTimingKey];
    AVURLAsset *urlAsset = [AVURLAsset URLAssetWithURL:[info objectForKey:UIImagePickerControllerMediaURL] options:opts];
    float second = 0;
    second = urlAsset.duration.value/urlAsset.duration.timescale;
    NSLog(@"视频时长为。。。。%f",second);
    if (second<3) {
        
        [picker.presentingViewController dismissViewControllerAnimated:YES completion:NULL];
        [SVProgressHUD showErrorWithStatus:@"视频不能少于3秒,请重拍"];
        return;
    };
    
    //传值 拍摄视频相关信息
    NSDictionary *dic=@{@"Video_image":[self getImage:[info objectForKey:UIImagePickerControllerMediaURL]],@"Video_Url":[info objectForKey:UIImagePickerControllerMediaURL]};
    //拍摄视频完成 发布通知
    [[NSNotificationCenter defaultCenter] postNotificationName:@"TakeVideo_completed" object:nil userInfo:dic];
