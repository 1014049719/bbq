...
+ (void)executePostWithUrl:(NSString *)url andParameters:(NSDictionary *)parameters andHeaders:(NSDictionary *)headers constructingBodyWithBlock:(void (^)(id <AFMultipartFormData> formData))block withSuccessHandler:(void (^)(AFHTTPRequestOperation *operation, id responseObject, bool apiSuccess))success withErrorHandler:(void (^)(NSDictionary *responseObject))error withFailureHandler:(void (^)(AFHTTPRequestOperation *operation, NSError *error))failure withLoadingViewOn:(UIView *)parentView {
    ...
    AFHTTPRequestOperation *operation;
    if (parentView == nil) {
        operation = [manager POST:url parameters:parameters constructingBodyWithBlock:block success:^(AFHTTPRequestOperation *operation, NSDictionary *responseObject) {
            if ([operation isCancelled]){
            ...
            if(![operation isEqual:nil] & ![error isEqual:nil]){
                if (failure) {
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)([self displayDurationForString:error.localizedDescription] * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        failure(operation, error);
                    });
                }
            }
...
+ (void)executePostWithUrl:(NSString *)url andParameters:(NSDictionary *)parameters andHeaders:(NSDictionary *)headers withSuccessHandler:(void (^)(AFHTTPRequestOperation *operation, NSDictionary *responseObject, bool apiSuccess))success withErrorHandler:(void (^)(NSDictionary *responseObject))error withFailureHandler:(void (^)(AFHTTPRequestOperation *operation, NSError *error))failure withLoadingViewOn:(UIView *)parentView {
    ...
    AFHTTPRequestOperation *operation;
    if (parentView == nil) {
        operation = [manager POST:url parameters:parameters success:^(AFHTTPRequestOperation *operation, NSDictionary *responseObject) {
            ...
            if ([responseObject[@"res"] integerValue] == 1) {
                ...
                //bugly
                NSError *errr = [[NSError alloc] initWithDomain:url code:[responseObject[@"res"] integerValue] userInfo:responseObject];
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    [[CrashReporter sharedInstance] reportError:errr reason:responseObject[@"msg"] extraInfo:responseObject];
                });
            ...
            //bugly
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [[CrashReporter sharedInstance] reportError:error reason:[NSString stringWithFormat:@"failure, url = %@", url] extraInfo:operation.responseObject];
            });
            if(![operation isEqual:nil] & ![error isEqual:nil]){
                if (failure) {
                    failure(operation, error);
