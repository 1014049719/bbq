...
//                view.backgroundColor = [UIColor colorWithWhite:0.5 alpha:0.5];
        ...
        [wxLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(shareWX.mas_bottom).offset(5);
        ...
        _contentView.size = CGSizeMake(kScreenWidth, contentHeight);
     ...
     completion:^(UMSocialResponseEntity *response) {
         if (response.responseCode == UMSResponseCodeSuccess) {
             ...
             if (self.shareType == BBQShareTypeDynamic){
                 [self uploadAction:@{@"actionid":@5}];
             }else if (self.shareType == BBQShareTypeCZS){
                 [self uploadAction:@{@"actionid":@6}];
             }
#endif
             NSLog(@"分享成功");
...
- (UMSocialUrlResource *)getUrlResoure {
    if (self.shareType == BBQShareTypeDynamic) {
        ...
        if (self.model.attachinfo.count > 0) {
            ...
            [[UMSocialUrlResource alloc] initWithSnsResourceType:UMSocialUrlResourceTypeImage
                                                             url:url];
...
- (void)uploadAction:(NSDictionary *)param{
    [BBQHTTPRequest
