...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        _collectionView.frame = CGRectMake(0, self.height*0.1, self.width, self.height*0.9);
        _collectionViewFlowLayout.itemSize = CGSizeMake(
                                                        _collectionView.width*((_scrollDirection==UICollectionViewScrollDirectionHorizontal)?0.5:1),
...
- (void)layoutSubviews
{
    ...
    if (self.header.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        ...
        if (self.header.scrollEnabled) {
            self.contentView.alpha = 1.0 - (1.0-self.header.appearance.headerMinimumDissolvedAlpha)*ABS(center-position)/self.width;
        } else {
            self.contentView.alpha = (position > 0 && position < self.header.width*0.75);
        }
        ...
        self.contentView.alpha = 1.0 - (1.0-self.header.appearance.headerMinimumDissolvedAlpha)*ABS(center-position)/self.height;
    }
