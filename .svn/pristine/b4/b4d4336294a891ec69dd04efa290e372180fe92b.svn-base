...
- (void)initialize
{
    ...
    [self.leftButton addTarget:self action:@selector(scrollCalendar:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.leftButton];
    
    self.rightButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.rightButton setImage:[UIImage imageNamed:@"calendar_right"] forState:UIControlStateNormal];
    [self.rightButton addTarget:self action:@selector(scrollCalendar:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)scrollCalendar:(UIButton *)button {
    ...
    if (button == self.leftButton) {
        [self.collectionView setContentOffset:CGPointMake(currentOffset.x - self.fs_width, currentOffset.y) animated:YES];
    } else {
        [self.collectionView setContentOffset:CGPointMake(currentOffset.x + self.fs_width, currentOffset.y) animated:YES];
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, CGRectGetMinY(label.frame), CGRectGetWidth(self.frame), CGRectGetHeight(label.frame)) byRoundingCorners:UIRectCornerTopRight | UIRectCornerTopLeft cornerRadii:CGSizeMake(5, 5)];
    [[self colorWithHex:@"ff6440"] setFill];
    [path fill];
    
    path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, CGRectGetMaxY(label.frame), CGRectGetWidth(self.frame), CGRectGetMaxY(self.frame) - CGRectGetMaxY(label.frame)) byRoundingCorners:UIRectCornerBottomLeft | UIRectCornerBottomRight cornerRadii:CGSizeMake(5, 5)];
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView {
    ...
    _currentMonth = [_minimumDate fs_dateByAddingMonths:scrollView.contentOffset.x/scrollView.fs_width].fs_dateByIgnoringTimeComponents;
