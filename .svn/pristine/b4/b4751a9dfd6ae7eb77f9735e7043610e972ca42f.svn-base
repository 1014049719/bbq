...
- (void)layoutTimelineStyle {
    ...
    if (_comment.groupkey.integerValue == BBQGroupkeyTypeParent && [_comment.baobaoname isNotBlank]) {
        ...
        username = [_comment.nickname isNotBlank] ? _comment.nickname : @"匿名";
    }
...
- (void)layoutDetailStyle {
    ...
    NSString *username = @"";
    if (_comment.groupkey.integerValue == BBQGroupkeyTypeParent && [_comment.baobaoname isNotBlank] && (_style != BBQDynamicLayoutStyleDetail && _style != BBQDynamicLayoutStyleSquareDetail)) {
        username = [_comment.baobaoname
                    stringByAppendingString:_comment.gxname];
    } else {
        username = [_comment.nickname isNotBlank] ? _comment.nickname : @"匿名";
    ...
    YYTextContainer *container = [YYTextContainer new];
    container.size = CGSizeMake(kScreenWidth - 85, HUGE);
    container.linePositionModifier = modifier;
    
    _textLayout = [YYTextLayout layoutWithContainer:container text:text];
    _height = MAX(70, CGFloatPixelRound([modifier heightForLineCount:_textLayout.lines.count]));
}
