...
- (instancetype)init {
  if (self = [super init]) {
    ...
    NSString *queueName =
        [[NSString alloc] initWithFormat:@"%@.%p", PINMemoryCachePrefix, self];
    _concurrentQueue = dispatch_queue_create([queueName UTF8String],
...
- (void)didObserveApocalypticNotification:(NSNotification *)notification {
  ...
  if ([[notification name]
          isEqualToString:UIApplicationDidReceiveMemoryWarningNotification]) {
    ...
    dispatch_async(_concurrentQueue, ^{
      ...
      PINMemoryCacheBlock didReceiveMemoryWarningBlock =
          strongSelf->_didReceiveMemoryWarningBlock;
      [strongSelf unlock];
    ...
    dispatch_async(_concurrentQueue, ^{
      ...
      PINMemoryCacheBlock didEnterBackgroundBlock =
          strongSelf->_didEnterBackgroundBlock;
      [strongSelf unlock];
...
- (void)trimToCostLimit:(NSUInteger)limit {
  ...
  NSArray *keysSortedByCost =
      [_costs keysSortedByValueUsingSelector:@selector(compare:)];
  [self unlock];
  ...
  for (NSString *key in
       [keysSortedByCost reverseObjectEnumerator]) { // costliest objects first
    [self removeObjectAndExecuteBlocksForKey:key];
...
- (void)trimToAgeLimitRecursively {
  ...
  dispatch_time_t time =
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(ageLimit * NSEC_PER_SEC));
...
- (void)setObject:(id)object
           forKey:(NSString *)key
            block:(PINMemoryCacheObjectBlock)block {
...
- (void)enumerateObjectsWithBlock:(PINMemoryCacheObjectBlock)block
                  completionBlock:(PINMemoryCacheBlock)completionBlock {
  __weak PINMemoryCache *weakSelf = self;
