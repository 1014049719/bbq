...
- (void)setButtons:(NSArray *)buttons animated:(BOOL)animated {
  ...
  NSMutableArray *buttonFrames =
      [NSMutableArray arrayWithCapacity:_buttons.count];

  for (RFToolbarButton *button in _buttons) {
    CGRect frame = CGRectMake(originX, 0, button.frame.size.width,
  ...
  [addButtons
      enumerateObjectsUsingBlock:^(RFToolbarButton *button, BOOL *stop) {
        button.frame = CGRectMake(originX, 0, button.frame.size.width,
                                  button.frame.size.height);
  ...
  [UIView animateWithDuration:0.2
      animations:^{
        ...
        [_buttons enumerateObjectsUsingBlock:^(RFToolbarButton *button,
