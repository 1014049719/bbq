...
// result of all applications of `block` to the remaining values in the
// receiver.
- (instancetype)bind:(RACStreamBindBlock)block
...
+ (RACSequence *)sequenceWithHeadBlock:(id (^)(void))headBlock
  ...
  return
      [[RACDynamicSequence sequenceWithHeadBlock:headBlock tailBlock:tailBlock]
...
- (instancetype)bind:(RACStreamBindBlock)bindBlock
  ...
  RACSequence *sequence = [RACDynamicSequence sequenceWithLazyDependency:^id {
    ...
    NSCAssert([current isKindOfClass:RACSequence.class],
              @"-bind: block returned an object that is not a sequence: %@",
              current);
    return nil;
  } headBlock:^(id _) {
    return current.head;
    ...
    return [valuesSeq bind:bindBlock
        passingThroughValuesFromSequence:current.tail];
  }];
...
- (instancetype)concat:(RACStream *)stream {
  ...
  return
      [[[RACArraySequence sequenceWithArray:@[ self, stream ] offset:0] flatten]
...
- (instancetype)zipWith:(RACSequence *)sequence {
  ...
  return [[RACSequence sequenceWithHeadBlock:^id {
    if (self.head == nil || sequence.head == nil)
      return nil;
    return RACTuplePack(self.head, sequence.head);
  } tailBlock:^id {
    if (self.tail == nil || [[RACSequence empty] isEqual:self.tail])
      return nil;
    if (sequence.tail == nil || [[RACSequence empty] isEqual:sequence.tail])
      return nil;
...
- (BOOL)isEqual:(RACSequence *)seq {
  if (self == seq)
    return YES;
  if (![seq isKindOfClass:RACSequence.class])
    return NO;
