...
- (void)selectDate:(NSDate *)date
      scrollToDate:(BOOL)scrollToDate
    forPlaceholder:(BOOL)forPlaceholder;
...
- (void)initialize {
  ...
  [collectionView registerClass:[FSCalendarCell class]
  ...
  view.backgroundColor =
      [[UIColor lightGrayColor] colorWithAlphaComponent:0.25];
...
- (void)layoutSubviews {
  ...
  if (_needsAdjustingViewFrame) {
    ...
    _header.frame = CGRectMake(
        0, 0, self.fs_width,
        _headerHeight == -1 ? kFSCalendarDefaultHeaderHeight : _headerHeight);
    CGFloat width = _contentView.fs_width / _weekdays.count;
    CGFloat height = kFSCalendarDefaultWeekHeight;
    [_weekdays enumerateObjectsUsingBlock:^(UILabel *weekdayLabel,
                                            NSUInteger idx, BOOL *stop) {
      NSUInteger absoluteIndex = ((idx - (_firstWeekday - 1)) + 7) % 7;
      weekdayLabel.frame =
          CGRectMake(absoluteIndex * width, _header.fs_height, width, height);
    }];
    ...
    _collectionViewLayout.sectionInset =
    ...
    if (_pagingEnabled) {
      ...
      case FSCalendarScopeMonth: {
        ...
        _daysContainer.frame =
            CGRectMake(0, (kFSCalendarDefaultWeekHeight + _header.fs_height),
                       self.fs_width, contentHeight);
        _collectionView.frame = _daysContainer.bounds;
        _collectionViewLayout.itemSize =
            CGSizeMake(_collectionView.fs_width / 7 -
      ...
      case FSCalendarScopeWeek: {
        ...
        _daysContainer.frame = CGRectMake(
            0, kFSCalendarDefaultWeekHeight + _header.fs_height, self.fs_width,
            MAX(kFSCalendarMinimumRowHeight, contentHeight));
        _collectionView.frame = _daysContainer.bounds;
        _collectionViewLayout.itemSize =
            CGSizeMake(_collectionView.fs_width / 7, _rowHeight);
        break;
      }
      default: { break; }
      }
      ...
      _collectionViewLayout.headerReferenceSize =
          CGSizeMake(_contentView.fs_width, self.preferedHeaderHeight);
  ...
  if (_needsAdjustingMonthPosition) {
    ...
    [self scrollToDate:_pagingEnabled ? _currentPage
                                      : (_currentPage ?: self.selectedDate)];
  }
...
- (void)layoutSublayersOfLayer:(CALayer *)layer {
  ...
  if (layer == self.layer) {
    if (_needsAdjustingViewFrame) {
      ...
      _maskLayer.path =
          [UIBezierPath bezierPathWithRect:(CGRect){CGPointZero, size}].CGPath;
    }
  }
}
...
- (void)prepareForInterfaceBuilder {
  ...
  [self selectDate:[NSDate fs_dateWithYear:date.fs_year
                                     month:date.fs_month
                                       day:_appearance.fakedSelectedDay ?: 1]];
}
...
- (CGSize)sizeThatFits:(CGSize)size {
  ...
  headerHeight =
      headerHeight == -1 ? kFSCalendarDefaultHeaderHeight : headerHeight;

  if (!self.floatingMode) {
    switch (_scope) {
    case FSCalendarScopeMonth: {
      CGFloat height = kFSCalendarDefaultWeekHeight + headerHeight +
                       6 * _rowHeight + kFSCalendarDefaultWeekHeight * 0.2;
      return CGSizeMake(size.width, height);
    }
    case FSCalendarScopeWeek: {
      return CGSizeMake(size.width, kFSCalendarDefaultWeekHeight +
                                        headerHeight + _rowHeight +
                                        kFSCalendarDefaultWeekHeight * 0.2);
    }
    default: { break; }
    }
  } else {
...
- (NSInteger)collectionView:(UICollectionView *)collectionView
  ...
  if (!self.floatingMode) {
    ...
    NSDate *currentPage =
        [_minimumDate.fs_firstDayOfMonth fs_dateByAddingMonths:section];
    NSDate *firstDayOfMonth = [NSDate fs_dateWithYear:currentPage.fs_year
                                                month:currentPage.fs_month
                                                  day:1];
    NSInteger numberOfRows =
        (firstDayOfMonth.fs_weekday - _calendar.firstWeekday +
         currentPage.fs_numberOfDaysInMonth) /
            7 +
        ((firstDayOfMonth.fs_weekday - _calendar.firstWeekday +
          currentPage.fs_numberOfDaysInMonth) %
             7 !=
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  switch (_scope) {
  case FSCalendarScopeMonth: {
    NSDate *month = [_minimumDate.fs_firstDayOfMonth
                        fs_dateByAddingMonths:indexPath.section]
                        .fs_dateByIgnoringTimeComponents;
    cell.dateIsPlaceholder = ![cell.date fs_isEqualToDateForMonth:month] ||
                             ![self isDateInRange:cell.date];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
  ...
  if (self.floatingMode) {
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
      FSCalendarStickyHeader *stickyHeader =
      ...
      stickyHeader.month =
          [_minimumDate fs_dateByAddingMonths:indexPath.section]
              .fs_dateByIgnoringTimeComponents.fs_firstDayOfMonth;
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  FSCalendarCell *cell =
      (FSCalendarCell *)[collectionView cellForItemAtIndexPath:indexPath];
  ...
  if (!self.floatingMode) {
    [collectionView.visibleCells
        enumerateObjectsUsingBlock:^(FSCalendarCell *cell, NSUInteger idx,
                                     BOOL *stop) {
          if (cell.dateIsPlaceholder) {
            cell.dateIsSelected = [self.selectedDates containsObject:cell.date];
            cell.dateIsToday = [cell.date fs_isEqualToDateForDay:_today];
...
- (BOOL)collectionView:(UICollectionView *)collectionView
  ...
  if ([collectionView.indexPathsForSelectedItems containsObject:indexPath] ||
      [self.selectedDates containsObject:[self dateForIndexPath:indexPath]]) {
  ...
  if (shouldSelect && cell.date && [self isDateInRange:cell.date] &&
      !_supressEvent) {
  ...
  if (!self.allowsMultipleSelection) {
    ...
    if (![indexPath isEqual:selectedIndexPath]) {
      return [self collectionView:collectionView
          shouldDeselectItemAtIndexPath:selectedIndexPath];
    }
  }
  FSCalendarCell *cell =
      (FSCalendarCell *)[collectionView cellForItemAtIndexPath:indexPath];
  return [self
      shouldDeselectDate:(cell.date ?: [self dateForIndexPath:indexPath])];
}
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if ([elementKind isEqualToString:UICollectionElementKindSectionHeader] &&
      [view isKindOfClass:[FSCalendarStickyHeader class]]) {
    [view setNeedsLayout];
  }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  if (self.floatingMode && _collectionView.indexPathsForVisibleItems.count) {
    ...
    NSMutableOrderedSet *visibleSections =
        [NSMutableOrderedSet orderedSetWithCapacity:2];
    [_collectionView.indexPathsForVisibleItems
        enumerateObjectsUsingBlock:^(NSIndexPath *indexPath, NSUInteger idx,
                                     BOOL *stop) {
          if (indexPath.item == 0) {
        ...
        [_collectionView
            layoutAttributesForItemAtIndexPath:
                [NSIndexPath indexPathForItem:0
                                    inSection:minimumSectionOnScreen]]
            .frame;
    CGFloat headerTop =
        cellFrame.origin.y - _collectionViewLayout.headerReferenceSize.height;
    CGRect headerFrame =
        CGRectMake(0, headerTop, _collectionView.fs_width,
                   _collectionViewLayout.headerReferenceSize.height);
    CGRect headerFrameOnScreen =
        [_collectionView convertRect:headerFrame toView:_daysContainer];
    if (headerFrameOnScreen.origin.y >=
        MIN(self.rowHeight * 2.5, _collectionView.fs_height * 0.5)) {
      minimumSectionOnScreen--;
    }
    NSDate *currentPage = [self.minimumDate.fs_firstDayOfMonth
        fs_dateByAddingMonths:minimumSectionOnScreen];
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView
  ...
  CGFloat pannedOffset = 0, targetOffset = 0, currentOffset = 0,
          contentSize = 0;
  switch (_collectionViewLayout.scrollDirection) {
  case UICollectionViewScrollDirectionHorizontal: {
    pannedOffset =
        [scrollView.panGestureRecognizer translationInView:scrollView].x;
  ...
  case UICollectionViewScrollDirectionVertical: {
    pannedOffset =
        [scrollView.panGestureRecognizer translationInView:scrollView].y;
  ...
  BOOL shouldTriggerPageChange =
      ((pannedOffset < 0 && targetOffset > currentOffset) ||
       (pannedOffset > 0 && targetOffset < currentOffset)) &&
      _minimumDate;
  if (shouldTriggerPageChange) {
    ...
    case FSCalendarScopeMonth: {
      _currentPage =
          [_minimumDate fs_dateByAddingMonths:targetOffset / contentSize]
              .fs_dateByIgnoringTimeComponents.fs_firstDayOfMonth;
      break;
    }
    case FSCalendarScopeWeek: {
      _currentPage =
          [_minimumDate fs_dateByAddingWeeks:targetOffset / contentSize]
              .fs_dateByIgnoringTimeComponents.fs_firstDayOfWeek;
      break;
    }
    default: { break; }
    }
...
- (void)orientationDidChange:(NSNotification *)notification {
  ...
  [self setNeedsLayout];
  [self adjustRowHeight];
  [_collectionViewLayout invalidateLayout]; // Necessary in Swift. Anyone can
                                            // tell why?
}
...
- (CGFloat)preferedHeaderHeight {
  if (_headerHeight == -1) {
    return _pagingEnabled ? kFSCalendarDefaultHeaderHeight
                          : kFSCalendarDefaultStickyHeaderHeight;
  }
...
- (void)reloadData {
  ...
  [_weekdays setValue:[UIFont systemFontOfSize:_appearance.weekdayTextSize]
               forKey:@"font"];

  CGFloat width = self.fs_width / _weekdays.count;
  CGFloat height = kFSCalendarDefaultWeekHeight;
  [_weekdays enumerateObjectsUsingBlock:^(UILabel *weekdayLabel, NSUInteger idx,
                                          BOOL *stop) {
    NSUInteger absoluteIndex = ((idx - (_firstWeekday - 1)) + 7) % 7;
    weekdayLabel.frame =
        CGRectMake(absoluteIndex * width, _header.fs_height, width, height);
...
#define m_set_scope                                                            \
  ...
  [self didChangeValueForKey:@"scope"];
    ...
    BOOL weekToMonth =
        prevScope == FSCalendarScopeWeek && scope == FSCalendarScopeMonth;
    NSInteger rowNumber = -1;

    switch (prevScope) {
    case FSCalendarScopeMonth: {
      FSCalendarCell *cell = (FSCalendarCell *)[_collectionView
    ...
    case FSCalendarScopeWeek: {
      ...
      if (rowNumber == -1) {
        NSDate *firstDayOfMonth =
            [currentPage fs_dateByAddingMonths:1].fs_firstDayOfMonth;
        NSInteger numberOfPlaceholdersForPrev =
            ((firstDayOfMonth.fs_weekday - _firstWeekday) + 7) % 7 ?: 7;
        NSDate *firstDateOfPage = [firstDayOfMonth
            fs_dateBySubtractingDays:numberOfPlaceholdersForPrev];
        for (int i = 0; i < 6; i++) {
          if ([currentPage
                  fs_isEqualToDateForDay:[firstDateOfPage
                                             fs_dateByAddingDays:7 * i]]) {
            rowNumber = i;
    ...
    void (^resizeBlock)() = ^{
      ...
      void (^transitionCompletion)() = ^{
        _maskLayer.path =
            [UIBezierPath bezierPathWithRect:(CGRect){CGPointZero, size}]
                .CGPath;
        [_maskLayer removeAllAnimations];
        _daysContainer.clipsToBounds = _collectionView.isDecelerating ||
                                       _collectionView.isTracking ||
                                       _collectionView.isDragging;
        if (!weekToMonth) {
          completion();
        }
      };
      ...
      if (animated) {
        ...
        path.toValue =
            (id)[UIBezierPath bezierPathWithRect:(CGRect){CGPointZero, size}]
                .CGPath;
        path.duration = 0.3;
        path.timingFunction = [CAMediaTimingFunction
        ...
        if (translation) {
          [_collectionView.layer addAnimation:translation
                                       forKey:@"translation"];
        }
        ...
        if (_delegate &&
            [_delegate
                respondsToSelector:@selector(
                                       calendarCurrentScopeWillChange:
                                                             animated:)]) {
...
- (void)deselectDate:(NSDate *)date {
  ...
  if ([_collectionView.indexPathsForSelectedItems containsObject:indexPath]) {
    ...
    FSCalendarCell *cell =
        (FSCalendarCell *)[_collectionView cellForItemAtIndexPath:indexPath];
    cell.dateIsSelected = NO;
    [cell setNeedsLayout];
    ...
    [_collectionView.visibleCells
        enumerateObjectsUsingBlock:^(FSCalendarCell *cell, NSUInteger idx,
                                     BOOL *stop) {
          if (cell.dateIsPlaceholder &&
              [cell.date fs_isEqualToDateForDay:date] && cell.dateIsSelected) {
            cell.dateIsSelected = NO;
            [cell setNeedsLayout];
            *stop = YES;
          }
...
- (void)selectDate:(NSDate *)date
  ...
  NSDate *targetDate =
      [date fs_daysFrom:_minimumDate] < 0 ? _minimumDate.copy : date;
  targetDate =
      [targetDate fs_daysFrom:_maximumDate] > 0 ? _maximumDate.copy : date;
  ...
  if (forPlaceholder) {
    ...
    if (shouldSelect && ![self isDateSelected:targetDate]) {
      if (_collectionView.indexPathsForSelectedItems.count &&
          self.selectedDate && !self.allowsMultipleSelection) {
        NSIndexPath *currentIndexPath =
            [self indexPathForDate:self.selectedDate];
        [_collectionView deselectItemAtIndexPath:currentIndexPath animated:YES];
        [self collectionView:_collectionView
            didDeselectItemAtIndexPath:currentIndexPath];
      }
      if ([self collectionView:_collectionView
              shouldSelectItemAtIndexPath:targetIndexPath]) {
        [_collectionView
            selectItemAtIndexPath:targetIndexPath
                         animated:YES
                   scrollPosition:UICollectionViewScrollPositionNone];
        [self collectionView:_collectionView
            didSelectItemAtIndexPath:targetIndexPath];
      }
    }
    ...
    if (self.selectedDate && !self.allowsMultipleSelection) {
      ...
      FSCalendarCell *cell = (FSCalendarCell *)
          [_collectionView cellForItemAtIndexPath:currentIndexPath];
      cell.dateIsSelected = NO;
      [cell setNeedsLayout];
      _daysContainer.clipsToBounds = NO;
      [_selectedDates removeLastObject];
    }
    [_collectionView selectItemAtIndexPath:targetIndexPath
                                  animated:YES
                            scrollPosition:UICollectionViewScrollPositionNone];

    FSCalendarCell *cell = (FSCalendarCell *)
        [_collectionView cellForItemAtIndexPath:targetIndexPath];
    _daysContainer.clipsToBounds = NO;
    [cell performSelecting];
    ...
    [_collectionView.visibleCells
        enumerateObjectsUsingBlock:^(FSCalendarCell *cell, NSUInteger idx,
                                     BOOL *stop) {
          if (cell.dateIsPlaceholder &&
              [cell.date fs_isEqualToDateForDay:targetDate] &&
              !cell.dateIsSelected) {
...
- (void)scrollToDate:(NSDate *)date animated:(BOOL)animated {
  ...
  NSDate *targetDate =
      [date fs_daysFrom:_minimumDate] < 0 ? _minimumDate : date;
  ...
  if (_pagingEnabled) {
    ...
    case UICollectionViewScrollDirectionHorizontal: {
      [_collectionView
          setContentOffset:CGPointMake(scrollOffset * _collectionView.fs_width,
                                       0)
                  animated:animated];
      break;
    }
    ...
    CGRect itemFrame =
        [_collectionView
            layoutAttributesForItemAtIndexPath:
                [NSIndexPath indexPathForItem:0 inSection:scrollOffset]]
            .frame;
    if (self.floatingMode && CGRectEqualToRect(itemFrame, CGRectZero)) {
      ...
      CGRect headerFrame = CGRectOffset(
          itemFrame, 0, -_collectionViewLayout.headerReferenceSize.height);
...
- (void)scrollToPageForDate:(NSDate *)date animated:(BOOL)animated {
  if (!_collectionView.tracking && !_collectionView.decelerating) {
    if (!self.floatingMode && [self isDateInDifferentPage:date]) {
      ...
      if (!_supressEvent &&
          !CGSizeEqualToSize(_collectionView.frame.size, CGSizeZero)) {
        _supressEvent = YES;
        [self currentPageDidChange];
        _supressEvent = NO;
      }
...
- (NSDate *)dateForIndexPath:(NSIndexPath *)indexPath {
  switch (_scope) {
  case FSCalendarScopeMonth: {
    NSDate *currentPage = [_minimumDate.fs_firstDayOfMonth
        fs_dateByAddingMonths:indexPath.section];
    NSDate *firstDayOfMonth = [NSDate fs_dateWithYear:currentPage.fs_year
                                                month:currentPage.fs_month
                                                  day:1];
    NSInteger numberOfPlaceholdersForPrev =
        ((firstDayOfMonth.fs_weekday - _firstWeekday) + 7) % 7
            ?: (7 * !self.floatingMode);
    NSDate *firstDateOfPage =
        [firstDayOfMonth fs_dateBySubtractingDays:numberOfPlaceholdersForPrev];
    switch (_collectionViewLayout.scrollDirection) {
    case UICollectionViewScrollDirectionHorizontal: {
  ...
  case FSCalendarScopeWeek: {
    NSDate *currentPage =
        [_minimumDate.fs_firstDayOfWeek fs_dateByAddingWeeks:indexPath.section];
...
- (BOOL)isDateInRange:(NSDate *)date {
  return [date fs_daysFrom:_minimumDate] >= 0 &&
         [date fs_daysFrom:_maximumDate] <= 0;
}

- (BOOL)isDateSelected:(NSDate *)date {
  return [self.selectedDates containsObject:date] ||
         [_collectionView.indexPathsForSelectedItems
             containsObject:[self indexPathForDate:date]];
}
...
- (void)adjustRowHeight {
  ...
  CGFloat contentHeight =
      self.fs_height - kFSCalendarDefaultWeekHeight - headerHeight;
  CGFloat padding = kFSCalendarDefaultWeekHeight * 0.1;
  if (!self.floatingMode) {
    switch (_scope) {
    case FSCalendarScopeMonth: {
    ...
    case FSCalendarScopeWeek: {
      _rowHeight =
          MAX((contentHeight - padding * 2) / 6, kFSCalendarMinimumRowHeight);
      break;
    }
    default: { break; }
    }
  ...
  if (!self.floatingMode) {
    ...
    if (!_header) {
      ...
      FSCalendarHeader *header =
          [[FSCalendarHeader alloc] initWithFrame:CGRectZero];
    ...
    if (_scrollEnabled) {
      if (!_deliver) {
        FSCalendarHeaderTouchDeliver *deliver =
            [[FSCalendarHeaderTouchDeliver alloc] initWithFrame:CGRectZero];
    ...
    if (!_weekdays.count) {
      ...
      UIFont *weekdayFont =
          [UIFont systemFontOfSize:_appearance.weekdayTextSize];
    ...
    _collectionViewLayout.scrollDirection =
        (UICollectionViewScrollDirection)self.scrollDirection;
...
- (void)invalidateWeekdaySymbols {
  NSArray *weekdaySymbols = _appearance.useVeryShortWeekdaySymbols
                                ? _calendar.veryShortStandaloneWeekdaySymbols
                                : _calendar.shortStandaloneWeekdaySymbols;
  [_weekdays enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger index,
                                          BOOL *stop) {
...
- (void)invalidateAppearanceForCell:(FSCalendarCell *)cell {
  ...
  cell.preferedTitleDefaultColor =
      [self preferedTitleDefaultColorForDate:cell.date];
  cell.preferedTitleSelectionColor =
      [self preferedTitleSelectionColorForDate:cell.date];
  if (cell.subtitle) {
    cell.preferedSubtitleDefaultColor =
        [self preferedSubtitleDefaultColorForDate:cell.date];
    cell.preferedSubtitleSelectionColor =
        [self preferedSubtitleSelectionColorForDate:cell.date];
  }
  if (cell.hasEvent)
    cell.preferedEventColor = [self preferedEventColorForDate:cell.date];
  cell.preferedBorderDefaultColor =
      [self preferedBorderDefaultColorForDate:cell.date];
  cell.preferedBorderSelectionColor =
      [self preferedBorderSelectionColorForDate:cell.date];
...
- (void)enqueueSelectedDate:(NSDate *)date {
  ...
  [_selectedDates
      sortUsingComparator:^NSComparisonResult(NSDate *d1, NSDate *d2) {
...
- (BOOL)shouldSelectDate:(NSDate *)date {
  if (_delegate &&
      [_delegate respondsToSelector:@selector(calendar:shouldSelectDate:)]) {
...
- (BOOL)shouldDeselectDate:(NSDate *)date {
  if (_delegate &&
      [_delegate respondsToSelector:@selector(calendar:shouldDeselectDate:)]) {
...
- (void)didDeselectDate:(NSDate *)date {
  if (_delegate &&
      [_delegate respondsToSelector:@selector(calendar:didDeselectDate:)]) {
...
- (void)currentPageDidChange {
  if (_delegate &&
      [_delegate respondsToSelector:@selector(calendarCurrentPageDidChange:)]) {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
           ...
           [_delegate
               respondsToSelector:@selector(calendarCurrentMonthDidChange:)]) {
...
- (UIColor *)preferedSelectionColorForDate:(NSDate *)date {
  if (self.delegateAppearance &&
      [self.delegateAppearance
          respondsToSelector:@selector(calendar:
                                            appearance:
                                 selectionColorForDate:)]) {
    UIColor *color = [self.delegateAppearance calendar:self
                                            appearance:self.appearance
                                 selectionColorForDate:date];
    return color;
  }
  return nil;
}
...
- (UIColor *)preferedTitleSelectionColorForDate:(NSDate *)date {
  if (self.delegateAppearance &&
      [self.delegateAppearance
          respondsToSelector:@selector(calendar:appearance:
                                 titleSelectionColorForDate:)]) {
    UIColor *color = [self.delegateAppearance calendar:self
                                            appearance:self.appearance
                            titleSelectionColorForDate:date];
    return color;
  }
  return nil;
}
...
- (UIColor *)preferedSubtitleDefaultColorForDate:(NSDate *)date {
  if (self.delegateAppearance &&
    ...
    UIColor *color = [self.delegateAppearance calendar:self
                                            appearance:self.appearance
                           subtitleDefaultColorForDate:date];
    return color;
  }
  return nil;
}
...
- (UIColor *)preferedSubtitleSelectionColorForDate:(NSDate *)date {
  if (self.delegateAppearance &&
      [self.delegateAppearance
          respondsToSelector:@selector(calendar:appearance:
                                 subtitleSelectionColorForDate:)]) {
    UIColor *color = [self.delegateAppearance calendar:self
                                            appearance:self.appearance
                         subtitleSelectionColorForDate:date];
    return color;
  }
  return nil;
}
...
- (UIColor *)preferedEventColorForDate:(NSDate *)date {
  if (self.delegateAppearance &&
      [self.delegateAppearance respondsToSelector:@selector(calendar:
                                                             appearance:
                                                      eventColorForDate:)]) {
    UIColor *color = [self.delegateAppearance calendar:self
                                            appearance:self.appearance
                                     eventColorForDate:date];
    return color;
  }
  return nil;
}
...
- (UIColor *)preferedBorderDefaultColorForDate:(NSDate *)date {
  if (self.delegateAppearance &&
      [self.delegateAppearance
          respondsToSelector:@selector(calendar:
                                                appearance:
                                 borderDefaultColorForDate:)]) {
    UIColor *color = [self.delegateAppearance calendar:self
                                            appearance:self.appearance
                             borderDefaultColorForDate:date];
    return color;
  }
  return nil;
}
...
- (UIColor *)preferedBorderSelectionColorForDate:(NSDate *)date {
  if (self.delegateAppearance &&
      [self.delegateAppearance
          respondsToSelector:@selector(calendar:appearance:
                                 borderSelectionColorForDate:)]) {
    UIColor *color = [self.delegateAppearance calendar:self
                                            appearance:self.appearance
                           borderSelectionColorForDate:date];
    return color;
  }
  return nil;
}
...
- (FSCalendarCellShape)preferedCellShapeForDate:(NSDate *)date {
  if (self.delegateAppearance &&
    ...
    FSCalendarCellShape cellShape =
        [self.delegateAppearance calendar:self
                               appearance:self.appearance
                         cellShapeForDate:date];
    return cellShape;
  }
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
  else if (self.delegateAppearance &&
           [self.delegateAppearance
               respondsToSelector:@selector(calendar:
                                            appearance:
                                      cellStyleForDate:)]) {
    FSCalendarCellShape cellShape =
        (FSCalendarCellShape)[self.delegateAppearance calendar:self
                                                    appearance:self.appearance
                                              cellStyleForDate:date];
    return cellShape;
  }
#pragma GCC diagnostic pop
...
- (NSString *)subtitleForDate:(NSDate *)date {
  if (_dataSource &&
      [_dataSource respondsToSelector:@selector(calendar:subtitleForDate:)]) {
...
- (UIImage *)imageForDate:(NSDate *)date {
  if (_dataSource &&
      [_dataSource respondsToSelector:@selector(calendar:imageForDate:)]) {
...
- (BOOL)hasEventForDate:(NSDate *)date {
  if (_dataSource &&
      [_dataSource respondsToSelector:@selector(calendar:hasEventForDate:)]) {
    return [_dataSource calendar:self hasEventForDate:date];
  }
  return _ibEditing &&
         ([@[ @3, @5, @8, @16, @20, @25 ] containsObject:@(date.fs_day)]);
}

- (NSDate *)minimumDateForCalendar {
  if (_dataSource &&
      [_dataSource respondsToSelector:@selector(minimumDateForCalendar:)]) {
...
- (NSDate *)maximumDateForCalendar {
  if (_dataSource &&
      [_dataSource respondsToSelector:@selector(maximumDateForCalendar:)]) {
