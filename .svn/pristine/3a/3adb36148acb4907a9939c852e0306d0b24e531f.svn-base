...
- (instancetype)initWithTime:(CGFloat)time
                       value:(id)value
              easingFunction:(IFTTTEasingFunction)easingFunction;
...
- (void)setValue:(id<IFTTTInterpolatable>)value atTime:(CGFloat)time {
  [self setValue:value
                  atTime:time
      withEasingFunction:IFTTTEasingFunctionLinear];
}

- (void)setValue:(id<IFTTTInterpolatable>)value
                atTime:(CGFloat)time
    withEasingFunction:(IFTTTEasingFunction)easingFunction {
  NSAssert([self canInterpolateNewValue:value], @"New value must have the same "
                                                @"interpolatable type as "
                                                @"existing keyframe values.");

  IFTTTKeyframe *newKeyframe =
      [[IFTTTKeyframe alloc] initWithTime:time
                                    value:value
...
- (id<IFTTTInterpolatable>)valueAtTime:(CGFloat)time {
  NSAssert(!self.isEmpty,
           @"At least one KeyFrame must be set before animation begins.");
  id value;
  NSUInteger indexAfter = [self indexOfKeyframeAfterTime:time];
  if (indexAfter == 0) {
    value = ((IFTTTKeyframe *)self.keyframes[0]).value;
  } else if (indexAfter < self.keyframes.count) {
    IFTTTKeyframe *keyframeBefore =
        (IFTTTKeyframe *)self.keyframes[indexAfter - 1];
    IFTTTKeyframe *keyframeAfter = (IFTTTKeyframe *)self.keyframes[indexAfter];
    CGFloat progress = [self progressFromTime:keyframeBefore.time
                                       toTime:keyframeAfter.time
                                       atTime:time
                           withEasingFunction:keyframeBefore.easingFunction];
    value = [keyframeBefore.value interpolateTo:keyframeAfter.value
...
- (CGFloat)progressFromTime:(CGFloat)fromTime
                     toTime:(CGFloat)toTime
...
- (BOOL)canInterpolateNewValue:(id)newValue {
  ...
  return (
      [newValue respondsToSelector:@selector(interpolateTo:withProgress:)] &&
