...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        CGFloat weekdayWidth = _contentView.width/_weekdays.count;
        CGFloat padding = weekdayHeight*0.1;
        
        if (_needsLayoutForWeekMode) _scope = FSCalendarScopeWeek;
        
        _header.frame = CGRectMake(0, 0, self.width, headerHeight);
        ...
        _deliver.frame = CGRectMake(_header.left, _header.top, _header.width, headerHeight+weekdayHeight);
        ...
        if (!self.floatingMode) {
            ...
            switch (_scope) {
                case FSCalendarScopeMonth: {
                    ...
                    _daysContainer.frame = CGRectMake(0, headerHeight+weekdayHeight, self.width, contentHeight);
            ...
            CGFloat contentHeight = _contentView.height;
            _daysContainer.frame = CGRectMake(0, 0, self.width, contentHeight);
            _collectionView.frame = _daysContainer.bounds;
            _collectionViewLayout.headerReferenceSize = CGSizeMake(_collectionView.width, _collectionViewLayout.headerReferenceSize.height);
            
        }
        _topBorder.frame = CGRectMake(0, -1, self.width, 1);
        _bottomBorder.frame = CGRectMake(0, self.height, self.width, 1);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.floatingMode && _collectionView.indexPathsForVisibleItems.count) {
        ...
        if (_collectionView.contentOffset.y < 0 || _collectionView.contentOffset.y > _collectionView.contentSize.height-_collectionView.height) {
            return;
        }
        NSDate *currentPage = _currentPage;
        CGPoint significantPoint = CGPointMake(_collectionView.width*0.5,MIN(self.preferedRowHeight*2.75, _collectionView.height*0.5)+_collectionView.contentOffset.y);
        ...
        switch (_collectionViewLayout.scrollDirection) {
            case UICollectionViewScrollDirectionHorizontal: {
                scrollOffset = scrollView.contentOffset.x/scrollView.width;
                break;
            }
            case UICollectionViewScrollDirectionVertical: {
                scrollOffset = scrollView.contentOffset.y/scrollView.height;
                break;
            }
            default: {
                break;
    ...
    if (_headerHeight == FSCalendarAutomaticDimension) {
        if (_preferedWeekdayHeight == FSCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                ...
                _preferedHeaderHeight = (FSCalendarStandardHeaderHeight/divider)*self.height;
            } else {
...
- (CGFloat)preferedWeekdayHeight
{
    if (_weekdayHeight == FSCalendarAutomaticDimension) {
        if (_preferedWeekdayHeight == FSCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                CGFloat divider = _scope == FSCalendarScopeMonth ? FSCalendarStandardMonthlyPageHeight : FSCalendarStandardWeeklyPageHeight;
                _preferedWeekdayHeight = (FSCalendarStandardWeekdayHeight/divider)*self.height;
            } else {
...
- (CGFloat)preferedRowHeight
{
    if (_preferedRowHeight == FSCalendarAutomaticDimension) {
        CGFloat headerHeight = self.preferedHeaderHeight;
        CGFloat weekdayHeight = self.preferedWeekdayHeight;
        CGFloat contentHeight = self.height-headerHeight-weekdayHeight;
        CGFloat padding = weekdayHeight*0.1;
        if (!self.floatingMode) {
            switch (_scope) {
                case FSCalendarScopeMonth: {
    ...
    void(^resizeBlock)() = ^{
        ...
        if (animated) {
            ...
            if (_delegate && [_delegate respondsToSelector:@selector(calendarCurrentScopeWillChange:animated:)]) {
                ...
                _bottomBorder.frame = CGRectMake(0, size.height, self.width, 1);
...
- (void)scrollToDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if (!self.floatingMode) {
        ...
        switch (_collectionViewLayout.scrollDirection) {
            case UICollectionViewScrollDirectionVertical: {
                [_collectionView setContentOffset:CGPointMake(0, scrollOffset * _collectionView.height) animated:animated];
                break;
            }
            case UICollectionViewScrollDirectionHorizontal: {
                [_collectionView setContentOffset:CGPointMake(scrollOffset * _collectionView.width, 0) animated:animated];
                break;
            }
            default: {
                break;
        ...
        if (self.hasValidateVisibleLayout) {
            ...
            CGPoint targetOffset = CGPointMake(0, MIN(headerFrame.origin.y,_collectionView.contentSize.height-_collectionView.bottom));
