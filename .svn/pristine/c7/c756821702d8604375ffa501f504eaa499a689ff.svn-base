...
static void RACSwizzleForwardInvocation(Class class) {
  ...
  Method forwardInvocationMethod =
      class_getInstanceMethod(class, forwardInvocationSEL);
...
static void RACSwizzleRespondsToSelector(Class class) {
  ...
  Method respondsToSelectorMethod =
      class_getInstanceMethod(class, respondsToSelectorSEL);
  BOOL (*originalRespondsToSelector)(id, SEL, SEL) =
      (__typeof__(originalRespondsToSelector))method_getImplementation(
...
static void RACSwizzleMethodSignatureForSelector(Class class) {
  IMP newIMP = imp_implementationWithBlock(^(id self, SEL selector) {
    ...
    if (method == NULL) {
      ...
      struct objc_super target = {
          .super_class = class_getSuperclass(class), .receiver = self,
      };
  ...
  Method methodSignatureForSelectorMethod =
...
static void RACCheckTypeEncoding(const char *typeEncoding) {
  ...
  NSCAssert(*typeEncoding < '1' || *typeEncoding > '9',
            @"unknown method return type not supported in type encoding: %s",
            typeEncoding);
  NSCAssert(strstr(typeEncoding, "(") != typeEncoding,
            @"union method return type not supported");
  NSCAssert(strstr(typeEncoding, "{") != typeEncoding,
            @"struct method return type not supported");
  NSCAssert(strstr(typeEncoding, "[") != typeEncoding,
            @"array method return type not supported");
...
static RACSignal *NSObjectRACSignalForSelector(NSObject *self, SEL selector,
  ...
  @synchronized(self) {
    ...
    subject = [[RACSubject subject]
        setNameWithFormat:@"%@ -rac_signalForSelector: %s",
                          self.rac_description, sel_getName(selector)];
    objc_setAssociatedObject(self, aliasSelector, subject,
    ...
    if (targetMethod == NULL) {
      ...
      if (!class_addMethod(class, selector, _objc_msgForward, typeEncoding)) {
        NSDictionary *userInfo = @{
          ...
          NSLocalizedRecoverySuggestionErrorKey :
              NSLocalizedString(@"Invoke -rac_signalForSelector: again to "
        ...
        return [RACSignal
            error:[NSError
                      errorWithDomain:RACSelectorSignalErrorDomain
      ...
      class_replaceMethod(class, selector, _objc_msgForward,
...
static Class RACSwizzleClass(NSObject *self) {
  ...
  const char *subclassName =
      [className stringByAppendingString:RACSubclassSuffix].UTF8String;
  Class subclass = objc_getClass(subclassName);

  if (subclass == nil) {
    subclass =
        [RACObjCRuntime createClass:subclassName inheritingFromClass:baseClass];
    if (subclass == nil)
      return nil;
