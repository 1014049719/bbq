...
- (void)setButtonTexts:(NSMutableArray *)buttonTexts
  ...
  NSAssert(buttonTexts != nil,
           @"Array containing texts to be set on buttons is nil");
  if ([category hasPrefix:@"big_"]) {
    if (([self.buttons count] - 1) == [buttonTexts count]) {
      ...
      for (NSUInteger i = 0; i < [buttonTexts count]; ++i) {
        ...
        CGFloat imageHeight = CGRectGetHeight([((UIButton *)self.buttons[i]) frame]) - 20;
        [self.buttons[i]
            setImage:[[UIImage imageNamed:buttonTexts[i]]
                         scaledToSize:CGSizeMake(imageHeight, imageHeight)
                          highQuality:YES]
      ...
      self.buttons = nil;
      self.buttons =
          [NSMutableArray arrayWithCapacity:self.rows * self.columns];
      for (NSUInteger i = 0; i < [buttonTexts count]; ++i) {
        ...
        [button setImageEdgeInsets:UIEdgeInsetsMake(
                                       10, (buttonWidth - imageHeight) / 2, 10,
                                       (buttonWidth - imageHeight) / 2)];
        [button setImage:[[UIImage imageNamed:buttonTexts[i]]
                             scaledToSize:CGSizeMake(imageHeight, imageHeight)
                              highQuality:YES]
                forState:UIControlStateNormal];
    ...
    if (([self.buttons count] - 1) == [buttonTexts count]) {
      ...
      UIButton *button =
          [self createButtonAtIndex:self.rows * self.columns - 1];
...
- (CGFloat)XMarginForButtonInColumn:(NSInteger)column {
  CGFloat padding =
      ((CGRectGetWidth(self.bounds) - self.columns * self.buttonSize.width) /
       self.columns);
...
- (CGFloat)YMarginForButtonInRow:(NSInteger)rowNumber {
  CGFloat padding =
      ((CGRectGetHeight(self.bounds) - self.rows * self.buttonSize.height) /
       self.rows);
...
- (UIButton *)createButtonAtIndex:(NSUInteger)index {
  ...
  button.titleLabel.font =
      [UIFont fontWithName:@"Apple color emoji" size:BUTTON_FONT_SIZE];
  ...
  button.frame = CGRectIntegral(CGRectMake(
      [self XMarginForButtonInColumn:column], [self YMarginForButtonInRow:row],
