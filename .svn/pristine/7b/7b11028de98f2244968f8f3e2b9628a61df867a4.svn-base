...
- (id)initWithDisposables:(NSArray *)otherDisposables {
  ...
  [otherDisposables enumerateObjectsUsingBlock:^(RACDisposable *disposable,
    ...
    // array.
    if (index == RACCompoundDisposableInlineCount - 1)
      *stop = YES;
  }];
#endif
  ...
  if (otherDisposables.count > RACCompoundDisposableInlineCount) {
    ...
    CFRange range = CFRangeMake(RACCompoundDisposableInlineCount,
                                (CFIndex)otherDisposables.count -
                                    RACCompoundDisposableInlineCount);
    CFArrayAppendArray(_disposables, (__bridge CFArrayRef)otherDisposables,
                       range);
  }
...
- (void)dispose {
  ...
  if (remainingDisposables == NULL)
    return;

  CFIndex count = CFArrayGetCount(remainingDisposables);
  CFArrayApplyFunction(remainingDisposables, CFRangeMake(0, count),
                       &disposeEach, NULL);
