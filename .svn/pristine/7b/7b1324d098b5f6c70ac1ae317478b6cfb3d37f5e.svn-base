...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    ...
    if (!self.pullToRefreshView) {
        SVPullToRefreshView *view = [[SVPullToRefreshView alloc]
                                     initWithFrame:CGRectMake(0, -SVPullToRefreshViewHeight,
    ...
    if (self.superview && newSuperview == nil) {
        ...
        if (scrollView.showsPullToRefresh) {
            if (self.isObserving) {
                ...
                // "SVPullToRefreshView's dealloc", so remove observer here
...
- (void)layoutSubviews {
    ...
    titleFrame.origin.y =
    self.bounds.size.height - (self.subtitleLabel.text ? 48 : 40);
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 5) {
        ...
        int numComponents =
        (int)CGColorGetNumberOfComponents([self.arrowColor CGColor]);
        CGFloat colors[8];
