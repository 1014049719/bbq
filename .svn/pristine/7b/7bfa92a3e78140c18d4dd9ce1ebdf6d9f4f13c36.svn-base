...
- (void)loadUnderlyingImageAndNotify {
  ...
  if (self.underlyingImage) {
    ...
    if (_photoPath) {
      ...
      [self performSelectorInBackground:@selector(loadImageFromFileAsync)
                             withObject:nil];
    } else if (_photoURL) {
      if ([_photoURL.absoluteString hasPrefix:@"/var/mobile/"]) {
        self.underlyingImage = [UIImage imageNamed:_photoURL.absoluteString];
            ...
            completed:^(UIImage *image, NSError *error,
              ...
              if (error) {
                self.underlyingImage =
                    [UIImage imageNamed:@"placeholder_black_error"];
              } else {
                self.underlyingImage = image;
              }
              [self performSelectorOnMainThread:@selector(imageLoadingComplete)
      ...
      //            serializer];
      //
      //            [op setCompletionBlockWithSuccess:^(AFHTTPRequestOperation
      //            *operation, id responseObject) {
      //                UIImage *image = responseObject;
      //                self.underlyingImage = image;
      //                [self
      //                performSelectorOnMainThread:@selector(imageLoadingComplete)
      //                withObject:nil waitUntilDone:NO];
      //            } failure:^(AFHTTPRequestOperation *operation, NSError
      //            *error) { }];
      //
      //            [op setDownloadProgressBlock:^(NSUInteger bytesRead, long
      //            long totalBytesRead, long long totalBytesExpectedToRead) {
      //                CGFloat progress =
      //                ((CGFloat)totalBytesRead)/((CGFloat)totalBytesExpectedToRead);
      //                if (self.progressUpdateBlock) {
      //                    self.progressUpdateBlock(progress);
      //                }
      //            }];
...
/*- (UIImage *)decodedImageWithImage:(UIImage *)image {
 ...
 8,
 // width * 4 will be enough because are in ARGB format, don't read from the
 image
 ...
 CGRect rect = (CGRect){CGPointZero, CGImageGetWidth(imageRef),
 CGImageGetHeight(imageRef)};
 CGContextDrawImage(context, rect, imageRef);
 CGImageRef decompressedImageRef = CGBitmapContextCreateImage(context);
 CGContextRelease(context);

 UIImage *decompressedImage = [[UIImage alloc]
 initWithCGImage:decompressedImageRef];
...
- (void)imageLoadingComplete {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:IDMPhoto_LOADING_DID_END_NOTIFICATION
                    object:self];
}
