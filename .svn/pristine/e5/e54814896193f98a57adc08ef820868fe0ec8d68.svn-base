...
- (void)addActiveExecutionSignal:(RACSignal *)signal {
  ...
  @synchronized(self) {
    ...
    NSIndexSet *indexes =
        [NSIndexSet indexSetWithIndex:_activeExecutionSignals.count];
    [self willChange:NSKeyValueChangeInsertion
        valuesAtIndexes:indexes
                 forKey:@keypath(self.activeExecutionSignals)];
...
- (void)removeActiveExecutionSignal:(RACSignal *)signal {
  ...
  @synchronized(self) {
    ...
    // consistent.
    NSIndexSet *indexes = [_activeExecutionSignals
        indexesOfObjectsPassingTest:^BOOL(RACSignal *obj, NSUInteger index,
                                          BOOL *stop) {
          return obj == signal;
        }];
...
- (id)initWithEnabled:(RACSignal *)enabledSignal
  ...
  RACSignal *newActiveExecutionSignals = [[[[
      ...
      reduceEach:^(id _, NSDictionary *change) {
        ...
        return [signals.rac_sequence
            signalWithScheduler:RACScheduler.immediateScheduler];
      }] concat] publish] autoconnect];

  _executionSignals = [[[newActiveExecutionSignals map:^(RACSignal *signal) {
  ...
  RACMulticastConnection *errorsConnection =
      [[[newActiveExecutionSignals flattenMap:^(RACSignal *signal) {
  ...
  RACSignal *immediateExecuting =
      [RACObserve(self, activeExecutionSignals) map:^(NSArray *activeSignals) {
        return @(activeSignals.count > 0);
      }];

  _executing =
      [[[[[immediateExecuting deliverOn:RACScheduler.mainThreadScheduler]
          // This is useful before the first value arrives on the main thread.
          startWith:@NO] distinctUntilChanged] replayLast]
          setNameWithFormat:@"%@ -executing", self];

  RACSignal *moreExecutionsAllowed =
      [RACSignal if:RACObserve(self, allowsConcurrentExecution)
  ...
  _immediateEnabled =
      [[RACSignal combineLatest:@[ enabledSignal, moreExecutionsAllowed ]] and
  ];

  _enabled = [[[[[self.immediateEnabled take:1]
      concat:
          [[self.immediateEnabled skip:1]
              deliverOn:
                  RACScheduler
                      .mainThreadScheduler]] distinctUntilChanged] replayLast]
...
- (RACSignal *)execute:(id)input {
  ...
  if (!enabled) {
    NSError *error = [NSError
        errorWithDomain:RACCommandErrorDomain
  ...
  NSCAssert(signal != nil,
            @"nil signal returned from signal block for value: %@", input);
