...
- (void)viewDidLoad {
    ...
    [self.baby.baobaoschooldata enumerateObjectsUsingBlock:^(BBQSchoolDataModel*  _Nonnull school, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([school.schooltype isEqualToNumber:@0]) {
            self.school = school;
            *stop = YES;
        }
    }];
