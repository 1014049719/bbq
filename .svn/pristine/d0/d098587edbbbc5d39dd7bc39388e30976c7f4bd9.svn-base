...
- (id)initWithFrame:(CGRect)frame {
  if ((self = [super initWithFrame:frame])) {
    ...
    UIImage *videoIndicatorImage =
        [UIImage imageForResourcePath:@"MWPhotoBrowser.bundle/VideoOverlay"
                               ofType:@"png"
                             inBundle:[NSBundle bundleForClass:[self class]]];
    _videoIndicator.frame = CGRectMake(
        self.bounds.size.width - videoIndicatorImage.size.width -
            VIDEO_INDICATOR_PADDING,
        self.bounds.size.height - videoIndicatorImage.size.height -
            VIDEO_INDICATOR_PADDING,
        videoIndicatorImage.size.width, videoIndicatorImage.size.height);
    _videoIndicator.image = videoIndicatorImage;
    _videoIndicator.autoresizesSubviews = UIViewAutoresizingFlexibleLeftMargin |
    ...
    [_selectedButton
        setImage:[UIImage imageForResourcePath:
    ...
    _loadingIndicator = [[DACircularProgressView alloc]
        initWithFrame:CGRectMake(0, 0, 40.0f, 40.0f)];
...
- (void)setGridController:(MWGridViewController *)gridController {
  ...
  if (_gridController.browser.customImageSelectedSmallIconName) {
    [_selectedButton
        setImage:[UIImage imageNamed:_gridController.browser
                                         .customImageSelectedSmallIconName]
        forState:UIControlStateSelected];
  }
}
...
- (void)layoutSubviews {
  ...
  _loadingIndicator.frame = CGRectMake(
      ...
      floorf((self.bounds.size.height - _loadingIndicator.frame.size.height) /
             2),
      _loadingIndicator.frame.size.width, _loadingIndicator.frame.size.height);
  _selectedButton.frame = CGRectMake(
      self.bounds.size.width - _selectedButton.frame.size.width - 0, 0,
...
- (void)selectionButtonPressed {
  ...
  [_gridController.browser setPhotoSelected:_selectedButton.selected
                                    atIndex:_index];
}
