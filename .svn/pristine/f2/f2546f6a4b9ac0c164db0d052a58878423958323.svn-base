...
#define DEFAULT_DESCRIPTION_FONT                                               \
  [UIFont fontWithName:@"HelveticaNeue-Light" size:13.0]
...
+ (instancetype)pageWithCustomViewFromNibNamed:(NSString *)nibName {
  return [self pageWithCustomViewFromNibNamed:nibName
...
+ (instancetype)pageWithCustomViewFromNibNamed:(NSString *)nibName
  ...
  EAIntroPage *newPage = [[self alloc] init];
  newPage.customView =
      [[aBundle loadNibNamed:nibName owner:newPage options:nil] firstObject];
  newPage.customView.translatesAutoresizingMaskIntoConstraints = NO;
