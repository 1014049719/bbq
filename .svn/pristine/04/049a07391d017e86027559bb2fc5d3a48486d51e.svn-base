...
- (void)initializeViewComponents {
  ...
  self.MasterScrollView.autoresizingMask =
  ...
  self.PageControl = [[UIPageControl alloc]
      initWithFrame:CGRectMake((self.frame.size.width - kPageControlWidth) / 2,
                               self.frame.size.height - 48, kPageControlWidth,
                               37)];
  self.PageControl.currentPage = 0;
  self.PageControl.enabled = NO;
  //颜色
  self.PageControl.pageIndicatorTintColor = [UIColor colorWithRed:203 / 255.0
                                                            green:204 / 255.0
                                                             blue:205 / 255.0
                                                            alpha:1];
  ...
  if ([MYIntroductionPanel runningiOS7]) {
    ...
    NSDictionary *titleAttributes =
        [NSDictionary dictionaryWithObject:kSkipButtonFont
                                    forKey:NSFontAttributeName];
    skipStringWidth =
        [skipString boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT)
    ...
    skipStringWidth = [skipString sizeWithFont:kSkipButtonFont
  ...
  self.LeftSkipButton.frame =
      CGRectMake(10, self.frame.size.height - 48, 46, 37);
  ...
  [self.LeftSkipButton addTarget:self
                          action:@selector(didPressSkipButton)
                forControlEvents:UIControlEventTouchUpInside];
  [self addSubview:self.LeftSkipButton];

  // Right Skip Button
  self.RightSkipButton = [UIButton buttonWithType:UIButtonTypeCustom];
  self.RightSkipButton.frame = CGRectMake(
      self.frame.size.width - skipStringWidth - kLeftRightSkipPadding,
      self.frame.size.height - 48, skipStringWidth, 37);
  ...
  [self.RightSkipButton addTarget:self
                           action:@selector(didPressSkipButton)
                 forControlEvents:UIControlEventTouchUpInside];
...
/**
 *  Adds the overlay view just below the master scroll view for an overlay
 *background look.
 *
...
- (void)addOverlayViewWithFrame:(CGRect)frame {
  self.BackgroundColorView =
      [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, frame.size.width,
                                               frame.size.height)];
  self.BackgroundColorView.backgroundColor = self.UserBackgroundColor;
  [self insertSubview:self.BackgroundColorView
         belowSubview:self.MasterScrollView];
}
...
- (void)addPanelsToScrollView {
  if (Panels) {
    if (Panels.count > 0) {
      ...
      NSLog(@"You must pass in panels for the introduction view to have "
            @"content. 0 panels were found");
    }
  } else {
    NSLog(@"You must pass in panels for the introduction view to have content. "
          @"The panels object was nil.");
  }
}
...
- (void)buildScrollViewLeftToRight {
  ...
  for (MYIntroductionPanel *panelView in Panels) {
    panelView.frame = CGRectMake(panelXOffset, 0, self.frame.size.width,
  ...
  [self.MasterScrollView
      setContentSize:CGSizeMake(panelXOffset, self.frame.size.height)];
...
- (void)buildScrollViewRightToLeft {
  ...
  [self.MasterScrollView
      setContentSize:CGSizeMake(panelXOffset + self.frame.size.width,
                                self.frame.size.height)];
  ...
  [self.MasterScrollView
      setContentOffset:CGPointMake(self.frame.size.width * Panels.count, 0)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
  ...
  //        _btn.frame=CGRectMake(100, 300, 40, 40);
  //        _btn.backgroundColor=[UIColor redColor];
  //        [_btn addTarget:self action:@selector(btnClick)
  //        forControlEvents:UIControlEventTouchUpInside];
  ...
  if (self.LanguageDirection == MYLanguageDirectionLeftToRight) {
    self.CurrentPanelIndex =
        scrollView.contentOffset.x / self.MasterScrollView.frame.size.width;
    ...
    if (self.CurrentPanelIndex == (Panels.count)) {
      ...
      if ([Panels[self.PageControl.currentPage]
              respondsToSelector:@selector(panelDidDisappear)]) {
        [Panels[self.PageControl.currentPage] panelDidDisappear];
      }
      if ([(id)delegate
              respondsToSelector:@selector(introduction:didFinishWithType:)]) {
      ...
      if (LastPanelIndex !=
          self.CurrentPanelIndex) { // Keeps from making the callback when just
                                    // bouncing and not actually changing pages
        if ([(id)delegate respondsToSelector:@selector(introduction:
                                                   didChangeToPanel:
                                                          withIndex:)]) {
          [delegate introduction:self
                didChangeToPanel:Panels[self.CurrentPanelIndex]
                       withIndex:self.CurrentPanelIndex];
        }

        // Trigger the panel did appear method in the
        if ([Panels[self.CurrentPanelIndex]
                respondsToSelector:@selector(panelDidAppear)]) {
    ...
    self.CurrentPanelIndex =
        ...
        self.MasterScrollView.frame.size.width;
    ...
    if (self.CurrentPanelIndex == -1) {
      ...
      if (LastPanelIndex !=
        ...
        if ([(id)delegate respondsToSelector:@selector(introduction:
          ...
          [delegate
                  introduction:self
              didChangeToPanel:Panels[Panels.count - 1 - self.CurrentPanelIndex]
                     withIndex:Panels.count - 1 - self.CurrentPanelIndex];
        }
        // Trigger the panel did appear method in the
        if ([Panels[Panels.count - 1 - self.CurrentPanelIndex]
                respondsToSelector:@selector(panelDidAppear)]) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  if (self.LanguageDirection == MYLanguageDirectionLeftToRight) {
    if (self.CurrentPanelIndex == (Panels.count - 1)) {
      self.alpha =
          ((self.MasterScrollView.frame.size.width * (float)Panels.count) -
           self.MasterScrollView.contentOffset.x) /
          self.MasterScrollView.frame.size.width;
    }
  } else if (self.LanguageDirection == MYLanguageDirectionRightToLeft) {
    if (self.CurrentPanelIndex == 0) {
      self.alpha = self.MasterScrollView.contentOffset.x /
                   self.MasterScrollView.frame.size.width;
    }
  }
}
...
- (void)animatePanelAtIndex:(NSInteger)index {
  ...
  if (Panels.count > index) {
    ...
    CGRect initialDescriptionFrame =
        [Panels[index] PanelDescriptionLabel].frame;
    CGRect initialImageFrame = [Panels[index] PanelImageView].frame;

    // Offset frames
    [[Panels[index] PanelTitleLabel]
        setFrame:CGRectMake(initialTitleFrame.origin.x + 10,
                            initialTitleFrame.origin.y,
                            initialTitleFrame.size.width,
                            initialTitleFrame.size.height)];
    [[Panels[index] PanelDescriptionLabel]
        setFrame:CGRectMake(initialDescriptionFrame.origin.x + 10,
                            initialDescriptionFrame.origin.y,
                            initialDescriptionFrame.size.width,
                            initialDescriptionFrame.size.height)];
    [[Panels[index] PanelHeaderView]
        setFrame:CGRectMake(initialHeaderFrame.origin.x,
                            initialHeaderFrame.origin.y - 10,
                            initialHeaderFrame.size.width,
                            initialHeaderFrame.size.height)];
    [[Panels[index] PanelImageView]
        setFrame:CGRectMake(initialImageFrame.origin.x,
                            initialImageFrame.origin.y + 10,
                            initialImageFrame.size.width,
                            initialImageFrame.size.height)];
        ...
        completion:^(BOOL finished) {
          ...
          [UIView animateWithDuration:0.3
                           animations:^{
                             ...
                             [[Panels[index] PanelDescriptionLabel]
                                 setFrame:initialDescriptionFrame];
                             [[Panels[index] PanelImageView] setAlpha:1];
                             [[Panels[index] PanelImageView]
                                 setFrame:initialImageFrame];
                           }];
        }];
  }
}
...
- (void)appendCloseViewAtXIndex:(CGFloat *)xIndex {
  UIView *closeView = [[UIView alloc]
      initWithFrame:CGRectMake(*xIndex, 0, self.frame.size.width, 400)];
...
- (void)changeToPanelAtIndex:(NSInteger)index {
  ...
  if (Panels && index < Panels.count && currentIndex != index) {
    // For right-to-left, PageControl index is the inverse of the panel
    // indicies.
    ...
    if ([(id)delegate respondsToSelector:@selector(introduction:
                                               didChangeToPanel:
                                                      withIndex:)]) {
      [(id)delegate introduction:self
                didChangeToPanel:Panels[index]
                       withIndex:index];
    }
  } else {
    NSLog(@"The index: %ld is out of range for Panels array[0...%lu]",
          (long)index, Panels.count - 1);
  }
}
