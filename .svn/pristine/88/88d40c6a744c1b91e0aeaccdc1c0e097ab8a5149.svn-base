...
- (id)initWithFrame:(CGRect)frame {
  if ((self = [super initWithFrame:frame])) {
    ...
    _lastUpdatedAtLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 34.0f, width, 20.0f)];
...
- (void)setState:(SSPullToRefreshViewState)state
  ...
  case SSPullToRefreshViewStateClosing: {
    self.statusLabel.text = NSLocalizedString(@"正在加载……", nil);
...
- (void)setLastUpdatedAt:(NSDate *)date
  ...
  self.lastUpdatedAtLabel.text =
      [NSString stringWithFormat:NSLocalizedString(@"Last Updated: %@", nil),
