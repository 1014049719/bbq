...
- (RACSequence *)tail {
  RACSequence *sequence =
      [self.class sequenceWithArray:self.backingArray offset:self.offset + 1];
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
  ...
  NSCAssert(index > startIndex,
            @"Final index (%lu) should be greater than start index (%lu)",
...
- (NSArray *)array {
  return [self.backingArray
      subarrayWithRange:NSMakeRange(self.offset,
                                    self.backingArray.count - self.offset)];
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p>{ name = %@, array = %@ }",
