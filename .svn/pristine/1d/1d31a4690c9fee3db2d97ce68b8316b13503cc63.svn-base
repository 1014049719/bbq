...
#define PINDiskCacheError(error)                                               \
  if (error) {                                                                 \
    NSLog(@"%@ (%d) ERROR: %@",                                                \
...
- (instancetype)init {
  @throw
      [NSException exceptionWithName:@"Must initialize with a name"
                              reason:@"PINDiskCache must be initialized with a "
                                     @"name. Call initWithName: instead."
                            userInfo:nil];
...
- (instancetype)initWithName:(NSString *)name rootPath:(NSString *)rootPath {
  ...
  if (self = [super init]) {
    _name = [name copy];
    _asyncQueue = dispatch_queue_create(
        [[NSString stringWithFormat:@"%@ Asynchronous Queue",
                                    PINDiskCachePrefix] UTF8String],
    ...
    NSString *pathComponent =
        [[NSString alloc] initWithFormat:@"%@.%@", PINDiskCachePrefix, _name];
...
- (NSString *)description {
  return [[NSString alloc]
      initWithFormat:@"%@.%@.%p", PINDiskCachePrefix, _name, self];
}
...
- (NSString *)decodedString:(NSString *)string {
  ...
  CFStringRef unescapedString =
      CFURLCreateStringByReplacingPercentEscapesUsingEncoding(
          kCFAllocatorDefault, (__bridge CFStringRef)string, CFSTR(""),
          kCFStringEncodingUTF8);
...
+ (dispatch_queue_t)sharedTrashQueue {
  ...
  dispatch_once(&predicate, ^{
    NSString *queueName =
        [[NSString alloc] initWithFormat:@"%@.trash", PINDiskCachePrefix];
    trashQueue =
...
+ (NSURL *)sharedTrashURL {
  ...
  dispatch_once(&predicate, ^{
    sharedTrashURL = [[[NSURL alloc] initFileURLWithPath:NSTemporaryDirectory()]
        URLByAppendingPathComponent:PINDiskCachePrefix
                        isDirectory:YES];

    if (![[NSFileManager defaultManager]
            fileExistsAtPath:[sharedTrashURL path]]) {
      NSError *error = nil;
...
+ (BOOL)moveItemAtURLToTrash:(NSURL *)itemURL {
  ...
  NSURL *uniqueTrashURL =
      [[PINDiskCache sharedTrashURL] URLByAppendingPathComponent:uniqueString];
...
+ (void)emptyTrash {
  ...
  dispatch_async([self sharedTrashQueue], ^{
    NSError *error = nil;
    NSArray *trashedItems = [[NSFileManager defaultManager]
          contentsOfDirectoryAtURL:[self sharedTrashURL]
    ...
    for (NSURL *trashedItemURL in trashedItems) {
      NSError *error = nil;
      [[NSFileManager defaultManager] removeItemAtURL:trashedItemURL
                                                error:&error];
      PINDiskCacheError(error);
    }
...
- (void)initializeDiskProperties {
  ...
  NSArray *keys =
      @[ NSURLContentModificationDateKey, NSURLTotalFileAllocatedSizeKey ];
  ...
  for (NSURL *fileURL in files) {
    ...
    NSDictionary *dictionary =
        [fileURL resourceValuesForKeys:keys error:&error];
    PINDiskCacheError(error);
    ...
    NSNumber *fileSize =
        [dictionary objectForKey:NSURLTotalFileAllocatedSizeKey];
    if (fileSize) {
...
- (BOOL)setFileModificationDate:(NSDate *)date forURL:(NSURL *)fileURL {
  ...
  } ofItemAtPath:[fileURL path] error:&error];
  PINDiskCacheError(error);
...
- (void)trimDiskToSize:(NSUInteger)trimByteCount {
  ...
  NSArray *keysSortedBySize =
      [_sizes keysSortedByValueUsingSelector:@selector(compare:)];

  for (NSString *key in
       [keysSortedBySize reverseObjectEnumerator]) { // largest objects first
...
- (void)trimDiskToSizeByDate:(NSUInteger)trimByteCount {
  ...
  NSArray *keysSortedByDate =
      [_dates keysSortedByValueUsingSelector:@selector(compare:)];
...
- (void)trimDiskToDate:(NSDate *)trimDate {
  ...
  for (NSString *key in keysSortedByDate) { // oldest files first
    ...
    if ([accessDate compare:trimDate] ==
        NSOrderedAscending) { // older than trim date
      [self removeFileAndExecuteBlocksForKey:key];
    } else {
      break;
    }
...
- (void)trimToAgeLimitRecursively {
  ...
  dispatch_time_t time =
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(_ageLimit * NSEC_PER_SEC));
...
- (void)lockFileAccessWhileExecutingBlock:
    (void (^)(PINDiskCache *diskCache))block {
  __weak PINDiskCache *weakSelf = self;
...
- (void)setObject:(id<NSCoding>)object
           forKey:(NSString *)key
            block:(PINDiskCacheObjectBlock)block {
  __weak PINDiskCache *weakSelf = self;
...
- (void)enumerateObjectsWithBlock:(PINDiskCacheObjectBlock)block
                  completionBlock:(PINDiskCacheBlock)completionBlock {
  __weak PINDiskCache *weakSelf = self;
...
- (void)setObject:(id<NSCoding>)object
  ...
  if (written) {
    ...
    NSError *error = nil;
    NSDictionary *values =
        [fileURL resourceValuesForKeys:@[ NSURLTotalFileAllocatedSizeKey ]
                                 error:&error];
    PINDiskCacheError(error);

    NSNumber *diskFileSize =
        [values objectForKey:NSURLTotalFileAllocatedSizeKey];
    if (diskFileSize) {
      [self->_sizes setObject:diskFileSize forKey:key];
      self.byteCount =
          self->_byteCount + [diskFileSize unsignedIntegerValue]; // atomic
    }
...
- (void)enumerateObjectsWithBlock:(PINDiskCacheObjectBlock)block {
  ...
  [self lock];
  NSArray *keysSortedByDate =
      [self->_dates keysSortedByValueUsingSelector:@selector(compare:)];
...
#if !defined(PIN_APP_EXTENSIONS) &&                                            \
    __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_4_0
  task.taskID = [[UIApplication sharedApplication]
