...
- (void)willRotateToInterfaceOrientation:
  ...
  self.indexPathForLastVisibleItem =
      [[self.collectionView indexPathsForVisibleItems] lastObject];
...
- (void)didRotateFromInterfaceOrientation:
  ...
  [self.collectionView
      scrollToItemAtIndexPath:self.indexPathForLastVisibleItem
...
- (void)updateAssets {
  ...
  [self.assetsGroup enumerateAssetsUsingBlock:^(ALAsset *result,
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  cell.showsOverlayViewWhenSelected =
      self.imagePickerController.allowsMultipleSelection;
  ...
  if (kind == UICollectionElementKindSectionFooter) {
    ...
    case QBImagePickerControllerFilterTypeNone: {
      ...
      if (numberOfPhotos == 1) {
        if (numberOfVideos == 1) {
          format = NSLocalizedStringFromTableInBundle(
              @"format_photo_and_video", @"QBImagePicker", bundle, nil);
        } else {
          format = NSLocalizedStringFromTableInBundle(
              @"format_photo_and_videos", @"QBImagePicker", bundle, nil);
        }
      } else if (numberOfVideos == 1) {
        format = NSLocalizedStringFromTableInBundle(
            @"format_photos_and_video", @"QBImagePicker", bundle, nil);
      } else {
        format = NSLocalizedStringFromTableInBundle(
            @"format_photos_and_videos", @"QBImagePicker", bundle, nil);
      }

      label.text =
          [NSString stringWithFormat:format, numberOfPhotos, numberOfVideos];
    } break;

    case QBImagePickerControllerFilterTypePhotos: {
      NSString *key =
          (numberOfPhotos == 1) ? @"format_photo" : @"format_photos";
      NSString *format = NSLocalizedStringFromTableInBundle(
          key, @"QBImagePicker", bundle, nil);
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  CGFloat width =
      (CGRectGetWidth(self.view.frame) - 2.0 * (numberOfColumns + 1)) /
      numberOfColumns;
