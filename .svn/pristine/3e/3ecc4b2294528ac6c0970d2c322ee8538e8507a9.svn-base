 ...
 */
+ (void)setValueWithModel:(NSMutableDictionary *)model
                      set:(FMResultSet *)set
               columeName:(NSString *)columeName
               columeType:(NSString *)columeType {
    if ([columeType isEqualToString:@"NSString"]) {
        ...
        [model
         setValue:[NSNumber
                   numberWithLongLong:[set longLongIntForColumn:columeName]]
         forKey:columeName];
    } else if ([columeType isEqualToString:@"BOOL"] ||
               [columeType isEqualToString:@"bool"]) {
        [model setValue:[NSNumber numberWithBool:[set boolForColumn:columeName]]
                 forKey:columeName];
    } else if ([columeType isEqualToString:@"char"]) {
        [model setValue:[NSNumber numberWithInt:[set intForColumn:columeName]]
                 forKey:columeName];
    } else if ([columeType isEqualToString:@"float"] ||
               [columeType isEqualToString:@"double"]) {
        [model setValue:[NSNumber numberWithDouble:[set doubleForColumn:columeName]]
                 forKey:columeName];
    } else if ([columeType isEqualToString:@"NSNumber"]) {
        [model
         setValue:[NSNumber numberWithLongLong:[set stringForColumn:columeName]
                   .longLongValue]
         forKey:columeName];
    } else if ([columeType isEqualToString:@"UIImage"]) {
        NSString *filename = [set stringForColumn:columeName];
        if ([JJSandBox isFileExists:[JJSandBox getPathForDocuments:filename
                                                             inDir:@"dbImages"]]) {
            UIImage *img = [UIImage
                            imageWithContentsOfFile:[JJSandBox getPathForDocuments:filename
                                                                             inDir:@"dbImages"]];
        ...
        if ([JJSandBox isFileExists:[JJSandBox getPathForDocuments:filename
                                                             inDir:@"dbData"]]) {
            NSData *data = [NSData
                            dataWithContentsOfFile:[JJSandBox getPathForDocuments:filename
                                                                            inDir:@"dbData"]];
            [model setValue:data forKey:columeName];
        }
    } else {
        NSString *modelStr = [set stringForColumn:columeName];
        [model setValue:[modelStr jsonValueDecoded] forKey:columeName];
    }
}
...
+ (void)valueForFileName:(id)value {
    ...
    if ([value isKindOfClass:[UIImage class]]) {
        NSString *filename =
        [NSString stringWithFormat:@"img%f", [date timeIntervalSince1970]];
        [UIImageJPEGRepresentation(value, 1)
         writeToFile:[JJSandBox getPathForDocuments:filename inDir:@"dbImages"]
         atomically:YES];
        value = filename;
    } else if ([value isKindOfClass:[NSData class]]) {
        NSString *filename =
        [NSString stringWithFormat:@"data%f", [date timeIntervalSince1970]];
        [value writeToFile:[JJSandBox getPathForDocuments:filename inDir:@"dbdata"]
                atomically:YES];
        value = filename;
 ...
 */
+ (NSString *)dictionaryToSQLWhereDic:(NSDictionary *)dic
 ...
 *  @param count   count
 */
+ (void)SQLString:(NSMutableString *)SQL
          AddOder:(NSString *)orderby
           offset:(int)offset
            count:(int)count {
    if (!SQL || !count) {
        return;
 ...
 *  @param model        model
 */
+ (void)createSetKey:(NSMutableString *)setKey
        andSetValues:(NSMutableArray *)setValues
    ...
    for (int i = 0; i < self.propertys.count; i++) {
        ...
        if ([value isKindOfClass:[NSNull class]] ||
            ...
            [property.orignType isEqualToString:@"BOOL"] ||
            [property.orignType isEqualToString:@"long long"]) {
            [setValues addObject:value];
        } else {
            [setValues addObject:[value modelToJSONString]];
        }
    }
    if (setKey.length > 0) {
...
+ (void)createSetKey:(NSMutableString *)setKey
        andSetValues:(NSMutableArray *)setValues
    withupdateKeyDic:(NSDictionary *)setDic {
...
+ (void)createInsertKey:(NSMutableString *)insertKey
        insertValuesStr:(NSMutableString *)insertValuesStr
    ...
    for (int i = 0; i < self.propertys.count; i++) {
        ...
        if ([value isKindOfClass:[NSNull class]] ||
            ...
            [insertValues addObject:[value modelToJSONString]];
        }
    }
    ...
    if (insertValuesStr.length > 0) {
        [insertValuesStr
         deleteCharactersInRange:NSMakeRange(insertValuesStr.length - 1, 1)];
    }
}
...
+ (void)createTable {
    ...
    @weakify(self)
    [self.dbQueue inTransactionAsync:^(FMDatabase *db, BOOL *rollback) {
        @strongify(self)
        NSString *createTableSQL =
        [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@(id INTEGER "
         @"NOT NULL PRIMARY KEY AUTOINCREMENT,%@)",
         self.jj_tableName, [self appendTableSQL]];
        ...
        FMResultSet *resultSet = [db getTableSchema:self.jj_tableName];
        while ([resultSet next]) {
            NSString *column = [resultSet stringForColumn:@"name"];
            [columns addObject:column];
        }

        NSArray *properties = [self.propertys bk_map:^id(JJFMDBProperty *obj) {
            return obj.name;
        }];
        ...
        for (NSString *column in resultArray) {
            ...
            JJFMDBProperty *property = self.propertys[index];
            NSString *proType =property.orignType;
            NSString *fieldSql = [NSString stringWithFormat:@"%@ %@", column, proType];
            NSString *sql = [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ ", self.jj_tableName, fieldSql];
    ...
    //        IF NOT EXISTS %@(id INTEGER NOT NULL PRIMARY KEY
    //        AUTOINCREMENT,%@)",self.jj_tableName,[self appendTableSQL]];
    //        [db executeUpdate:createTableSQL];
    //    }];
}

+ (void)tableAddColumn:(NSString *)column
                  type:(NSString *)type
               results:(JJFMDBSuccess)block {
    @weakify(self)
    [self.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        NSString *updateTableSQL =
        [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ %@",
         self.jj_tableName, column, type];
...
+ (void)tableDropColumn:(NSString *)column results:(JJFMDBSuccess)block {
    ...
    [self.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        @strongify(self)
        NSString *updateTableSQL =
        [NSString stringWithFormat:@"ALTER TABLE %@ DROP COLUMN %@",
         self.jj_tableName, column];
...
+ (void)readTableColumns:(JJFMDBResults)block {
    ...
    [self.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        @strongify(self)
        NSMutableString *searchSQL = [NSMutableString
                                      stringWithFormat:@"PRAGMA TABLE_INFO(%@)", self.jj_tableName];
    ...
    while ([set next]) {
        ...
        for (int i = 0; i < self.propertys.count; i++) {
            ...
            [self setValueWithModel:model
                                set:set
                         columeName:property.name
                         columeType:property.orignType];
        }
        NSObject *new = [self.class modelWithDictionary:model];
        [array addObject:new];
    }
    [set close];
...
+ (void)searchCount:(JJFMDBCount)block {
    ...
    [self.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        @strongify(self)
        NSMutableString *searchSQL = [NSMutableString
                                      stringWithFormat:@"SELECT count(*) FROM %@", self.jj_tableName];
        FMResultSet *set = [db executeQuery:searchSQL];
        int count = 0;
        if ([set next]) {
...
+ (void)searchSyncAll:(JJFMDBResults)block {
    [self searchSyncWhere:nil orderBy:nil offset:0 count:0 results:block];
}
...
+ (void)searchOrderBy:(NSString *)orderBy
                count:(int)count
              results:(JJFMDBResults)block {
...
+ (void)searchSyncAllWhere:(NSString *)where results:(JJFMDBResults)block {
    [self searchSyncWhere:where orderBy:nil offset:0 count:0 results:block];
}
#pragma mark Search Page
+ (void)searchPageNum:(int)pageNum results:(JJFMDBResults)block {
    [self searchWhere:nil
              orderBy:nil
               offset:(pageNum * DB_SEARCH_COUNT)
                count:DB_SEARCH_COUNT
              results:block];
}
...
+ (void)searchSyncWhere:(NSString *)where
                orderBy:(NSString *)orderBy
                 offset:(int)offset
                  count:(int)count
                results:(JJFMDBResults)block {
    ...
    [self.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        NSMutableString *searchSQL = [NSMutableString
                                      stringWithFormat:@"SELECT * FROM %@ ", self.jj_tableName];
...
+ (void)searchSyncWhere:(NSString *)where orderBy:(NSString *)orderBy count:(int)count results:(JJFMDBResults)block {
    [self searchSyncWhere:where orderBy:orderBy offset:0 count:count results:block];
}
...
+ (void)searchWhereDic:(NSDictionary *)whereDic
               pageNum:(int)pageNum
               results:(JJFMDBResults)block;
{
    [self searchWhereDic:whereDic
                 orderBy:nil
                  offset:(pageNum * DB_SEARCH_COUNT)
                   count:DB_SEARCH_COUNT
                 results:block];
               ...
               results:(JJFMDBResults)block;
{
    ...
    [self.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        ...
        if (whereDic != nil && whereDic.count > 0) {
            NSString *whereKey =
            [self dictionaryToSQLWhereDic:whereDic andValues:values];
...
- (void)executeInsertToDB:(FMDatabase *)db result:(JJFMDBSuccess)block {
    ...
    [self.class createInsertKey:insertKey
                insertValuesStr:insertValuesStr
                   insertValues:insertValues
                          model:self];
    
    NSString *insertSQL = [NSString
                           stringWithFormat:@"INSERT INTO %@(%@) VALUES(%@)",
                           self.class.jj_tableName, insertKey, insertValuesStr];
...
- (void)insertUpdateSyncToDB:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        BOOL isExists = [self executeIsExistsToDB:db result:nil];
        if (isExists) {
            [self executeUpdateToDB:db result:block];
...
- (void)insertUpdateSyncToDB:(NSString *)where result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        BOOL isExists = [self executeIsExistsToDB:where db:db result:nil];
...
- (void)insertUpdateToDB:(NSString *)updateKey
                   where:(NSString *)where
                  result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        @strongify(self)
        BOOL isExists = [self executeIsExistsToDB:where db:db result:nil];
...
- (void)insertUpdateSyncToDB:(NSString *)updateKey where:(NSString *)where result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        BOOL isExists = [self executeIsExistsToDB:where db:db result:nil];
...
- (void)insertUpdateToDBDic:(NSDictionary *)whereDic
                     result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        @strongify(self)
...
- (void)insertUpdateSyncToDBDic:(NSDictionary *)whereDic result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        BOOL isExists = [self executeIsExistsToDBDic:whereDic db:db result:block];
...
- (void)insertUpdateToDBDic:(NSDictionary *)updateKeyDic
    ...
    [self.class.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        ...
        BOOL isExists = [self executeIsExistsToDBDic:whereDic db:db result:block];
        if (isExists) {
            [self.class executeUpdateToDB:db
                             updateKeyDic:updateKeyDic
                                 whereDic:whereDic
                                   result:block];
        } else {
            [self executeInsertToDB:db result:block];
...
- (void)insertUpdateSyncToDBDic:(NSDictionary *)updateKeyDic whereDic:(NSDictionary *)whereDic result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        BOOL isExists = [self executeIsExistsToDBDic:whereDic db:db result:block];
...
- (void)executeUpdateToDB:(FMDatabase *)db
                    where:(NSString *)where
                   result:(JJFMDBSuccess)block {
    ...
    if (where) {
        updateSQL =
        [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE %@",
         self.class.jj_tableName, updateKey, where];
    } else {
        //通过rowid来更新数据
        if (self.primaryKey) {
            //通过primarykey来更新数据
            updateSQL = [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE %@=?",
                         self.class.jj_tableName, updateKey,
                         self.primaryKey];
            
            [updateValues addObject:[self valueForKey:self.primaryKey]];
        } else if (self.rowid > 0) {
            updateSQL = [NSString
                         stringWithFormat:@"UPDATE %@ SET %@ WHERE rowid=%lld",
                         self.class.jj_tableName, updateKey, self.rowid];
        } else {
            if (block) {
                block(NO);
            }
...
+ (void)executeUpdateToDB:(NSString *)updateKey
                       db:(FMDatabase *)db
                    where:(NSString *)where
                   result:(JJFMDBSuccess)block {
    NSString *updateSQL =
...
- (void)executeUpdateToDB:(FMDatabase *)db
    ...
    if (whereDic && whereDic.count) {
        ...
        NSString *where =
        [self.class dictionaryToSQLWhereDic:whereDic andValues:setValues];
        updateSQL =
        [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE %@",
         self.class.jj_tableName, setKey, where];
    }
...
+ (void)executeUpdateToDB:(FMDatabase *)db
             updateKeyDic:(NSDictionary *)updateKeyDic
                 whereDic:(NSDictionary *)whereDic
                   result:(JJFMDBSuccess)block {
    ...
    /** 创建updateKey 和 UpdateValues */
    [self.class createSetKey:setKey
                andSetValues:setValues
            withupdateKeyDic:updateKeyDic];
...
- (void)updateSyncToDB:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        [self executeUpdateToDB:db result:block];
...
+ (void)updateToDB:(NSString *)updateKey
             where:(NSString *)where
            result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseAsync:^(FMDatabase *db) {
...
+ (void)updateToDBDic:(NSDictionary *)updateKeyDic
             whereDic:(NSDictionary *)whereDic
               result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        @strongify(self)
        [self executeUpdateToDB:db
                   updateKeyDic:updateKeyDic
                       whereDic:whereDic
                         result:block];
    }];
    ...
    [self.class.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        ...
        if (self.primaryKey) {
            ...
            [NSString stringWithFormat:@"DELETE FROM %@ WHERE %@=?",
             self.class.jj_tableName, self.primaryKey];
            result = [db executeUpdate:deleteSQL, [self valueForKey:self.primaryKey]];
        } else if (self.rowid > 0) {
            deleteSQL =
            [NSString stringWithFormat:@"DELETE FROM %@ WHERE rowid=%lld",
             self.class.jj_tableName, self.rowid];
...
- (void)deleteSyncToDB:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        NSString *deleteSQL = nil;
...
- (void)deleteSyncToDB:(NSString *)where result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        NSString *deleteSQL =
...
- (void)deleteToDBDic:(NSDictionary *)whereDic result:(JJFMDBSuccess)block {
    ...
    [self.class.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        ...
        NSMutableArray *values = [NSMutableArray arrayWithCapacity:0];
        NSString *whereKey =
        [self.class dictionaryToSQLWhereDic:whereDic andValues:values];
        NSString *deleteSQL =
        [NSString stringWithFormat:@"DELETE FROM %@ WHERE %@",
         self.class.jj_tableName, whereKey];
...
- (void)deleteSyncToDBDic:(NSDictionary *)whereDic result:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        NSMutableArray *values = [NSMutableArray arrayWithCapacity:0];
...
- (void)clearTable:(JJFMDBSuccess)block {
    ...
    [self.class.dbQueue inDatabaseAsync:^(FMDatabase *db) {
        @strongify(self)
        NSString *delete =
        [NSString stringWithFormat:@"DELETE FROM %@", self.class.jj_tableName];
...
+ (void)clearTableSync:(JJFMDBSuccess)block {
    @weakify(self)
    [self.class.dbQueue inDatabaseNotInQueue:^(FMDatabase *db) {
        @strongify(self)
        NSString *delete =
...
- (BOOL)executeIsExistsToDB:(FMDatabase *)db result:(JJFMDBSuccess)block {
    if (!self.primaryKey && self.rowid < 1) {
        if (block) {
            block(NO);
        }
        return NO;
    ...
    NSString *where;
    if (self.primaryKey) {
        where = [NSString stringWithFormat:@"%@='%@'", self.primaryKey,
                 [self valueForKey:self.primaryKey]];
    } else if (self.rowid > 0) {
        where = [NSString stringWithFormat:@"rowid=%lld", self.rowid];
    }
...
- (BOOL)executeIsExistsToDB:(NSString *)where
                         db:(FMDatabase *)db
                     result:(JJFMDBSuccess)block {
...
- (BOOL)executeIsExistsToDBDic:(NSDictionary *)whereDic
                            db:(FMDatabase *)db
                        result:(JJFMDBSuccess)block {
    ...
    [db executeQuery:querySQL withArgumentsInArray:values];
...
+ (void)setPropertys:(NSMutableArray *)propertys {
    objc_setAssociatedObject(self, &kJJFMDBPropertysKey, propertys,
