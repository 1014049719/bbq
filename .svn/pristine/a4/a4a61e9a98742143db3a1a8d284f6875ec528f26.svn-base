...
- (BOOL)isFinished {
  return self.eventType == RACEventTypeCompleted ||
         self.eventType == RACEventTypeError;
}
...
+ (instancetype)completedEvent {
  ...
  dispatch_once(&pred, ^{
    singleton =
        [[self alloc] initWithEventType:RACEventTypeCompleted object:nil];
  });
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:@"<%@: %p>{ %@ }", self.class, self, eventDescription];
}
...
- (BOOL)isEqual:(id)event {
  if (event == self)
    return YES;
  if (![event isKindOfClass:RACEvent.class])
    return NO;
  if (self.eventType != [event eventType])
    return NO;
