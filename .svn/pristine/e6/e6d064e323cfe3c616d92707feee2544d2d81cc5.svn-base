...
+ (instancetype)sequenceWithTupleBackingArray:(NSArray *)backingArray
                                       offset:(NSUInteger)offset {
  NSCParameterAssert(offset <= backingArray.count);

  if (offset == backingArray.count)
    return self.empty;
...
- (RACSequence *)tail {
  RACSequence *sequence =
      [self.class sequenceWithTupleBackingArray:self.tupleBackingArray
...
- (NSArray *)array {
  NSRange range =
      NSMakeRange(self.offset, self.tupleBackingArray.count - self.offset);
  ...
  [self.tupleBackingArray
      enumerateObjectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:range]
                     ...
                     usingBlock:^(id object, NSUInteger index, BOOL *stop) {
                       id mappedObject =
                           (object == RACTupleNil.tupleNil ? NSNull.null
                                                           : object);
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p>{ name = %@, tuple = %@ }",
