...
- (instancetype)initWithView:(UIView *)view path:(CGPathRef)path {
  if ((self = [super init])) {
    ...
    // CAAnimations are lost when application enters the background, so re-add
    // them
...
- (void)addKeyframeForTime:(CGFloat)time
         animationProgress:(CGFloat)animationProgress {
  if (![self validAnimationProgress:animationProgress])
    return;
...
- (BOOL)validAnimationProgress:(CGFloat)animationProgress {
  NSAssert((animationProgress >= 0.f) && (animationProgress <= 1.f),
           @"Animation progress values must be between zero and one.");
  if ((animationProgress < 0.f) || (animationProgress > 1.f))
    return NO;
  return YES;
}
...
- (void)animate:(CGFloat)time {
  ...
  self.view.layer.timeOffset =
      (CFTimeInterval)[(NSNumber *)[self valueAtTime:time] doubleValue];
}
