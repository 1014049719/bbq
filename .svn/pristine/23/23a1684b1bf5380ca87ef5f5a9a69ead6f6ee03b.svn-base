...
static RACDisposable *subscribeForever(RACSignal *signal, void (^next)(id),
  ...
  RACSchedulerRecursiveBlock recursiveBlock = ^(void (^recurse)(void)) {
    RACCompoundDisposable *selfDisposable =
        [RACCompoundDisposable compoundDisposable];
  ...
  recursiveBlock(^{
    RACScheduler *recursiveScheduler =
        RACScheduler.currentScheduler ?: [RACScheduler scheduler];
...
- (RACSignal *)throttle:(NSTimeInterval)interval
      valuesPassingTest:(BOOL (^)(id next))predicate {
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    return compoundDisposable;
  }] setNameWithFormat:@"[%@] -throttle: %f valuesPassingTest:", self.name,
                       (double)interval];
}
...
- (RACSignal *)delay:(NSTimeInterval)interval {
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    void (^schedule)(dispatch_block_t) = ^(dispatch_block_t block) {
      RACScheduler *delayScheduler = RACScheduler.currentScheduler ?: scheduler;
      RACDisposable *schedulerDisposable =
          [delayScheduler afterDelay:interval schedule:block];
...
- (RACSignal *) catch:(RACSignal * (^)(NSError *error))catchBlock {
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    RACDisposable *subscriptionDisposable = [self subscribeNext:^(id x) {
      ...
      NSCAssert(signal != nil,
                @"Expected non-nil signal from catch block on %@", self);
...
- (RACSignal *) try:
  (BOOL (^)(id value, NSError **errorPtr)) tryBlock {
...
- (RACSignal *)tryMap:(id (^)(id value, NSError **errorPtr))mapBlock {
  ...
  return [[self flattenMap:^(id value) {
    ...
    return (mappedValue == nil ? [RACSignal error:error]
                               : [RACSignal return:mappedValue]);
...
- (RACSignal *)bufferWithTime:(NSTimeInterval)interval
      ...
      @synchronized(values) {
        if (values.count == 0) {
          timerDisposable.disposable =
              [scheduler afterDelay:interval schedule:flushValues];
        }
  ...
  }] setNameWithFormat:@"[%@] -bufferWithTime: %f onScheduler: %@", self.name,
                       (double)interval, scheduler];
}
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    void (^sendNext)(void) = ^{
      @synchronized(disposable) {
        if (lastSelfValue == nil || lastOtherValue == nil)
          return;
...
+ (RACSignal *)combineLatest:(id<NSFastEnumeration>)signals
  ...
  if (reduceBlock != nil)
    result = [result reduceEach:reduceBlock];

  return [result setNameWithFormat:@"+combineLatest: %@ reduce:", signals];
}

- (RACSignal *)merge:(RACSignal *)signal {
  return [[RACSignal merge:@[ self, signal ]]
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    recur = subscribeToSignal = ^(RACSignal *signal) {
      RACSerialDisposable *serialDisposable =
          [[RACSerialDisposable alloc] init];
    ...
    [compoundDisposable addDisposable:[self subscribeNext:^(RACSignal *signal) {
                          ...
                          @synchronized(subscriber) {
                            if (maxConcurrent > 0 &&
                                activeDisposables.count >= maxConcurrent) {
    ...
    return compoundDisposable;
  }] setNameWithFormat:@"[%@] -flatten: %lu", self.name,
...
- (RACSignal *)aggregateWithStartFactory:(id (^)(void))startFactory
                                  reduce:
                                      (id (^)(id running, id next))reduceBlock {
...
- (RACSignal *)aggregateWithStart:(id)start
  ...
  return [[self aggregateWithStart:start
                   ...
                   }] setNameWithFormat:@"[%@] -aggregateWithStart: %@ reduce:",
                                        self.name, [start rac_description]];
}

- (RACSignal *)aggregateWithStart:(id)start
                  reduceWithIndex:(id (^)(id, id, NSUInteger))reduceBlock {
  return
      [[[[self scanWithStart:start reduceWithIndex:reduceBlock] startWith:start]
          takeLast:1]
          setNameWithFormat:@"[%@] -aggregateWithStart: %@ reduceWithIndex:",
                            self.name, [start rac_description]];
}
...
- (RACDisposable *)setKeyPath:(NSString *)keyPath
  ...
  RACDisposable *subscriptionDisposable = [self subscribeNext:^(id x) {
    ...
    NSCAssert(
        NO, @"Received error from %@ in binding for key path \"%@\" on %@: %@",
        self, keyPath, object, error);

    // Log the error if we're running with assertions disabled.
    NSLog(@"Received error from %@ in binding for key path \"%@\" on %@: %@",
          self, keyPath, object, error);
  ...
  @synchronized(object) {
    ...
    if (bindings == nil) {
      ...
      objc_setAssociatedObject(object, bindingsKey, bindings,
  ...
  @synchronized(bindings) {
    NSCAssert(bindings[keyPath] == nil,
              @"Signal %@ is already bound to key path \"%@\" on object %@, "
              @"adding signal %@ is undefined behavior",
              [bindings[keyPath] nonretainedObjectValue], keyPath, object,
              self);
...
+ (RACSignal *)interval:(NSTimeInterval)interval
  ...
  return [[RACSignal interval:interval onScheduler:scheduler withLeeway:0.0]
      setNameWithFormat:@"+interval: %f onScheduler: %@", (double)interval,
                        scheduler];
}
...
+ (RACSignal *)interval:(NSTimeInterval)interval
  ...
  }] setNameWithFormat:@"+interval: %f onScheduler: %@ withLeeway: %f",
                       (double)interval, scheduler, (double)leeway];
}
  ...
  return [RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    if (!selfDisposable.disposed) {
      selfDisposable.disposable =
          [[self concat:[RACSignal never]] subscribe:subscriber];
    }
...
- (RACSignal *)switchToLatest {
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    RACDisposable *subscriptionDisposable =
        [[connection.signal flattenMap:^(RACSignal *x) {
          NSCAssert(x == nil || [x isKindOfClass:RACSignal.class],
                    @"-switchToLatest requires that the source signal (%@) "
                    @"send signals. Instead we got: %@",
                    self, x);
...
+ (RACSignal *) switch:(RACSignal *)signal
  ...
  return [[[signal map:^(id key) {
    ...
    if (signal == nil) {
      NSString *description = [NSString
          stringWithFormat:NSLocalizedString(
                           ...
                           key];
      return [RACSignal
          error:[NSError errorWithDomain:RACSignalErrorDomain
    ...
    return signal;
  }] switchToLatest] setNameWithFormat:@"+switch: %@ cases: %@ default: %@",
                                       signal, cases, defaultSignal];
}
...
+ (RACSignal *) if:(RACSignal *)boolSignal
  ...
  return [[[boolSignal map:^(NSNumber *value) {
    NSCAssert([value isKindOfClass:NSNumber.class],
              @"Expected %@ to send BOOLs, not %@", boolSignal, value);

    return (value.boolValue ? trueSignal : falseSignal);
  }] switchToLatest] setNameWithFormat:@"+if: %@ then: %@ else: %@", boolSignal,
...
- (id)firstOrDefault:(id)defaultValue
  ...
  condition.name =
      [NSString stringWithFormat:@"[%@] -firstOrDefault: %@ success:error:",
...
- (BOOL)waitUntilCompleted:(NSError **)error {
  ...
  [[[self ignoreValues] setNameWithFormat:@"[%@] -waitUntilCompleted:",
                                          self.name] firstOrDefault:nil
                                                            success:&success
                                                              error:error];
...
- (RACSequence *)sequence {
  return [[RACSignalSequence sequenceWithSignal:self]
      setNameWithFormat:@"[%@] -sequence", self.name];
}

- (RACMulticastConnection *)publish {
  RACSubject *subject =
      [[RACSubject subject] setNameWithFormat:@"[%@] -publish", self.name];
...
- (RACMulticastConnection *)multicast:(RACSubject *)subject {
  ...
  RACMulticastConnection *connection =
      [[RACMulticastConnection alloc] initWithSourceSignal:self
                                                   subject:subject];
  return connection;
}
...
- (RACSignal *)replayLazily {
  RACMulticastConnection *connection =
      [self multicast:[RACReplaySubject subject]];
  return [[RACSignal defer:^{
    [connection connect];
    return connection.signal;
...
- (RACSignal *)timeout:(NSTimeInterval)interval
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    RACDisposable *timeoutDisposable = [scheduler
        afterDelay:interval
          schedule:^{
            [disposable dispose];
            [subscriber
                sendError:[NSError errorWithDomain:RACSignalErrorDomain
    ...
    [disposable addDisposable:subscriptionDisposable];
    return disposable;
  }] setNameWithFormat:@"[%@] -timeout: %f onScheduler: %@", self.name,
                       (double)interval, scheduler];
}
...
- (RACSignal *)groupBy:(id<NSCopying> (^)(id object))keyBlock
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    return [self subscribeNext:^(id x) {
      ...
      [orderedGroups makeObjectsPerformSelector:@selector(sendError:)
                                     withObject:error];
    } completed:^{
      [subscriber sendCompleted];
...
- (RACSignal *)groupBy:(id<NSCopying> (^)(id object))keyBlock {
  return [[self groupBy:keyBlock transform:nil]
      setNameWithFormat:@"[%@] -groupBy:", self.name];
}
...
- (RACSignal *)retry:(NSInteger)retryCount {
  ...
  }] setNameWithFormat:@"[%@] -retry: %lu", self.name,
...
- (RACSignal *) not{
  return [[self map:^(NSNumber *value) {
    NSCAssert(
        ...
        @"-not must only be used on a signal of NSNumbers. Instead, got: %@",
        value);
...
- (RACSignal *) and {
  return [[self map:^(RACTuple *tuple) {
    NSCAssert([tuple isKindOfClass:RACTuple.class],
              @"-and must only be used on a signal of RACTuples of NSNumbers. "
              @"Instead, received: %@",
              tuple);
    NSCAssert(tuple.count > 0, @"-and must only be used on a signal of "
                               @"RACTuples of NSNumbers, with at least 1 "
                               @"value in the tuple");

    return @([tuple.rac_sequence all:^(NSNumber *number) {
      NSCAssert([number isKindOfClass:NSNumber.class],
                @"-and must only be used on a signal of RACTuples of "
                @"NSNumbers. Instead, tuple contains a non-NSNumber value: %@",
                tuple);
...
- (RACSignal *) or {
  return [[self map:^(RACTuple *tuple) {
    NSCAssert([tuple isKindOfClass:RACTuple.class],
              @"-or must only be used on a signal of RACTuples of NSNumbers. "
              @"Instead, received: %@",
              tuple);
    NSCAssert(tuple.count > 0, @"-or must only be used on a signal of "
                               @"RACTuples of NSNumbers, with at least 1 "
                               @"value in the tuple");
    ...
    return @([tuple.rac_sequence any:^(NSNumber *number) {
      NSCAssert([number isKindOfClass:NSNumber.class],
                @"-or must only be used on a signal of RACTuples of NSNumbers. "
                @"Instead, tuple contains a non-NSNumber value: %@",
                tuple);
...
- (RACSignal *)reduceApply {
  return [[self map:^(RACTuple *tuple) {
    NSCAssert([tuple isKindOfClass:RACTuple.class],
              @"-reduceApply must only be used on a signal of RACTuples. "
              @"Instead, received: %@",
              tuple);
    NSCAssert(tuple.count > 1, @"-reduceApply must only be used on a signal of "
                               @"RACTuples, with at least a block in tuple[0] "
                               @"and its first argument in tuple[1]");
...
- (RACSignal *)windowWithStart:(RACSignal *)openSignal
                         close:(RACSignal * (^)(RACSignal *start))closeBlock {
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    RACDisposable *openObserverDisposable = [openSignal
        subscribe:[RACSubscriber subscriberWithNext:^(id x) {
          if (currentWindow == nil) {
            currentWindow = [RACSubject subject];
            [subscriber sendNext:currentWindow];

            currentCloseWindow = closeBlock(currentWindow);
            closeObserverDisposable = [currentCloseWindow
                subscribe:[RACSubscriber subscriberWithNext:^(id x) {
                  closeCurrentWindow();
                } error:^(NSError *error) {
                  closeCurrentWindow();
  ...
  }] setNameWithFormat:@"[%@] -windowWithStart: %@ close:", self.name,
                       openSignal];
}
...
- (RACSignal *)buffer:(NSUInteger)bufferCount {
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    RACDisposable *outerDisposable =
        [[self windowWithStart:self
                         close:^(RACSignal *start) {
  ...
  }] setNameWithFormat:@"[%@] -buffer: %lu", self.name,
...
- (RACSignal *)let:(RACSignal * (^)(RACSignal *sharedSignal))letBlock {
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    RACMulticastConnection *connection = [self publish];
    RACDisposable *finalDisposable =
        [letBlock(connection.signal) subscribeNext:^(id x) {
          [subscriber sendNext:x];
        } error:^(NSError *error) {
          [subscriber sendError:error];
        } completed:^{
...
- (RACSignal *)aggregateWithStart:(id)start
                          combine:(id (^)(id running, id next))combineBlock {
  return [self aggregateWithStart:start reduce:combineBlock];
}

- (RACSignal *)aggregateWithStartFactory:(id (^)(void))startFactory
                                 combine:
                                     (id (^)(id running, id next))combineBlock {
