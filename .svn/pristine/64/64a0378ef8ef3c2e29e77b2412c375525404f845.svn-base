...
- (void)setupProfileView {
    ...
    self.profileView.nameLabel.left = self.profileView.avatarView.right + 12.5;
...
- (void)setupToolbar {
    ...
    [self addSubview:self.sgiftButton];
}

- (void)setupGiftView {
    self.giftView = [[BBQDynamicGiftView alloc] initWithFrame:CGRectMake(15, 0, kScreenWidth - 30, 0)];
...
- (void)setLayout:(BBQDynamicLayout *)layout {
    ...
    self.profileView.nameLabel.top = 12;
    self.profileView.sourceLabel.left = self.profileView.nameLabel.left;
    self.profileView.sourceLabel.top = self.profileView.nameLabel.bottom + 3;
    ...
    self.sgiftButton.top = layout.topAnchor;
    self.graphtimeButton.left = 15;
    self.visitsButton.right = kScreenWidth - 15;
    self.sgiftButton.right = self.visitsButton.left - 15;
...
- (UIButton *)graphtimeButton {
    if(!_graphtimeButton) {
        ...
        _graphtimeButton.size = CGSizeMake(34, 34);
...
- (UIButton *)sgiftButton {
    if(!_sgiftButton) {
        _sgiftButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _sgiftButton.size = CGSizeMake(34, 34);
        @weakify(self)
        [_sgiftButton bk_addEventHandler:^(id sender) {
            @strongify(self)
        ...
        [_sgiftButton setImage:[UIImage imageNamed:@"gift_button"] forState:UIControlStateNormal];
    }
    return _sgiftButton;
}
