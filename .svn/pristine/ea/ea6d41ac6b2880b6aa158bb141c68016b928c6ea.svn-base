...
- (UIImage *)rotateInRadians:(CGFloat)radians
      flipOverHorizontalAxis:(BOOL)doHorizontalFlip
                verticalAxis:(BOOL)doVerticalFlip {
  ...
  CGContextRef bmContext = NYXCreateARGBBitmapContext(
      (size_t)rotatedRect.size.width, (size_t)rotatedRect.size.height,
      (size_t)rotatedRect.size.width * kNyxNumberOfComponentsPerARBGPixel, YES);
  if (!bmContext)
    return nil;
  ...
  CGContextTranslateCTM(bmContext, +(rotatedRect.size.width / 2.0f),
                        +(rotatedRect.size.height / 2.0f));
  CGContextRotateCTM(bmContext, radians);

  // Do flips
  CGContextTranslateCTM(bmContext, (doHorizontalFlip ? width : 0.0f),
                        (doVerticalFlip ? height : 0.0f));
  CGContextScaleCTM(bmContext, (doHorizontalFlip ? -1.0f : 1.0f),
                    (doVerticalFlip ? -1.0f : 1.0f));

  /// Draw the image in the bitmap context
  CGContextDrawImage(
      bmContext, CGRectMake(-(width / 2.0f), -(height / 2.0f), width, height),
      self.CGImage);
...
- (UIImage *)rotateInRadians:(float)radians {
  return
      [self rotateInRadians:radians flipOverHorizontalAxis:NO verticalAxis:NO];
}
...
- (UIImage *)rotateImagePixelsInRadians:(float)radians {
  ...
  CGContextRef bmContext =
      NYXCreateARGBBitmapContext(width, height, bytesPerRow, YES);
  if (!bmContext)
    return nil;

  /// Draw the image in the bitmap context
  CGContextDrawImage(bmContext, CGRectMake(0.0f, 0.0f, width, height),
                     self.CGImage);
  ...
  vImageRotate_ARGB8888(&src, &dest, NULL, radians, bgColor,
...
- (UIImage *)rotateImagePixelsInDegrees:(float)degrees {
  return
      [self rotateImagePixelsInRadians:(float)NYX_DEGREES_TO_RADIANS(degrees)];
}
