...
#define SBStringIsIllegalSurrogateHighCharacter(character)                     \
  (((character) >= 0xD800UL) && ((character) <= 0xDFFFUL))
#define SBStringIsSurrogateLowCharacter(character)                             \
  ((character >= 0xDC00UL) && (character <= 0xDFFFUL))
#define SBStringIsSurrogateHighCharacter(character)                            \
  ((character >= 0xD800UL) && (character <= 0xDBFFUL))
...
- (void)setError:(NSString *)error {
  _error = [NSString stringWithFormat:@"%@ at index %lu", error,
...
- (sbjson4_token_t)match:(char *)str
                  retval:(sbjson4_token_t)tok
                   token:(char **)token
  ...
  if ([self haveRemainingCharacters:len]) {
    ...
    [self setError:[NSString
                       stringWithFormat:@"Expected '%s' after initial '%.1s'",
                                        str, str]];
...
- (sbjson4_token_t)getStringToken:(char **)token length:(NSUInteger *)length {
  ...
  for (;;) {
    ...
    case 0 ... 0x1F:
      [self setError:[NSString
                         stringWithFormat:
                             @"Unescaped control character [0x%0.2X] in string",
                             bytes[index]]];
      return sbjson4_token_error;
    ...
    case '\\':
      ...
      if (bytes[index] == 'u') {
        ...
        default:
          [self setError:[NSString
                             stringWithFormat:@"Illegal escape character [%x]",
                                              bytes[index]]];
          return sbjson4_token_error;
        }
      }
...
- (sbjson4_token_t)getToken:(char **)token length:(NSUInteger *)length {
  ...
  case 't': {
    tok =
        [self match:"true" retval:sbjson4_token_bool token:token length:length];
    break;
  }
  case 'f': {
    tok = [self match:"false"
               retval:sbjson4_token_bool
                token:token
               length:length];
    break;
  }
  case 'n': {
    tok =
        [self match:"null" retval:sbjson4_token_null token:token length:length];
    break;
  }
  case ',': {
    index++;
