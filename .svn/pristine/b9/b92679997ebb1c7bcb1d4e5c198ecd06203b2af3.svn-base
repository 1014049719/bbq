...
- (RACSignal *)rac_valuesForKeyPath:(NSString *)keyPath
  ...
  return
      [[[self rac_valuesAndChangesForKeyPath:keyPath
        ...
        // -map: because it doesn't require the block trampoline that
        // -reduceEach: uses
        return value[0];
      }] setNameWithFormat:@"RACObserve(%@, %@)", self.rac_description,
                           keyPath];
}
...
- (RACSignal *)rac_valuesAndChangesForKeyPath:(NSString *)keyPath
  ...
  return [
      ...
      setNameWithFormat:
          @"%@ -rac_valueAndChangesForKeyPath: %@ options: %lu observer: %@",
          self.rac_description, keyPath, (unsigned long)options,
          strongObserver.rac_description];
}

@end

static RACSignal *signalWithoutChangesFor(Class class, NSObject *object,
                                          NSString *keyPath,
                                          NSKeyValueObservingOptions options,
                                          NSObject *observer) {
  ...
  return
      [[class rac_signalWithChangesFor:object
                               keyPath:keyPath
                               options:options
                              observer:observer] map:^(NSDictionary *change) {
        @strongify(object);
...
+ (RACSignal *)rac_signalWithStartingValueFor:(NSObject *)object
                                      keyPath:(NSString *)keyPath
                                     observer:(NSObject *)observer {
  return signalWithoutChangesFor(self, object, keyPath,
                                 NSKeyValueObservingOptionInitial, observer);
}

+ (RACSignal *)rac_signalWithChangesFor:(NSObject *)object
                                keyPath:(NSString *)keyPath
  ...
  return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
    ...
    RACKVOTrampoline *KVOTrampoline = [object
        rac_addObserver:observer
             forKeyPath:keyPath
                options:options
                  block:^(id target, id observer, NSDictionary *change) {
...
- (RACSignal *)rac_signalWithStartingValueForKeyPath:(NSString *)keyPath
                                            observer:(NSObject *)observer {
  return [self.class rac_signalWithStartingValueFor:self
                                            keyPath:keyPath
                                           observer:observer];
}
