...
_##regularExpression =                                                   \
[[NSRegularExpression alloc] initWithPattern:(regex)                 \
options:(option)                \
error:nil];                   \
});                                                                        \
\
...
REGULAREXPRESSION_OPTION(regularExpression, regex,                           \
NSRegularExpressionCaseInsensitive)

REGULAREXPRESSION(URLRegularExpression,
                  @"((http[s]{0,1}|ftp)://"
                  @"[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/"
                  @"[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/"
                  @"=<>]*)?)|(www.[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?"
                  @"(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)")
REGULAREXPRESSION(PhoneNumerRegularExpression, @"\\d{3}-\\d{8}|\\d{3}-\\d{7}|"
                  @"\\d{4}-\\d{8}|\\d{4}-\\d{7}|"
                  @"1+[358]+\\d{9}|\\d{8}|\\d{7}")
...
+ (instancetype)linkWithType:(MLLinkType)type
                       value:(NSString *)value
                       range:(NSRange)range {
    return [MLLink linkWithType:type
                          value:value
                          range:range
             linkTextAttributes:nil
       activeLinkTextAttributes:nil];
}

+ (instancetype)linkWithType:(MLLinkType)type
                       value:(NSString *)value
                       range:(NSRange)range
          linkTextAttributes:(NSDictionary *)linkTextAttributes
    activeLinkTextAttributes:(NSDictionary *)activeLinkTextAttributes {
...
- (void)setActiveLink:(MLLink *)activeLink {
    BOOL isUnChanged =
    (!activeLink && !_activeLink) || [activeLink isEqual:_activeLink];
...
- (void)setAllowLineBreakInsideLinks:(BOOL)allowLineBreakInsideLinks {
    if (allowLineBreakInsideLinks == _allowLineBreakInsideLinks)
        return;
...
- (void)setDataDetectorTypesOfAttributedLinkValue:
(MLDataDetectorTypes)dataDetectorTypesOfAttributedLinkValue {
    _dataDetectorTypesOfAttributedLinkValue =
...
- (void)reSetText {
    ...
    //然后此类内部所有需要重新生成链接的都是调用[super
    //reSetText]，否则只是需要重绘的调用[self reSetText]
...
- (void)commonInit {
    ...
    //默认除了话题和@都检测
    _dataDetectorTypes = MLDataDetectorTypeURL | MLDataDetectorTypePhoneNumber |
    MLDataDetectorTypeEmail |
    MLDataDetectorTypeAttributedLink;
...
- (NSMutableAttributedString *)attributedTextForTextStorageFromLabelProperties {
    ...
    [super attributedTextForTextStorageFromLabelProperties];
    
    //默认的链接样式不是我们想要的，去除它
    [attributedString removeAttribute:NSLinkAttributeName
    ...
    for (MLLink *link in self.links) {
        ...
        if ([link isEqual:self.activeLink]) {
            attributes = link.activeLinkTextAttributes
            ? link.activeLinkTextAttributes
            : self.activeLinkTextAttributes;
            if (!attributes) {
                attributes = @{
                               NSForegroundColorAttributeName : kDefaultLinkColorForMLLinkLabel,
                               NSBackgroundColorAttributeName :
                                   kDefaultActiveLinkBackgroundColorForMLLinkLabel
                               };
            }
        } else {
            attributes = link.linkTextAttributes ? link.linkTextAttributes
        ...
        //        [attributedString removeAttributes:[attributes allKeys]
...
static NSArray *kAllRegexps() {
    ...
    dispatch_once(&onceToken, ^{
        _allRegexps = @[
                        kURLRegularExpression(),
                        kPhoneNumerRegularExpression(),
                        kEmailRegularExpression(),
                        kUserHandleRegularExpression(),
                        kHashtagRegularExpression()
                        ];
    });
    return _allRegexps;
}

- (NSArray *)regexpsWithDataDetectorTypes:
(MLDataDetectorTypes)dataDetectorTypes {
    MLDataDetectorTypes const allDataDetectorTypes[] = {
        MLDataDetectorTypeURL, MLDataDetectorTypePhoneNumber,
        MLDataDetectorTypeEmail, MLDataDetectorTypeUserHandle,
        MLDataDetectorTypeHashtag};
...
- (MLLinkType)linkTypeOfString:(NSString *)string
         withDataDetectorTypes:(MLDataDetectorTypes)dataDetectorTypes {
...
- (NSMutableArray *)linksWithString:(id)string {
    ...
    if ((self.dataDetectorTypes & MLDataDetectorTypeAttributedLink) &&
         ...
         usingBlock:^(id value, NSRange range, BOOL *stop) {
             if (value) {
                 ...
                 if (linkValue.length > 0) {
                     MLLink *link = [MLLink
                                     linkWithType:
                                     [self linkTypeOfString:linkValue
                                      withDataDetectorTypes:
                                      self.dataDetectorTypesOfAttributedLinkValue]
                                     value:linkValue
                                     range:range];
    ...
    for (NSRegularExpression *regexp in regexps) {
        [regexp
         enumerateMatchesInString:plainText
         options:0
         range:textRange
         usingBlock:^(NSTextCheckingResult *result,
             ...
             for (MLLink *link in links) {
                 if (NSMaxRange(NSIntersectionRange(
                                                    link.linkRange, result.range)) > 0) {
                     return;
                 }
             }
             ...
             //这个刚好和MLLinkType对应
             MLLinkType linkType =
             [allRegexps indexOfObject:regexp] + 1;
             
             if (linkType != MLLinkTypeNone) {
                 MLLink *link = [MLLink
                                 linkWithType:linkType
                                 value:[plainText
...
- (MLLink *)linkAtPoint:(CGPoint)location {
    if (self.links.count <= 0 || self.textStorage.string.length == 0 ||
    ...
    CGPoint textOffset;
    //在执行usedRectForTextContainer之前最好还是执行下glyphRangeForTextContainer
    //relayout
    [self.layoutManager glyphRangeForTextContainer:self.textContainer];
    textOffset = [self
                  textOffsetWithTextSize:[self.layoutManager
    ...
    // apple文档上写有说
    // 如果location的区域没字形，可能返回的是最近的字形index，所以我们再找到这个字形所处于的rect来确认
    CGRect glyphRect =
    [self.layoutManager boundingRectForGlyphRange:NSMakeRange(glyphIdx, 1)
                                  inTextContainer:self.textContainer];
    if (!CGRectContainsPoint(glyphRect, location))
        return nil;
    
    NSUInteger charIndex =
    [self.layoutManager characterIndexForGlyphAtIndex:glyphIdx];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.activeLink) {
        NSString *linkText =
        [self.textStorage.string substringWithRange:self.activeLink.linkRange];
        ...
        if (self.activeLink.didClickLinkBlock) {
            ...
            self.didClickLinkBlock(self.activeLink, linkText, self);
                   ...
                   [self.delegate respondsToSelector:@selector(didClickLink:
                                                               linkText:
                                                               linkLabel:)]) {
            [self.delegate didClickLink:self.activeLink
                               linkText:linkText
                              linkLabel:self];
        }
        
        [self performSelector:@selector(setActiveLink:)
                   withObject:nil
                   afterDelay:self.activeLinkToNilDelay];
    } else {
        //        [super touchesEnded:touches withEvent:event];
        if (self.delegate &&
            [self.delegate
             respondsToSelector:@selector(didClickNonLinkAreaInLinkLabel:)]) {
                [self.delegate didClickNonLinkAreaInLinkLabel:self];
            }
    }
}
...
//        respondsToSelector:@selector(didLongPressLink:linkText:linkLabel:)])
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        if (self.delegate &&
            [self.delegate
             respondsToSelector:@selector(
                                          didLongPressNonLinkAreaInLinkLabel:)]) {
                 [self.delegate didLongPressNonLinkAreaInLinkLabel:self];
             }
        //        MLLink *link = [self linkAtPoint:[sender locationInView:self]];
        //        if (link) {
        //            NSString *linkText = [self.textStorage.string
        //            substringWithRange:link.linkRange];
        //
        //            //告诉外面已经长按了某链接
        ...
        //            }else if (self.delegate&&[self.delegate
        //            respondsToSelector:@selector(didLongPressLink:linkText:linkLabel:)]){
        //                [self.delegate didLongPressLink:link linkText:linkText
        //                linkLabel:self];
        //            }
        //        }
    }
...
- (MLLink *)addLinkWithType:(MLLinkType)type
                      value:(NSString *)value
                      range:(NSRange)range {
...
- (NSArray *)addLinks:(NSArray *)links {
    ...
    for (MLLink *link in links) {
        ...
        for (MLLink *aLink in self.links) {
            if (NSMaxRange(NSIntersectionRange(aLink.linkRange, link.linkRange)) >
                0) {
                continue;
            }
        }
